...
export function detectActivityPatterns(
  ...
  for (const [pathname, rawEvents] of pageGroups) {
    ...
    proposals.push({
      ...
      proposed_content: `经常访问 ${pathname}（最近 ${windowDays} 天内，${distinctDays} 天里共 ${effectiveEvents.length} 次独立访问）。`,
  ...
  for (const [eventType, groupedEvents] of actionGroups) {
    ...
    proposals.push({
      ...
      proposed_content: `经常执行 ${eventType}（最近 ${windowDays} 天内，${distinctDays} 天里共 ${groupedEvents.length} 次操作）。`,
      recommended_status: proposalStatus(confidence, distinctDays),
      confidence,
