...
| My Tasks (per-staff task aggregation) | `app/my-tasks/page.tsx` | `app/api/my-tasks`; core computation extracted 2026-09-08 to `lib/my-tasks-data.ts`'s `computeMyTasks()`, shared with `app/api/assistant`'s `my_tasks_summary` tool so the on-screen list and the chat assistant's answers about "my tasks" can never disagree; the page's own daily "Today's Priority" banner is `lib/my-tasks-brief.ts`'s `generateMyTasksBrief()` (Claude-phrased when `ANTHROPIC_API_KEY` is set, else a rule-based sentence — never blocks page load) | `ar_reminder`, `late_filing_companies` (via `mirrored_ar_reminder_id`) | Anthropic API (briefing only — optional, degrades gracefully) | Yes for the 6 AR-Reminder-restricted accounts, whose only other page is AR Reminder itself — a bug here is their primary way to know what's overdue |
...
| Assistant (AI chat widget) | `components/AssistantWidget.tsx` | `app/api/assistant` | reads across most tables above; also `lib/my-tasks-data.ts` and `lib/activity-data.ts` since 2026-09-08 (`my_tasks_summary` / `my_activity_pattern` tools) | Anthropic API | Medium. Was fully anonymous before 2026-09-08 — now also resolves the real caller via `getRequestAccount(req)` so it can answer personal "what should I do today" / "what are my habits" questions; `account` is `null` when that lookup fails and every personal path handles that explicitly (never guesses whose tasks/habits to show) |
