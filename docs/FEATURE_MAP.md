...
| Reports (customer-profile analytics, 2026-09-03) | `app/reports/page.tsx` | `app/api/reports` | `companies`, `master_list` (`list_type`/`join_date`/`update_date`), `quickbooks_invoices`, `ar_reminder` | none beyond the DB itself | Low — gated to 4 named accounts (`ApprovedAccount.canViewReports`), read-only, no writes. `companies.customer_source` (new column) is the one thing staff edit, from Company 360. SSIC-based industry breakdown is a not-yet-built phase 2 (see `PROJECT_STATUS.md`); the client-type chart today is legal entity structure (`company_type`), not industry — don't conflate the two if extending this. |
