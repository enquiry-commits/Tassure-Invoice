...
| Assistant (AI chat widget) | `components/AssistantWidget.tsx` | `app/api/assistant` | reads across most tables above; also `lib/my-tasks-data.ts` since 2026-09-08 (`my_tasks_summary` tool) | Anthropic API | Medium. Was fully anonymous before 2026-09-08 — now also resolves the real caller via `getRequestAccount(req)` so it can answer personal "what should I do today" questions; `account` is `null` when that lookup fails and every personal-task path handles that explicitly (never guesses whose tasks to show) |
