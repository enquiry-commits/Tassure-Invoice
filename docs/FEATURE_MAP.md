...
| `lib/company-name.ts` (`normalize`) | Fuzzy company-name matching wherever a TeamWork `company_name` string must match a `companies` row (ND page, Master List reconciliation, others) | A change here can shift which rows match across many features simultaneously, not just one |
