...
async function checkOutstandingBalance(companyQuery: string) {
  ...
  return {
    ...
    // Only present when the company owes across 2+ QuickBooks books — the
    // real ONE-PDF-across-every-book Statement (/billing/soa/all, see
    // app/api/billing/soa/pdf's own company=ALL mode). Added 2026-09-18
    // after a real gap: a client owing on both TAB and TAO, asked for "All"
    // combined into one document, got told no such combined PDF exists —
    // this tool (and soaDeepLink() itself, see its own comment) used to
    // only ever build per-book links, from back when the combined download
    // didn't exist yet. Omitted when only one book has a balance since it
    // would just duplicate that book's own soa_link.
    ...(result.lines.length > 1
      ? { soa_link_all: soaDeepLink('ALL', result.companyName) }
      : {}),
    ...
    note: result.hasOutstanding
      ? `Real, current outstanding balance from QuickBooks — tell the user the total and, if useful, the breakdown by TAB/TAC/TAO, the oldest aging bucket, and which invoices are unpaid. A real INTERACTIVE CARD (Download SOA PDF + Draft Email buttons for each book${result.lines.length > 1 ? ', plus a combined "ALL" option covering every book at once' : ''}) renders automatically below your reply whenever hasOutstanding is true — do NOT repeat byQbCompany's soa_link (or soa_link_all) as markdown links in your own text, and never say you "can't directly help download" or can only point at a page: the card's own buttons genuinely perform the download or open a real Draft Email popup in one click, this system's usual rule (the user's own click performs the action, never claim you did it yourself) applies the same way here — it does NOT mean chat can't help; confirmed real gap 2026-09-18, a reply undersold this exact capability with bare page links instead of trusting the card.${result.lines.length > 1 ? ' This company owes across MORE THAN ONE QuickBooks book — if the user has not already said which they want, ask in ONE short sentence naming the real choices (one specific book, each book separately, or the combined "All" covering everything) rather than guessing; once they answer, a short confirmation pointing at the card is enough.' : ''} Never suggest Billing Drafts for this.`
...
async function companyUpdatePreviewTool(account: ApprovedAccount | null, input: Record<string, unknown>) {
  ...
  const allowed = ['service:secretary', 'service:accounts', 'service:tax', 'service:xbrl', 'master:remark', 'master:grade', 'trademark:mark_expired_date', 'trademark:status_text', 'trademark:updates_note', 'trademark:remarks', 'billto:care_of', 'billto:addr_source', 'billto:addr_custom', 'billto:attn', 'customer_source', 'parent_company'];
  ...
  const applicationNumber = typeof input.applicationNumber === 'string' && input.applicationNumber.trim() ? input.applicationNumber.trim() : undefined;
  const result = await previewCompanyUpdate(company, field as CompanyUpdateField, value === undefined ? null : value, applicationNumber);
  if (!result.found) {
    if (result.ambiguous) return { found: false as const, ambiguous: true as const, message: result.message, candidates: result.candidates };
    return { found: false as const, message: result.message };
  }
...
- Match depth to the question. A broad "大家在忙什么" gets a short synthesis; "XX 公司具体改了什么" gets every field. Don't pad a 3-item answer into a table, and don't bury a 40-row answer in prose.
...
HARD RULE, confirmed wrong again 2026-09-10: "我要开SOA" (and any 开SOA / 做SOA / 出SOA / SOA 要怎么弄) means the STATEMENT OF ACCOUNT, never a new invoice. Do NOT call preview_invoice_draft for it — not as a main answer, and not as a helpful extra "顺便看看开单" alongside the SOA answer. The Chinese verb 开 is shared between 开发票 and 开SOA and is NOT the signal; the noun SOA is. Call check_outstanding_balance, which gives the user a real SOA card with working download and email buttons. Only reach for preview_invoice_draft if the user separately and explicitly asks about 开单/发票/invoice.
...
const CLAUDE_TOOLS = [
  ...
  { name: 'preview_company_update', description: "Preview a change to ONE company's settings — force a service ON/OFF or clear that override ('service:secretary' | 'service:accounts' | 'service:tax' | 'service:xbrl', value true/false/null), set the customer source ('customer_source'), set/clear the parent company ('parent_company', value = the parent's company name or null), edit the Master List remark ('master:remark', free text) or grade ('master:grade', A/B/C), or update a TRADEMARK record's expiry date ('trademark:mark_expired_date', ISO date), status ('trademark:status_text'), progress note ('trademark:updates_note') or remarks ('trademark:remarks') — pass applicationNumber when the company holds more than one mark. It can also set the INVOICE Bill To defaults: 'billto:care_of' (the \"c/o\" party's name as it should print), 'billto:addr_source' ('b' = the c/o party's own address, 'a' = the client's own, 'custom'), 'billto:addr_custom' (the typed address when source is custom) and 'billto:attn' (an \"Attn: <person>\" line). These print on the real invoice the client receives — ask which address source they want rather than guessing, because it decides where the invoice is sent. READ-ONLY: it shows the real current value and what it would become; the user gets a Confirm popup and ONLY their click saves it. Use for e.g. \"把 XX 的 XBRL 服务打开\", \"XX 的客户来源是转介绍\", \"把 XX 设为 YY 的子公司\". ND and Address flags follow TeamWork and are NOT changeable here, and remark/grade are the ONLY Master List fields editable from chat (the compliance dates belong to preview_ar_update; directors/shareholders/secretary/status are written by the TeamWork sync). Never claim you performed the change yourself.", input_schema: { type: 'object', properties: {
    company: { type: 'string', description: 'Company name, partial match is fine' },
    field: { type: 'string', enum: ['service:secretary', 'service:accounts', 'service:tax', 'service:xbrl', 'master:remark', 'master:grade', 'trademark:mark_expired_date', 'trademark:status_text', 'trademark:updates_note', 'trademark:remarks', 'billto:care_of', 'billto:addr_source', 'billto:addr_custom', 'billto:attn', 'customer_source', 'parent_company'] },
    applicationNumber: { type: 'string', description: 'Only for trademark:* — which mark, when the company has more than one. Omit it and the tool will list the real application numbers to choose from.' },
  ...
  { name: 'service_pricing_lookup', description: 'Tassure\'s own STANDARD service catalog (company incorporation, secretarial services, EP/DP applications, accounting/tax/audit, and a full post-incorporation-changes fee schedule), plus company policy text (payment terms, termination/refund, confidentiality). Each row carries BOTH a price AND a real bilingual description of the actual scope/steps that service covers — use it for "你们提供什么服务", "XX包含什么", "XX怎么运作的/流程是什么" just as much as for "XX多少钱/费用是多少". This is a LIST price reference, NOT a specific client\'s real invoice — never use it to answer "how much does THIS client owe" or "what was THIS company billed" (use check_outstanding_balance / preview_invoice_draft for that). Omit both filters to see the whole catalog grouped by section; pass `search` to find one service by name/keyword (English or Chinese); pass `section` to browse one group (e.g. "First-Year Package", "Ongoing Maintenance", "Post-Incorporation Changes").', input_schema: { type: 'object', properties: {
