...
async function checkOutstandingBalance(companyQuery: string) {
  ...
  return {
    ...
    note: result.hasOutstanding
      ? `Real, current outstanding balance from QuickBooks — tell the user the total and, if useful, the breakdown by TAB/TAC/TAO, the oldest aging bucket, and which invoices are unpaid.${result.lines.length > 1 ? ' This company owes across MORE THAN ONE QuickBooks book — if the user wants "all of it"/"everything"/"合并"/"一起"/"All的" as ONE document, present soa_link_all (a single real combined Statement covering every book they owe on), not each line\'s own soa_link separately; if they want one specific book\'s own copy instead, use that line\'s soa_link.' : ''} If the user wants the SOA (Statement of Account) PDF or wants to send it to the client, present the relevant link(s) as clickable markdown — that page has the real "Download SOA PDF" and "Draft Email" buttons, already open to this company; never suggest Billing Drafts for this, and never claim a combined PDF doesn't exist when soa_link_all is present in this result.`
