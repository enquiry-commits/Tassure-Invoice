...
interface HistoryRow {
  ...
  status: 'pending' | 'opened' | 'sent' | 'skipped'; total_amount: number | null;
    ...
    <div>
      ...
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '60px 1.4fr 1fr 100px 100px 130px 140px', padding: '8px 14px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', fontSize: 10.5, fontWeight: 700, color: '#64748b', textTransform: 'uppercase' }}>
          <div>Type</div><div>Company</div><div>Subject</div><div style={{ textAlign: 'right' }}>Amount</div><div style={{ textAlign: 'center' }}>Status</div><div>Activity</div><div>By</div>
        </div>
        <div style={{ maxHeight: 'calc(100vh - 340px)', overflowY: 'auto' }}>
          {loading ? (
            ...
            const accent = r.status === 'sent' ? '#16a34a' : r.status === 'opened' ? '#2563eb' : r.status === 'skipped' ? '#94a3b8' : '#c2410c';
            return (
              <div key={r.id} style={{ display: 'grid', gridTemplateColumns: '60px 1.4fr 1fr 100px 100px 130px 140px', alignItems: 'center', padding: '9px 14px', borderBottom: '1px solid #f1f5f9', borderLeft: `3px solid ${accent}`, background: i % 2 === 0 ? '#fff' : '#fafbfc', fontSize: 12 }}>
                ...
                <div style={{ textAlign: 'center' }}>
                  <span style={{ fontSize: 9.5, fontWeight: 700, color: accent, background: `${accent}14`, borderRadius: 4, padding: '2px 7px', textTransform: 'uppercase' }}>{STATUS_LABEL[r.status] ?? r.status}</span>
                </div>
                <div style={{ color: '#94a3b8', fontSize: 11 }}>{r.sent_at ? new Date(r.sent_at).toLocaleString() : r.opened_at ? `Opened ${new Date(r.opened_at).toLocaleString()}` : '—'}</div>
                <div style={{ color: '#94a3b8', fontSize: 11, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.sent_by_name ?? r.opened_by_name ?? '—'}</div>
              </div>
            );
          })}
        </div>
