...
async function getData() {
  ...
  const [{ data: nds }, { data: appts }, { data: reviewRows }, { data: latestRuns }, { data: companies }] = await Promise.all([
    ...
    // Vincent, 2026-08-29 (rebalanced to 5 batches 2026-08-31, roster grew
    // past 4 batches' 3-worker capacity): the daily ND scrape now runs as
    // batches (teamwork_nd_1..5 — see app/api/teamwork/sync-nd/route.ts) —
    // without this, this card would silently show "Pending" forever
    // post-deploy since nothing writes source='teamwork_nd' on a daily
    // cron anymore.
    supabase.from('automation_sync_runs')
      .select('summary')
      .in('source', ['teamwork_nd', 'teamwork_nd_1', 'teamwork_nd_2', 'teamwork_nd_3', 'teamwork_nd_4', 'teamwork_nd_5'])
      .eq('status', 'success')
