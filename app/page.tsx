...
function Card({ title, eyebrow, icon, children, action, style }: {
  ...
  return (
    <section style={{ background: '#fff', borderRadius: 16, border: '1px solid #e4eaf1', boxShadow: '0 8px 30px rgba(16,37,66,.045)', padding: '20px 22px', ...style }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
        <span style={{ width: 34, height: 34, borderRadius: 10, background: '#f0f6ff', color: '#1d4f7c', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{icon}</span>
        <div>
          ...
          <h2 style={{ fontSize: 14, fontWeight: 750, color: '#173252', margin: 0, letterSpacing: '-.01em' }}>{title}</h2>
        </div>
...
function Kpi({ label, value, sub, Icon, tint, href }: {
  ...
  return (
    <Link href={href} className="dashboard-kpi" style={{ display: 'block', background: '#fff', borderRadius: 14, border: '1px solid #e4eaf1', boxShadow: '0 5px 20px rgba(16,37,66,.035)', padding: '16px 17px', textDecoration: 'none' }}>
      ...
      <div style={{ fontSize: 28, fontWeight: 800, color: '#12233b', lineHeight: 1, letterSpacing: '-.035em' }}>{value}</div>
...
function ActionItem({ title, description, value, href, color, background, Icon }: {
  ...
  return (
    <Link href={href} className="dashboard-action" style={{ display: 'grid', gridTemplateColumns: '38px 1fr auto', alignItems: 'center', gap: 12, padding: '13px 0', textDecoration: 'none', borderBottom: '1px solid #eef2f6' }}>
      ...
      <span>
        <span style={{ display: 'block', fontSize: 12.5, fontWeight: 750, color: '#1e3a5f' }}>{title}</span>
...
function TextLink({ href, children }: { href: string; children: React.ReactNode }) {
  return <Link href={href} style={{ fontSize: 11.5, color: '#1d4f7c', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 4, textDecoration: 'none' }}>{children}<ArrowRight size={12} /></Link>;
}
  ...
  return (
    <div style={{ maxWidth: 1760, margin: '0 auto' }}>
      <header style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 18, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 4 }}>
            ...
            <span style={{ fontSize: 10, fontWeight: 800, color: '#0f766e', textTransform: 'uppercase', letterSpacing: '.9px' }}>Live operations</span>
          </div>
          <h1 style={{ fontSize: 23, fontWeight: 800, color: '#12233b', margin: 0, letterSpacing: '-.03em' }}>Portfolio Overview</h1>
      ...
      {!data ? (
        ...
        <>
          <section className="dashboard-hero" style={{ position: 'relative', overflow: 'hidden', borderRadius: 18, background: 'linear-gradient(115deg,#173654 0%,#1d4f7c 62%,#176b70 100%)', padding: '22px 26px', marginBottom: 15, color: '#fff', boxShadow: '0 15px 35px rgba(23,54,84,.15)' }}>
            ...
            <div className="dashboard-hero-grid" style={{ position: 'relative', display: 'grid', gridTemplateColumns: '1.35fr repeat(3,.55fr)', alignItems: 'center', gap: 18 }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 10, fontWeight: 800, color: '#a7f3d0', textTransform: 'uppercase', letterSpacing: '.8px', marginBottom: 8 }}><Sparkles size={12} /> Portfolio command centre</div>
          ...
          <div className="dashboard-kpis" style={{ display: 'grid', gridTemplateColumns: 'repeat(5,minmax(150px,1fr))', gap: 12, marginBottom: 15 }}>
            <Kpi label="Active Clients" value={data.kpis.activeClients} sub={`${data.kpis.cssClients} CSS clients`} Icon={Building2} tint="#0f766e" href="/master-list/active-clients" />
            ...
            <Kpi label="Address Service" value={data.kpis.addressClients} sub="registered-address clients" Icon={MapPin} tint="#2563eb" href="/address-service" />
            <Kpi label="Upcoming AR" value={data.kpis.upcomingAR} sub="rolling six-month window" Icon={CalendarClock} tint="#d97706" href="/billing?tab=ar" />
            <Kpi label="Late-Filing Watch" value={data.kpis.lateFiling} sub="companies currently flagged" Icon={AlertTriangle} tint="#e11d48" href="/late-filing" />
          </div>
          ...
          <div className="dashboard-grid-primary" style={{ display: 'grid', gridTemplateColumns: '1.45fr .75fr', gap: 15, marginBottom: 15 }}>
            <Card title="Upcoming Annual Return Workload" eyebrow="Six-month outlook" icon={<CalendarClock size={16} />} action={<TextLink href="/billing?tab=ar">Open AR Reminder</TextLink>}>
              <VBars data={data.upcomingAR} color="#2563eb" height={156} />
            </Card>
            <Card title="Action Centre" eyebrow="Priority queue" icon={<ShieldCheck size={16} />} action={<span style={{ fontSize: 10, color: '#94a3b8' }}>Live counts</span>}>
              <ActionItem title="Late-filing review" description="Investigate and resolve flagged entities" value={data.kpis.lateFiling} href="/late-filing" color="#dc2626" background="#fef2f2" Icon={AlertTriangle} />
              <ActionItem title="AR preparation window" description="Review the next six months of filings" value={data.kpis.upcomingAR} href="/billing?tab=ar" color="#d97706" background="#fff7ed" Icon={Clock3} />
              <ActionItem title="Billing drafts" description="Review services before creating invoices" value="Open" href="/billing?tab=billing" color="#2563eb" background="#eff6ff" Icon={BriefcaseBusiness} />
            </Card>
          </div>
          ...
          <div className="dashboard-grid-secondary" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 15, marginBottom: 15 }}>
            <Card title="Client Portfolio Status" eyebrow="Entity lifecycle" icon={<Layers3 size={16} />} action={<TextLink href="/companies">View companies</TextLink>}>
              <Donut segments={data.statusDonut} size={154} thickness={22} />
            </Card>
            <Card title="Service Coverage" eyebrow="Active-client mix" icon={<BriefcaseBusiness size={16} />} action={<span style={{ fontSize: 10.5, color: '#94a3b8' }}>number of clients</span>}>
              <HBars data={data.serviceMix} accent="#0f766e" labelWidth={105} />
            </Card>
          </div>
          ...
          <div style={{ marginBottom: 15 }}>
            <Card title="Financial Year-End Calendar" eyebrow="Annual distribution" icon={<BarChart3 size={16} />} action={<span style={{ fontSize: 10.5, color: '#94a3b8' }}>active clients by FYE month</span>}>
              <VBars data={data.fyeMonths} color="#0f766e" height={165} />
            </Card>
          </div>
          ...
          <Card title="Nominee Director Portfolio" eyebrow="Active appointment load" icon={<Users size={16} />} action={<TextLink href="/nominee-directors">Open directory</TextLink>}>
            {data.topNDs.length > 0 ? <HBars data={data.topNDs} accent="#7c3aed" labelWidth={170} /> : <div style={{ padding: 30, textAlign: 'center', color: '#94a3b8', fontSize: 12 }}>No active nominee appointments.</div>}
          </Card>
        </>
      )}
