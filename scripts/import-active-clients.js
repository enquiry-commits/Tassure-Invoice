// Import the 3-sheet Active Clients Excel file into the shared `master_list` Supabase table.
// Run: node scripts/import-active-clients.js
...
const FILE_PATH = 'C:\\Users\\vincent\\Desktop\\Copy of 2026.04.10- Master Client List (ACTIVE CLIENTS).xlsx';

// Field maps per sheet — column index → DB field name
const SHEETS = [
  {
    sheetName: 'ACTIVE CLIENT',
    listType: 'active_client',
    fields: [
      'internal_code', 'company_name', 'roc_no', 'status', 'join_date',
      'add_here', 'invoice_address', 'mailing_address', 'contact_window', 'email', 'tel',
      ...
      'remark', 'referral', 'risk_level', 'incorp_with_us', 'mas', 'grade',
    ],
  },
  {
    sheetName: 'AD-HOC',
    ...
    fields: [
      'internal_code', 'company_name', 'roc_no', 'status', 'join_date',
      'sec_agent', 'kyc_year', 'register_of_controllers', 'corporate_tax', 'efiling_authorization',
      'ac', 'audit', 'gst', 'compil_report', 'cpf_submit',
  ...
  },
  {
    sheetName: 'MAS',
    listType: 'mas',
    fields: ['internal_code', 'company_name', 'roc_no', 'fye', 'last_accounts_date', 'next_agm_due_date', 'mas'],
  },
];
...
async function run() {
  ...
  for (const { sheetName, listType, fields } of SHEETS) {
