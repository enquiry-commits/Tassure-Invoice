...
Last updated: 2026-08-11 (Vincent pushed back on the multi-night rotation design directly: "我要的是每一天都能分批轮完，但是一天内要轮完全部公司，不是一天只轮250家，这样数据就很难同步了." That design (250 companies/run, oldest-`secretary_synced_at`-first) existed specifically to fit under Vercel's classic 300s function timeout — the route's own comment measured ~500ms/company fixed TeamWork throughput regardless of concurrency, so all ~783 current Active Client companies would take ~390s, over that cap. Checked whether the underlying constraint still holds rather than just cranking numbers: queried the Vercel project API directly and confirmed `resourceConfig.fluid: true` — Fluid Compute is enabled on this deployment, which raises the real duration ceiling well past 300s. Removed the batch cap entirely instead of adding more cron triggers per day: `sync-secretary/route.ts`'s `maxDuration` 300 → 650 and `BATCH_SIZE` 250 → 1200 (both with real headroom above the current ~783-company/~390s baseline for the roster to grow before needing another look), so the single existing 18:45 UTC cron now sweeps every Active Client company every night instead of rotating over several. `npx tsc --noEmit`/`npm run build` clean; will confirm the next real nightly run actually completes within the new duration rather than assuming the Fluid Compute ceiling is high enough. This also makes the earlier `UPDATE master_list SET secretary_synced_at = null WHERE id = 757` fix (to bump 1V Capital to the front of the next batch) unnecessary — a full sweep touches every company regardless of that ordering now, so Vincent doesn't need to run it.
