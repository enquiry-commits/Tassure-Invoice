...
Last updated: 2026-09-21 (SHIPPED: restored SOA Remarks as dedicated column matching Billing Drafts). Removed the cramped `Add remark` chip/editor from inside the Company Name cell so company headers remain clean and unbloated. Remarks is restored as a dedicated grid column immediately before Mail (160px width, left-aligned header), rendering Billing Drafts' exact clean auto-resizing white textarea style (`SoaRemarksInput`). For single-source companies and multi-source group parent rows, the company-level remark is directly editable with auto-save on blur; expanded child rows (TAB/TAO/TAC breakdown) leave this cell clean and blank, preserving 100% native CSS Grid column alignment without absolute-overlay drift or visual clutter. `npx tsc --noEmit` and `git diff --check` pass clean.

Last updated: 2026-09-21 (LOCAL, NOT YET PUSHED: default-expanded SOA source groups + shared company Remarks). Every multi-source company in Outstanding All now starts expanded. A new Remarks column sits immediately before the mail action; one company-level input visually spans the parent Total row and every TAB/TAC/TAO child, while single-source and individual book pages edit the same value normally. Remarks persist in new `soa_remarks`, keyed by normalized customer name (not book and not companies.id), so All/TAB/TAC/TAO always share one note even for QuickBooks-only customers. Migration required: `scripts/add-soa-remarks.sql`.

Last updated: 2026-09-21 (LOCAL, NOT YET PUSHED: multi-source SOA group hierarchy refinement). When a 2/3-source company is expanded, its combined Total row now switches to a slightly deeper gray-blue background and darker left rule, clearly separating the parent total from its white source children. The All view's Source column is widened from 64px to 120px and TAB/TAC/TAO badges are compact, non-wrapping flex items, so both two-source and three-source sets remain side-by-side.

Last updated: 2026-09-21 (LOCAL, NOT YET PUSHED: single-source SOA All rows no longer expand into a redundant duplicate). A company with only one active TAB/TAC/TAO source now stays as one complete row with its real Main PIC selector and source-scoped Draft Email action. Only genuine multi-source company groups expose expandable child rows. The Main PIC selector was extracted into one shared component so the single-row and expanded-source paths cannot drift.

Last updated: 2026-09-21 (LOCAL, NOT YET PUSHED: grouped SOA All view). The All Outstanding list now renders one parent row per normalized company and expandable TAB/TAC/TAO source rows rather than visually duplicating the company. The parent summarizes combined aging/total/PIC, shows one unified Reminder when every source agrees or a compact `Mixed` breakdown when they differ, and provides the combined draft action; each child keeps its own amount, Main PIC editor, Reminder, source detail, and source-only draft action. Combined sends still persist `ALL` and flow back to every included source page; source-only sends flow back only to the matching All child and source page. A mixed parent defaults to the earliest source `nextStage` so no source is skipped, while the picker remains manually selectable for companies whose earlier reminders pre-date this system. No SQL change required. `npx tsc --noEmit`, `git diff --check`, and the final production build pass.

Last updated: 2026-09-21 (LOCAL, NOT YET PUSHED: SOA Reminder visual follow-up). The Outstanding list now centers each Reminder value under its centered header. The shared `SoaDownloadPopover` now renders through a fixed `document.body` portal, automatically choosing above/below based on viewport space, so Company 360's scrollable Outstanding card and the SOA detail modal can no longer clip the Download SOA PDF choices.

Last updated: 2026-09-21 (LOCAL, NOT YET PUSHED: implemented verified SOA Reminder progression across Outstanding and Company 360). SOA template pickers/settings now expose only `1st Reminder`, `2nd Reminder`, `3rd Reminder` (old rows stay hidden, not deleted, because historical campaigns may reference them). New migration `scripts/add-soa-reminder-tracking.sql` adds per-draft `soa_reminder_stage`, `soa_qb_company`, and `outlook_send_verified_at`. The last field is written only after the local Outlook Helper returns success from Outlook `.Send()`; Delivery History's manual “Mark as Sent” deliberately does not set it and therefore cannot create a false Done. Outstanding pages now show a Reminder column immediately after Company Name; Company 360's Outstanding section mirrors the same status. After a verified 1st send the row shows `1st Reminder (Done)` and its Draft Email picker defaults to 2nd, then 3rd after a verified 2nd. ALL-scope sends apply across each book; TAB/TAC/TAO sends stay book-specific. `npx tsc --noEmit`, focused reminder-sequence logic checks, `git diff --check`, and the final `npm run build` all pass. SQL has not been run.
...
Fixed by giving `SoaDetail` (`app/billing/soa/_components.tsx`) a new `SoaDownloadPopover`, shown only when `qbCompany === 'ALL'`: clicking "Download SOA PDF" now lists every book with a real balance (derived straight from the same `invoices` array the line-item table already renders — can't disagree with what's on screen) as its own one-click single-book download, plus — Vincent's own follow-up refinement, "再优化一点就是点击下载 TAB / TAO / All (TAB+TAO)" — an explicitly-labeled combined option at the bottom, shown only when 2+ books actually have a balance, still calling the exact same unchanged `downloadSoaPdf(name, 'ALL')` from the earlier fix. Single-book pages (TAB/TAC/TAO on their own, never 'ALL') keep the original plain button, just parametrized to share the same download handler.

Verified the same way as the earlier fix, against the same real company: directly queried the real AR aging snapshot for "1V Capital Pte. Ltd." (not the API route, which needs a real login session) and confirmed TAB has 1 open row, TAO has 3, TAC has 0 — so the picker's book list is exactly `['TAB', 'TAO']` and the combined option reads `All (TAB+TAO)`, matching his screenshot exactly. `npx tsc --noEmit` / `npx eslint` / `npm run build` all clean (the 3 pre-existing lint errors in this file are unrelated — confirmed via `git stash` that they're identical before and after this change). `docs/INVARIANTS.md` INV-DOC-020 updated in place (not a new entry) since it's a correction of an already-wrong claim in an evergreen reference doc, not a new fact — `PROJECT_STATUS.md`'s own log convention is kept separate below.

Previous entry follows.

Last updated: 2026-09-18 (FIXED: the combined "All" SOA draft email now attaches each relevant book's own real Statement PDF separately (e.g. TAB's own + TAO's own, 2 files) instead of one merged cross-book PDF — Vincent corrected his own earlier framing of what "All" should do here).

Vincent: "其实是当我在All 的时候，就要出现TAB/TAO/TAC 单独的3个SOA PDF，而这3个SOA PDF 要加到All 的 Draft 内...类似于截图中只有 TAB/TAO两家公司，所以在Drafts 的时候就只需要附带 TAB/TAO 的SOA PDF，不需要TAC的" — for a company owing on 2 of 3 books, the draft needs each book's own complete Statement PDF as its own attachment, not one combined file, and never a third attachment for a book with no real balance. The standalone "Download SOA PDF" button (`downloadSoaPdf()`) is deliberately unchanged — his own follow-up ("当然在外面Download PDF的时候可以单独下载选择 TAB还是TAO的 SOA PDF") confirmed that part was never wrong.

Only `buildSoaDraft()` (`lib/soa-actions-client.ts`) changed: for `qbCompany === 'ALL'`, it now tries all 3 books' own single-book PDF endpoint (the same one each book's own download button already calls) in parallel and keeps only the ones that succeed — a 404 there already means "no real balance for this book," the exact same signal the standalone button already relies on, just reused rather than re-derived. `buildCampaignDraft()`'s `attachment?: File | null` widened to `attachments?: File[] | null` to carry multiple files — safe since `additional_attachments` on `DraftLike` was already `File[]` end-to-end (the real Outlook draft creation already handles multiple attachments elsewhere in this app), and a full `tsc` pass confirmed `buildSoaDraft()` is the only real caller of that param.

Verified against the real 1V Capital example from Vincent's own screenshot, not just reasoning about it: directly queried `quickbooks_invoices`/`quickbooks_credit_memos` (the same tables the PDF route's own 404 check reads) and confirmed TAB and TAO both genuinely have real invoice rows (their PDF fetch will succeed) while TAC genuinely has none (its fetch will 404 and get correctly skipped) — the draft will attach exactly 2 files, matching what he asked for. `npx tsc --noEmit` / `npx eslint` / `npm run build` all clean. `docs/INVARIANTS.md` INV-DOC-020.

Previous entry follows.

Last updated: 2026-09-18 (FIXED, and this time with a real deterministic backstop, not just prompt wording: the SOA "no tool can download this" false claim happened AGAIN, immediately after today's earlier prompt-only fix, on a short follow-up message — added a code-level guard, same family as the existing outstanding-balance/permission-denied guards, that catches the exact false claim and forces a retry).

Vincent's own reply after trying the earlier fix live: "这边的还是不完善" — a real chat showed the FIRST reply correctly listing TAB/TAO/All choices (today's earlier fix working), then a short follow-up just naming which one ("我要下载 All的") got "抱歉，我这边没有任何工具能直接下载或生成 PDF" — an even more absolute wrong claim than the original bug, on a request that was no longer even ambiguous. Root cause: a terse follow-up that only narrows an earlier SOA answer (no company re-named) reads to the model as answerable from conversation memory alone, so `check_outstanding_balance` never gets called again for that turn, no fresh card attaches, and with no tool result in hand the model fell back to a fabricated "I have no tool for this."

Two fixes, not one: (1) added an explicit HARD RULE to the static system prompt — any SOA-related follow-up, even a bare format name with no company repeated, must call `check_outstanding_balance` again every time, no exceptions; (2) — the more durable fix, since this is the SECOND time prompt wording alone didn't hold — added `claimsNoSoaDownloadTool()`, a deterministic regex guard in the same family as the existing `mentionsOutstandingBalance`/`claimsPermissionDenied` guards (both born from identical past incidents: the model confidently asserting something false that a real tool call would have caught). It fires whenever the reply flatly claims no tool exists to download/generate an SOA PDF AND the outstanding-balance tool wasn't actually called that turn, prepending a warning and telling the user to re-ask more explicitly. Verified against the real failure text from the screenshot (not a guess): the exact false claim is caught, the earlier bug's own wording is also caught, and a correctly-behaving reply is NOT falsely flagged. `npx tsc --noEmit` (caught and fixed a real stray-backtick syntax error inside the prompt's own template literal before this could have shipped broken) / `npx eslint` / `npm run build` all clean.

Previous entry follows.

Last updated: 2026-09-18 (FIXED: the SOA chat guidance no longer tells the model to undersell its own real download/draft capability — it used to reply "我没法直接帮你下载文件" and dump bare page links even though the real interactive card, with genuine one-click Download PDF/Draft Email buttons, was already rendering right below).

Vincent, on a real chat reply for a real 2-book company: "这种环节，你应该是问要下载哪个，如果我说都要下载，你就自动帮我下载两个，如果说我只要All的，你就帮我下载All的 SOA pdf, 然后如果我说要Draft 的，你要好像 Billing Drafts 那样可以打开那个 SOA drafts 的弹窗". Investigated what the assistant actually has available for this: `check_outstanding_balance` already auto-attaches a real interactive `SoaCard` below the reply whenever `hasOutstanding` is true (real Download SOA PDF / Draft Email buttons per book, PLUS today's earlier combined "ALL" row) — the Draft Email button already opens a real popup, `OutlookStyleSendModal`, the exact same pattern Billing Drafts' own full editor uses. The capability Vincent asked for already existed; the actual bug was the system prompt's own guidance telling the model to enumerate `soa_link`/`soa_link_all` as separate markdown text links and never explicitly correcting the impression that chat "can't help download" — which is exactly what produced the real reply he flagged.

Rewrote both the tool's dynamic `note` (`checkOutstandingBalance()`) and the static SOA prose block: stop repeating each line's link as text (the card already shows it, with real buttons instead of navigation-only links), explicitly say the card's buttons genuinely perform the download/draft in one click (same "user's own click performs the action" rule as everywhere else in this app, which does NOT mean chat can't help), and — matching Vincent's own ask — when a company owes on 2+ books and the request doesn't already say which, ask in one short sentence naming the real choices before assuming. Also explicitly named the exact hypothetical-sounding phrasing from the real example ("如果我要你帮我下载...可以吗") as still meaning a real request, not just conversation.

Same limitation as today's earlier `web_search` addition: this is system-prompt guidance, not testable via a direct function call — no `ANTHROPIC_API_KEY` available locally to verify the model's actual behavior changes as intended. `npx tsc --noEmit` / `npx eslint` / `npm run build` all clean; the real test is Vincent trying the same question again in the live app.

Previous entry follows.

Last updated: 2026-09-18 (SHIPPED: the chat assistant's structured SOA card now has its own combined "ALL" row — download PDF / draft email for every book at once — not just a text link above the card; Vincent: "我要这边多一个All的").

Closes the same gap as today's earlier `soa_link_all` fix (INV-AI-002), but inside `SoaCard` itself (`components/assistant/ChatCards.tsx`), not just the free-text reply above it: that fix made the assistant's PROSE correctly offer a combined download link when a company owes across 2+ books, but the structured card underneath still only ever showed one row per book, with no combined option baked into the card's own buttons. Added a highlighted "ALL" section (shown only when `preview.lines.length > 1`) with its own "下载合并 SOA PDF"/"起草合并邮件" buttons, reusing the exact same `downloadSoaPdf`/`buildSoaDraft` actions the per-book rows already call — both already accept `'ALL'` via `SoaCompanySelector` from the earlier combine-books work, so this needed no new plumbing, just wiring up the UI. Also fixed the card's bottom "Open in SOA" link, which used to always default to `lines[0]`'s own single-book page even when the company owed on multiple books — now goes to the real combined `/billing/soa/all` page in that case. `npx tsc --noEmit` / `npx eslint` / `npm run build` all clean.

Previous entry follows.

Last updated: 2026-09-18 (FIXED: chat assistant's "查看完整发票并生成" button — its pencil icon sat off the text baseline, from a `marginRight`/`verticalAlign` hack on just the icon instead of `display: flex` on the button itself, which its own sibling link right below it already used correctly).

Vincent: "这个的UI 为什么不对的不在一个平行线上" pointing at that exact button in `components/assistant/ChatCards.tsx`. Fixed by adding `display: flex, alignItems: center, justifyContent: center, gap: 5` to the button and dropping the now-unnecessary inline `marginRight`/`verticalAlign` on the `Pencil` icon — the same `deepLinkStyle` pattern the "Open in Billing Drafts" link one line below it already uses, which is exactly why that one looked fine and this one didn't. Checked the rest of the file for the same fragile pattern (icon + manual margin/verticalAlign with no flex on the parent) — this was the only instance. `npx tsc --noEmit` / `npx eslint` / `npm run build` all clean; visually confirmed the alignment fix with a side-by-side before/after mockup in the browser pane.

Previous entry follows.

Last updated: 2026-09-18 (FIXED: Company 360's Outstanding panel's invoice numbers are now the same gray clickable chip as Billing Drafts/the SOA detail modal, opening the real QuickBooks PDF, instead of bare text).

Vincent: "这边的单号要弄成和 Billing Drafts 那种灰色UI格式，可以打开点击的" — same request he made for the SOA detail modal on 2026-09-16 ("SOA 里面的可点击式INVOICE 号码UI格式能不能设计成和 Billing Drafts的那个INVOICE 格式那样灰色的"), which is exactly why that request already resulted in a SHARED component, `BillingInvoiceReference` (`components/billing/BillingInvoiceReference.tsx`) — this just needed reusing in one more place, `OutstandingSection` (`app/companies/[id]/_components.tsx`), not a new implementation. Only Invoice/Credit Note lines get the clickable chip — a Payment/Journal Entry/Deposit line has no real QuickBooks PDF document behind it (same established `canOpenPdf`-style guard the SOA detail modal already uses), so those stay the plain tagged text they were before, not a dead link. Confirmed against 1V Capital's own real Outstanding data (the same example throughout today): the Journal Entry and Payment lines correctly stay plain text with their colored tag, the two real invoices (02610894, 02660519) correctly become clickable gray chips. `npx tsc --noEmit` / `npx eslint` / `npm run build` all clean.

Previous entry follows.

Last updated: 2026-09-18 (SHIPPED: TAO billing's "+ Add new company" now has a real undo — a Delete button on any never-billed row, safely gated so it can only ever remove a company with zero real dependent data anywhere and no real TeamWork sync).

Vincent asked for this after I manually deleted a real example for him (a placeholder "AAAA" row from testing the Add button): "以后这种自己在系统开的公司for 开单的，能不能可以添加过后删除" (companies I create myself in the system for billing — can they be deletable after adding). Added `DELETE /api/billing/tao`, the symmetric undo to that same route's existing manual-add `POST` (2026-09-05) — deliberately narrow, never a general company-delete tool: it refuses outright if TeamWork has ever synced the company (a real `tw_status`), and separately refuses if ANY real dependent row exists anywhere in the system (AR Reminder, email drafts, QuickBooks invoices/credit memos, generated invoices, trademark records, ND appointments, Post Incorporate records), naming the specific real reason(s) rather than a generic failure. A query error in any of those checks throws rather than silently defaulting to "0, safe" — that distinction matters here specifically because a false negative would mean real client data getting deleted.

Verified against real data both ways before shipping, not just the happy path: a fresh throwaway company (built exactly like the real POST handler would) correctly passed with zero blockers; "1V Capital Pte. Ltd." (known real AR Reminder/QuickBooks/Post Incorporate history from earlier this same session) correctly came back with 5 real blocking reasons. UI: a small trash icon next to the company name, only on rows with no invoice history yet (`lastInvoice === null`, the same "Never billed" status already shown), reusing the existing `ConfirmDeleteModal` component. `npx tsc --noEmit` / `npx eslint` / `npm run build` all clean. `docs/INVARIANTS.md` INV-DATA-054.

Previous entry follows.

Last updated: 2026-09-18 (FIXED: the AI assistant now knows about the combined "All books" SOA PDF — a real feature this same session shipped on 2026-09-17 that the assistant's own tool never learned about, so it confidently told a real user no such combined download exists).

Vincent flagged a chat screenshot: asked the assistant for "All" of 1V Capital's combined outstanding (owed on both TAB and TAO), it replied there's no single combined PDF and offered the two books' SOAs separately to download one at a time — wrong, and worse than not answering, since `/billing/soa/pdf?company=ALL` is a real, already-shipped, extensively-tested feature from earlier today (INV-DOC-012 through 018). Root cause: `lib/deep-links.ts`'s `soaDeepLink()` had an explicit 2026-09-09 comment/type saying `qbCompany` must never be `'ALL'` — true the day it was written, never revisited once the combined PDF actually shipped a week later. `checkOutstandingBalance()` (`app/api/assistant/route.ts`) only ever built per-book links off that same stale restriction.

Fixed by widening `soaDeepLink()` to accept `'ALL'` (it already reuses the same `/billing/soa/all` page and `openCompany` deep-link param every other SOA page uses — no new page logic needed) and having `checkOutstandingBalance()` also return `soa_link_all` whenever a company owes across 2+ QuickBooks books, with routing guidance (tool description + system prompt) telling the model to offer it for "全部/All/合并/一起" requests instead of the per-book links. Verified against the exact real case from the screenshot: 1V Capital, TAB $1,000 + TAO $2,650 = $3,650 — `soa_link_all` now correctly resolves to `/billing/soa/all?openCompany=1V+CAPITAL+PTE.+LTD.`. `npx tsc --noEmit` / `npx eslint` / `npm run build` all clean. `docs/INVARIANTS.md` INV-AI-002 — the general lesson (a tool-routing comment describing "what's possible today" goes stale exactly like a stale invariant, and needs the same suspicion) is the more durable takeaway here.

Previous entry follows.

Last updated: 2026-09-18 (SHIPPED: the AI assistant can now answer general questions unrelated to Tassure's own data by searching the web — Anthropic's own hosted `web_search` server tool, added to the existing Claude tool-calling loop with no new vendor/API key needed).

Vincent: chatbot 还是很简陋，问不相关的问题应该能去网上找答案 (the chatbot is too limited — an unrelated question should be answerable by searching the web). Since this assistant (`app/api/assistant/route.ts`) already calls the Anthropic Messages API directly, this needed no new integration — just adding Anthropic's own hosted `web_search_20250305` tool to the existing `CLAUDE_TOOLS` array (`max_uses: 3` per turn, a deliberate cost cap, not unbounded) and routing guidance in the system prompt: prefer this system's own real tools for anything about Tassure's own data, web search only as a last resort for genuinely unrelated questions, and always say plainly when an answer came from the web rather than this system's own records.

The one real architectural subtlety, worth documenting since it's non-obvious: unlike every other tool in `CLAUDE_TOOLS`, a server tool like this one needs ZERO dispatch code in `runTool()` — Anthropic executes it server-side and the result comes back already resolved, never matching the `tool_use`-filtered loop this route uses to decide what to run itself. Traced the existing loop code carefully to confirm this (no code change needed there) rather than assuming — see `docs/INVARIANTS.md` INV-AI-001 (new section) for the full reasoning, so a future server tool (code execution, etc.) doesn't get dispatch logic it doesn't need.

Also added one line to the assistant's own "what can you do" help text mentioning this. `npx tsc --noEmit` / `npx eslint` / `npm run build` all clean — but this genuinely could NOT be end-to-end tested against the real Anthropic API before shipping: no `ANTHROPIC_API_KEY` is available locally (same limitation as not being able to log into the live app), so this is verified as correct against Anthropic's own documented tool schema and by tracing the exact response-handling code path, not by a real live call. First real test is Vincent trying it in the app. If it errors, the most likely cause is web search not being enabled for this Anthropic account/org in the Anthropic Console (an account-level setting outside this repo).

Previous entry follows.

Last updated: 2026-09-18 (FIXED: Statement PDF's DESCRIPTION column no longer prints Vincent's internal 2-letter staff shorthand — "(JE)", "(PM)" — to real clients; added a separate client-facing full-word label map used only by the PDF, leaving the on-screen staff UI's existing shorthand untouched).

Vincent, looking at a real generated Statement: "这个SOA PDF内的 DESCRIPTION 的简写是要显示完整的，不然客户也不知道是什么" (the abbreviations in the PDF's DESCRIPTION need to be spelled out in full — otherwise the client has no idea what they mean), specifically about non-Invoice lines showing "(JE)"/"(PM)". Root cause: the PDF's fallback description format reused `TXN_TYPE_TAGS` (`lib/soa.ts`) — Vincent's own explicit 2026-09-15 spec for the on-screen SOA list/Company 360 ("kept short since these sit inline next to a number"), never meant for a document going out to a real client. Fixed by adding a new export, `TXN_TYPE_LABELS`, used only by `lib/statement-pdf.ts` — full English words (Payment, Journal Entry, Deposit, Credit Note), only remapping QuickBooks' own "Credit Memo" type name to this business's client-facing "Credit Note" term. `TXN_TYPE_TAGS` itself is untouched, so the internal staff UI keeps its existing compact codes exactly as Vincent asked for there — this was additive, not a narrowing of existing behavior anywhere.

Verified against real data: re-rendered the combined-books Statement for "1V Capital Pte. Ltd." (the same real example with a Journal Entry and a Payment line) and extracted the actual PDF text directly (content-stream read, not a screenshot) — confirmed "Trial Balance -Dec'23 (Journal Entry)" and "3343 (Payment)" now print in full. `npx tsc --noEmit` / `npx eslint` / `npm run build` all clean. `docs/INVARIANTS.md` INV-DOC-019.

Previous entry follows.

Last updated: 2026-09-17 (FIXED, verified against the ORIGINAL reference PDF FILE, not a screenshot: the Statement's TO block now prints the customer's real QuickBooks CompanyName as a second line — explaining the "name appears twice" the reference has always shown — and the itemized DATE column now shows the real transaction date instead of the due date).

Vincent asked point-blank, a fifth round on this same page: "SOA PDF格式生成已经做到和QB一模一样了吗？" (has the Statement PDF format generation now been made to match QuickBooks exactly?). Rather than answer from screenshots again, found the ORIGINAL real reference PDF file still on disk from earlier in this investigation (`TAB-1V Capital Pte. Ltd.-s$1000.pdf`) and did a genuine side-by-side render-vs-reference comparison, both opened in the browser at identical zoom. Two real, previously-invisible gaps surfaced: (1) "1V Capital Pte. Ltd." visually appears TWICE in the reference's TO block — confirmed via a direct QuickBooks Customer query this is real: `CompanyName` is a distinct field from `DisplayName` (for this customer, coincidentally identical), and QuickBooks' own template always prints both stacked; (2) the reference's DATE column reads "24/07/2026" (the invoice's real transaction date) while this app's own render showed "31/07/2026" (the due date) — the real txn_date was already sitting in `quickbooks_ar_aging_detail` (found days earlier building INV-QB-022) but was never carried through into `SoaCompanyRow.lineItems`, which only ever had `dueDate`.

Fixed both at their real source: `findCustomer()` (`lib/qb-invoice-conventions.ts`) now also returns `companyName`, printed unconditionally as a second TO-block line whenever QuickBooks has a real value (never deduplicated against DisplayName — the reference doesn't deduplicate either, so hiding a genuine repeat would itself be wrong). `SoaCompanyRow.lineItems` gained a real `txnDate` field end to end (both the fresh-snapshot and legacy paths), used for the DATE column, while the DESCRIPTION text's own "Due DD/MM/YYYY" correctly keeps using `dueDate` — two genuinely different real dates, both now shown where the reference shows them.

Known, deliberate remaining difference — the ONLY one left, and Vincent's own explicit decision (confirmed earlier today): "STATEMENT NO." stays omitted since there is no real value for it (QuickBooks' own internal web-UI-only numbering sequence, no API). Every other visible field on the reference — letterhead, logo, colors, "Statement" heading, TO block (name ×2 + real address), DATE/TOTAL DUE/ENCLOSED, itemized table (real DATE/rich DESCRIPTION/AMOUNT/OPEN AMOUNT, full-width blue header), and the bottom-anchored aging summary — is now confirmed matching against the actual reference FILE, not an approximation. `npx tsc --noEmit` / `npx eslint` / `npm run build` all clean. `docs/INVARIANTS.md` INV-DOC-018.

Previous entry follows.

Last updated: 2026-09-17 (FIXED — broader than the PDF: `loadArAgingSnapshot()`'s own Invoice `doc_number` now self-corrects against `quickbooks_invoices.invoice_no`, fixing a real, systemic leading-zero-stripping bug confirmed on 433 of 541 real unpaid invoices across all 3 books — this was already silently showing the wrong invoice number on Company 360's Outstanding section and in the Excel export, not just this PDF).

Vincent, after the DESCRIPTION-column fix (previous entry below), pushed back on treating it as done: "那么我需要你给我一个肯定的答案，而不是单单只是因为完成了其中一次就告诉我完成了，你不要忘记看远一点" (I need a definitive answer, not "done" just because one instance worked — don't forget to look further). Went back and checked the ACTUAL scope of the leading-zero bug the DESCRIPTION fix had worked around only for the PDF (INV-DOC-017): queried every real unpaid invoice across TAB/TAC/TAO against its own authoritative `quickbooks_invoices.invoice_no` — 433 of 541 (72-92% per book) had a stripped leading zero in `quickbooks_ar_aging_detail.doc_number` (QuickBooks' own AgedReceivableDetail REPORT API reformats a purely-numeric DocNumber; the Invoice ENTITY API, synced separately, does not). That table is `computeSoaRows()`'s fresh-path source for EVERY consumer of `lineItems[].docNumber` — not just the PDF, but Company 360's Outstanding section (renders it directly to staff) and the Excel export too. Fixed at the one shared chokepoint, `loadArAgingSnapshot()` (`lib/soa-data.ts`): Invoice-type rows now cross-reference `quickbooks_invoices` by `qb_invoice_id` and use its real `invoice_no` whenever the report's own value differs. Credit Note doc numbers (e.g. "CN240023") are untouched — never purely numeric, never reformatted by the report.

Verified two ways, not just re-checking the one known example: (1) re-ran the exact mismatch check against the FULL real dataset post-fix — 0 remaining mismatches across all 541 invoices, all 3 books; (2) rendered 5 more real, deliberately varied Statements end-to-end (not just 1V Capital) to stress-test the whole pipeline broadly: Orbitez Pte Ltd (12 line items, single book — confirms pagination/footer-pinning still works on a long list, and confirms the leading-zero fix is genuinely per-invoice-accurate, not a blanket normalization — Orbitez's own real invoice numbers only started using a leading zero from Nov 2025 onward, and the render correctly reproduces BOTH eras faithfully); Han Kun LLP (combine-ALL-3-books mode); Iuiga Retail Plus (a real unapplied Credit Note mixed into the itemized list, correctly falls back to the plain format since Credit Notes were never broken); World Precision Machinery (no BillAddr on file — confirmed the address block just cleanly disappears, no layout break); FAITH CAPITAL GLOBAL FUND VCC (long name, multi-line address). No crashes, no overlapping content, no layout breaks in any of the 5. One unrelated, pre-existing data-quality issue spotted in passing (not caused by or related to this fix, not touched): "FAITH CAPITAL GLOBAL FUND VCC" exists as two separate real QuickBooks customer records with a spelling typo ("Global" vs "Glocal") — worth a future cleanup, flagged to Vincent, out of scope here.

`npx tsc --noEmit` / `npx eslint` / `npm run build` all clean. `docs/INVARIANTS.md` INV-QB-022 (the systemic root-cause fix) cross-referenced from INV-DOC-017 (the original narrower PDF-only finding).

Previous entry follows.

Last updated: 2026-09-17 (FIXED: Statement cover page's DESCRIPTION column now shows the real invoice number, correctly padded, plus the real invoice line description — e.g. "Invoice No.02610894: Due 31/07/2026. XBRL for the year (FYE 31.12.2025)" — instead of the bare "2610894 (Invoice)" it printed before).

Vincent, on the DESCRIPTION column specifically: "这部分为什么生成出来的没有像这个那么完整" (why isn't the generated version as complete as the reference's). Two real gaps, both traced to `SoaCompanyRow.lineItems` (`computeSoaRows()`'s own shape) not being a reliable source for a document's exact printed form: (1) the real invoice number for this exact example is "02610894" in both `quickbooks_invoices` and `quickbooks_invoice_items` — but `lineItems` carried it as "2610894", missing its leading zero somewhere in the fresh-snapshot path; (2) `lineItems` never carried the invoice's real line `Description` at all (a real, already-synced field — `quickbooks_invoice_items.description` has the actual "XBRL for the year (FYE 31.12.2025)..." text). Fixed by adding `resolveInvoiceDetails()` to the route: instead of trusting `docNumber`, it re-derives both the correct invoice number and description from `matched` — the same real `quickbooks_invoices` rows (with real `qb_invoice_id`) already fetched earlier in the route to merge the actual invoice PDFs — joined to `quickbooks_invoice_items` by exact `qb_invoice_id`, keyed for lookup by numeric value so the possibly-stripped `docNumber` still resolves to the right row. Best-effort, never blocks the Statement on failure. Only the description's first line is shown, matching the reference's own style (the real field has 3 more lines of boilerplate after a blank line).

Verified against real data: re-rendered the exact same TAB-only "1V Capital Pte. Ltd." Statement and confirmed the DESCRIPTION column now reads "Invoice No.02610894: Due 31/07/2026. XBRL for the year (FYE 31.12.2025)", wrapping to 2 lines exactly like the reference, with the itemized table's row height and everything below it still correctly positioned (manually wrapped, not pdf-lib's own auto-wrap — see the two prior INV-DOC-015/017 entries on why that matters on this page). `npx tsc --noEmit` / `npx eslint` / `npm run build` all clean. `docs/INVARIANTS.md` INV-DOC-017.

Previous entry follows.

Last updated: 2026-09-17 (FIXED: Statement cover page now reproduces "ENCLOSED" — a fixed boilerplate caption, safe to copy exactly, unlike "STATEMENT NO." which stays omitted since it's a real per-Statement QuickBooks-only sequence number this app has no source for).

Vincent, after the position/width fixes, pointing at the whole STATEMENT NO./DATE/TOTAL DUE/ENCLOSED block: "这个你没有部分重现吗？" (didn't you reproduce any of this?). On review, the earlier round's decision to omit ENCLOSED was simply wrong reasoning — it's fixed text on every real QuickBooks-printed Statement regardless of content, never a per-Statement data value, so there was nothing to fabricate by printing it (the reference itself shows the label with nothing beside it — that's the correct look, not a placeholder waiting on a value). Now reproduced. STATEMENT NO. is the genuinely different case and stays correctly omitted: it's QuickBooks' own internal per-Statement numbering sequence (the reference shows 10498) with no API equivalent — this app has no real value for it, and inventing one would be a new business record Vincent hasn't authorized. Documented the general distinction (constant caption vs. real per-document data) as `docs/INVARIANTS.md` INV-DOC-016, since the same question will come up again on any future reference-matching work.

`npx tsc --noEmit` / `npx eslint` / `npm run build` all clean. Re-rendered the real TAB-only "1V Capital Pte. Ltd." Statement again and visually confirmed ENCLOSED now prints right-aligned under TOTAL DUE, matching the reference's column.

Previous entry follows.

Last updated: 2026-09-17 (FIXED: Statement cover page's aging-summary footer now anchors near the page bottom instead of crowding right under a short item list, the itemized table's blue header now spans the true right margin matching the aging table below it, and BillAddr wrapping no longer silently miscounts vertical space).

Vincent, a third round comparing his reference PDF pixel-by-pixel: "位置很重要，宽度也很重要，上下的位置，蓝色的宽度，是否有对齐" (position matters, width matters too — top-to-bottom position, the blue's width, whether things line up). Two real, visible bugs plus one latent one found in the same pass: (1) the aging table was drawn immediately after the last itemized row instead of anchored a fixed distance from the page bottom — the reference's own 1-invoice example leaves most of the page blank with the footer pinned low, mine crowded it right under the single row; fixed with a fixed `AGING_TABLE_Y = 140` that only pulls the footer DOWN into empty space, never up over real content. (2) the itemized table's `OPEN AMOUNT` column had a fixed width, so its blue header band summed to ~480pt against a ~512pt content area — stopped short of the right margin the aging table's own header reaches, so the two tables' right edges didn't line up; fixed by letting that last column stretch to the true margin. (3) `billAddrLines` — a real BillAddr can be one long unsplit line (see yesterday's INV-DOC-014 finding) — was drawn via pdf-lib's own `maxWidth` auto-wrap, which silently drew 2+ visual lines this function's own `y` tracker never accounted for, undercounting vertical space; fixed with manual greedy word-wrapping so every drawn line is one this function explicitly advances `y` for. Also reformatted the itemized table's DATE column from raw ISO to DD/MM/YYYY, matching the rest of the page (was printing "2026-07-31" next to a "TOTAL DUE" block already showing "17/09/2026").

Verified against real data again: re-rendered the exact TAB-only single-invoice Statement for "1V Capital Pte. Ltd." (SGD 1,000.00, invoice 2610894) via the same standalone `tsx` script and visually confirmed the layout now matches the reference's own proportions — footer pinned ~82% down the page, item table's blue bar reaching the full margin. `npx tsc --noEmit` / `npx eslint` / `npm run build` all clean. See `docs/INVARIANTS.md` INV-DOC-015.

Previous entry follows.

Last updated: 2026-09-17 (FIXED: Statement cover page's TO block — real client mailing address now prints (was missing entirely), and the printed name switched from the `companies` table's ALL-CAPS convention to QuickBooks' own real mixed-case DisplayName).

Vincent, comparing his real reference PDF against this route's output a third time today: "地址都没有看到" (the client's address wasn't shown at all), then "你最新版的位置不对" (the block's position looked wrong) — the second complaint was a direct symptom of the first: with no address lines, the TO block just left a wrong-looking gap before the itemized table. Root cause was two real data-source mistakes in `drawStatementCoverPage()`: it printed `row.companyName` (which for a `computeSoaRows()`-sourced row can be the `companies` table's own ALL-CAPS convention, e.g. "1V CAPITAL PTE. LTD.") instead of QuickBooks' real DisplayName, and it never fetched or printed the client's registered `BillAddr` at all — that data isn't in `SoaCompanyRow`.

Fixed by live-fetching that exact QuickBooks Customer's `BillAddr` at generation time (`findCustomer()` + newly-exported `addrToLines()` in `lib/qb-invoice-conventions.ts`) and printing the already-resolved raw QB customer name instead of the company row's display name — verified against real data via a standalone `tsx` script (outside the proxy.ts auth wall, same reasoning as always): TAB's own BillAddr for "1V Capital Pte. Ltd." turned out to have its whole address jammed into one `Line1` string, while TAO's record for the same real company has it properly split across fields AND is a genuinely different physical address (60 Paya Lebar Road vs 100 Lorong 23 Geylang) — prints whatever that book's own real field contains rather than guessing/re-splitting. The cover-page drawing code (`drawStatementCoverPage`, `combineStatementRows`, `safeText`, `StatementRow`) moved out of the route into `lib/statement-pdf.ts` specifically so it's independently testable against real data. `STATEMENT NO.` and `ENCLOSED` (both in the reference) stay deliberately omitted — no real data source for either, see `docs/INVARIANTS.md` INV-DOC-014.

`npx tsc --noEmit` / `npx eslint` / `npm run build` all clean. Actual rendered PDF inspected via the browser pane against real 1V Capital data (TAB+TAO combined, SGD 3,650.00 total, matching Vincent's own screenshot exactly) before shipping.

Previous entry follows.

Last updated: 2026-09-17 (SHIPPED: a daily automated audit catches a confirmed Main PIC going stale relative to real invoice data — plus a real, significant pre-existing bug found and fixed along the way: the Automation Health dashboard's exception tracking had silently never worked for 6 of 7 real exception types.

Vincent, after this session's second real "stale Tey Shemin PIC" finding: "要严格按照QB的更新，要准确，这个Outstanding内的公司和PIC和欠款 都是要很准确和实时的，这些都是要最新的不能延迟" — confirmed the amounts/aging and auto-suggested PIC already refresh near-real-time off the QuickBooks webhook (INV-QB-021), but a human-CONFIRMED Main PIC (`soa_owners.soa_pic`) never re-validates itself once set, which is exactly how 19 real companies stayed silently wrong for weeks. He confirmed building a daily automated self-check (flag for human review, not auto-correct — a manual override can legitimately be ahead of the invoice data, e.g. right after a real handoff with no new invoice keyed yet).

New `/api/soa-owners/audit` (cron, 22:00 UTC daily, after the QuickBooks sync and AR workflow crons finish): for every `soa_owners` row in TAB/TAO (TAC skipped — its PIC is the current Nominee Director, not Class-derived, per INV-QB-008), compares the confirmed `soa_pic` against `computeSoaRows()`'s own `suggestedOwner` for that customer — the exact same shared computation the on-screen list/PDF/Excel export already use, never a second signal. A mismatch gets recorded via `replaceAutomationExceptions()` and shows up on the existing Automation Health dashboard, same as every other sync source.

**Building and testing this against real data surfaced a much bigger, pre-existing bug**: `replaceAutomationExceptions()`'s resolve-cutoff was computed from a fresh `Date.now()` taken AFTER its own upsert completed — always a few ms later than the `observedAt` just written to the upserted rows — so with the default `graceMs=0`, every exception was marked `'resolved'` in the SAME call that created it, never actually visible as open. Confirmed against real production history: `quickbooks/duplicate_doc_number_*` (8 real rows), `teamwork_companies/unknown_pic_id` (55 real rows!), `teamwork_companies/missing_from_teamwork` (1), and `quickbooks/oauth_refresh_*` (3) had ALL been silently auto-resolving within under 2 seconds of creation for as long as this function has existed — real operational issues that have never actually been visible on the dashboard. The one caller that happened to survive, `teamwork_nd/missing_nominee_subrole`, only did so by accident (it passes a real non-zero `graceMs` for an unrelated reason). Fixed by anchoring the cutoff to `observedAt` itself instead of a fresh timestamp — verified by re-running the new audit against real data and confirming its own 20 real findings now correctly stay `'open'` (they did not before the fix).

Full `npx tsc --noEmit` / `npm run build` clean; also manually ran the new route against real production data via a local dev server + the real `CRON_SECRET` bearer (not just unit logic) and inspected the actual persisted `automation_exceptions` rows before and after the fix. New `docs/INVARIANTS.md` INV-CRON-015 documents the timestamp bug (a durable lesson: even code that's been in production a while deserves a real read-back test, not just a read-through). Every OTHER affected source's historical hidden exceptions will only re-surface once their own next scheduled cron run re-observes them — not manually re-triggered today, since those are heavier syncs with real external API calls.
...
**1. Pixel-faithful Statement template.** Rather than eyeballing the reference PDF's screenshot, extracted its ACTUAL design straight from the file's own content stream (inflated its Flate-compressed page stream, read the literal `rg` fill-color operators and `/BaseFont` declarations): header/heading color `#4F90BB`, table-header fill `#DCE9F1`, fonts are plain `Helvetica`/`Helvetica-Bold` (the exact same StandardFonts this route already embeds — no new font asset needed). Rewrote `drawStatementCoverPage()` to match: the aging-summary table now appears at BOTH top and bottom (matching the reference exactly, replacing the old plain "Total Outstanding" footer line), the "Statement" heading is 20pt regular weight (confirmed NOT bold, from the reference's own `Tf` operator) in that exact blue, table headers use the light-blue fill with blue regular-weight text, and the TO/DATE/TOTAL DUE block matches its layout. Date format changed to match the reference's own "DD/MM/YYYY" rather than this app's usual ISO convention (this page only). Two honest, disclosed gaps: no "STATEMENT NO." (that's QuickBooks' own internal numbering — inventing one would be a fabricated business record) and no logo image (this app's own `public/logo.png` is an unrelated generic icon, not the real "T Assure" wordmark — using the wrong logo would be worse than none). Verified by actually rendering test PDFs (not just reading the code) and visually inspecting them via the Read tool, including a 25-line-item pagination test.

**2. "All" page combine mode.** `app/api/billing/soa/pdf/route.ts` now accepts `company=ALL` — queries invoices/credit-memos with `.in('qb_company', [...])` instead of `.eq()`, and the cover page is built via new `combineStatementRows()`, which sums each of TAB/TAC/TAO's own `computeSoaRows()` result (aging buckets, total, line items) into one synthetic row — never a second, independently-computed total, so a combined Statement can never disagree with the 3 single-book pages added up by hand. Verified against the exact real numbers from this feature's own original bug report: TAB $1,000 + TAO $2,150 + $500 = combined $3,650.00, matching the S$3,650.00 Vincent's real client email actually stated. Letterhead uses "Tassure Group" (not any one entity's legal name — TAB/TAC/TAO are 3 separate legal entities, and this is literally the umbrella term Vincent's own real reminder emails already use). `app/api/billing/soa/detail/route.ts` (the invoice-level list in the detail modal) got the same treatment, refactored into a per-book `resolveOneBookDetail()` helper called once per book and concatenated. `lib/soa-actions-client.ts`'s `buildSoaDraft()`/`downloadSoaPdf()` widened to a new `SoaCompanySelector` type (`QbCompany | 'ALL'`); when combining, passes `qbCompany: undefined` to `buildCampaignDraft()` — `buildRow()`'s own default (no filter), not a 4th fake `QbCompany` value threaded through `lib/client-comms-resolve.ts`. `app/billing/soa/_components.tsx`: `SoaDraftPopover`/`SoaDetail` now accept `'ALL'`, and the All page's own row rendering passes it through (reversing a PRIOR explicit design comment that said the opposite — see `docs/INVARIANTS.md` INV-DOC-012 for why that was right for its own time but is now superseded). The on-screen All LIST itself is unaffected — still 2+ separate un-deduplicated rows per company, one per book — only the Draft/PDF ACTION combines.

Also fixed a genuine ID collision while documenting this: my earlier INV-DOC-007 (the pdf-lib CJK crash-safety entry) collided with an already-existing, unrelated INV-DOC-007 much later in the file — renumbered to INV-DOC-011.

`npx tsc --noEmit` / `npm run build` both clean throughout. New `docs/INVARIANTS.md` INV-DOC-012, `docs/CURRENT_STATE.md` updated.
...
Scanned the whole `soa_owners` table (383 rows): 119 (31%) recorded a staff member from the wrong team for their `qb_company`, ALL written by a one-time 2026-09-07 backfill script (`updated_by_email='backfill@internal'`) that ran under the pre-split "same owner regardless of book" theory — a stale leftover from before the per-book PIC rule existed, never touched by a real human since. Of those, 63 (52 TAB + 11 TAO) were all the same person — Tey Shemin, Corporate Secretarial (Malaysia) — which turned out to be a genuinely correct signal my earlier restriction rule was simply too narrow for: confirmed with Vincent that Malaysia-side staff aren't split into separate Secretarial/Accounts/Tax teams the way Singapore staff are, so the same person legitimately handles both. Widened `lib/soa-owner.ts`'s `PIC_TEAMS_BY_COMPANY` to include `'Corporate Secretarial (Malaysia)'` on both TAB and TAO, then deleted the remaining 56 genuinely wrong-team rows via new `scripts/backfill-clean-soa-owner-mismatches.js` (idempotent, scoped strictly to `backfill@internal` rows still mismatched under the corrected rule — never touches a row a real human has confirmed/edited). Deleting (not nulling) lets `effectiveOwner()` fall through to the real, now-correctly-computed `suggestedOwner` instead of showing a stale wrong value.

`npx tsc --noEmit` / `npm run build` both clean. `docs/INVARIANTS.md` INV-PIC-007 updated in place with the widening and the "check the human-override table too, not just the computed signal" lesson.

Previous entry follows.

Last updated: 2026-09-17 (SHIPPED: the merged SOA PDF now opens with a real "Statement of Account" cover page, and the SOA draft email's body no longer leaks another QB book's invoices into a single-company draft.

Vincent, via WhatsApp screenshots of a real client email for "1V Capital Pte Ltd" (owing on both TAB and TAO): "tao tab 有欠款为什么只attached tab 而已，而且不是soa 是inv" — the attached PDF only covered TAB and looked like a bare invoice, not a statement. Investigated with a background Workflow (3 parallel Explore agents) before touching anything: confirmed `/api/billing/soa/pdf` (this repo's own merge route) has never drawn any cover/summary page — it only concatenates raw QuickBooks invoice/credit-memo PDF pages — and separately confirmed QuickBooks Online's public Accounting API has no "Statement" entity/endpoint at all (Statements are QBO web-UI-only; independently verified against Intuit's own docs and third-party developer sources, not just this repo's code). The PDF Vincent showed as a reference was one he'd downloaded directly from the QuickBooks web UI himself, for format reference only. Confirmed with him (AskUserQuestion) before implementing: build our own cover page using QuickBooks' own already-synced AgedReceivableDetail data (same computation `computeSoaRows()` the on-screen list/Excel export already use) rather than trying to automate/scrape QBO's native Statement feature — deliberately still scoped to one QB company per PDF (TAB/TAC/TAO), matching the 2026-09-07 SOA split; combining multiple books into one statement stays a separate, larger follow-up.

**Two real, independent bugs fixed together** (confirmed unrelated to each other, bundled per Vincent's own "等 Statement 封面页方案定了一起改"):
1. **New Statement cover page** (`app/api/billing/soa/pdf/route.ts`'s `drawStatementCoverPage()`) — Tassure letterhead, aging-bucket summary table, itemized outstanding list, total — drawn as page 1 before the real invoice/credit-memo pages get merged in below (matches Vincent's own framing: "inv 我们会放在soa 下面，在一个pdf 里面").
2. **Cross-book email-body leak** — `lib/client-comms-resolve.ts`'s `loadInvoicesByCompany()` (SOA branch) pools TAB+TAC+TAO invoice data into ONE array per customer name (by design, for Campaign Centre's own bulk cross-book SOA candidate flow — a different, intentionally-unscoped caller) — but the ad-hoc single-company "Draft Email" button (`buildSoaDraft`) was never filtering that pooled array down to its own one book, so a TAB-titled draft's body listed TAO invoices and a combined total too, even though the PDF attachment was correctly TAB-only. Fixed by threading an optional `qbCompany` filter through `buildRow()` → the campaigns-preview GET route → `resolveCampaignRow()`/`buildCampaignDraft()` → `buildSoaDraft()` (which already received `qbCompany`, just wasn't forwarding it). Campaign Centre's bulk POST flow and the AI chat assistant's own `previewEmailDraft()` were deliberately left untouched — both are genuinely meant to stay cross-book.

**Adversarially reviewed before shipping** (background Workflow, 3 review dimensions in parallel → each finding independently re-verified against the live file, not trusted from the first pass) — all 4 raised findings confirmed real, all fixed:
- **Critical**: `page.drawText()` throws SYNCHRONOUSLY for any character `pdf-lib`'s StandardFonts (WinAnsi) can't encode — confirmed live against real data: `'吉木锌国际贸易（上海）有限公司'` (one of the very customers the mojibake fix above touches) throws `WinAnsi cannot encode "吉"`. The cover page's own `try/catch` swallowed such a throw WITHOUT removing the already-`addPage()`'d page, so a Chinese-registered client's Statement would have silently shipped with a half-drawn page 1 (letterhead + a bare "To:" label, no name/totals/items) and zero error surfaced anywhere. Fixed with two layers: a new `safeText()` filter (drops unencodable characters, falls back to a visible "(name unavailable)" placeholder rather than blank) applied to every dynamic string drawn, PLUS the outer `try/catch` now tracks the page count before drawing and `removePage()`s anything added if it throws anyway — belt-and-suspenders, since `safeText()` can't be proven to cover every future draw call. New `docs/INVARIANTS.md` INV-DOC-011 (originally mislabeled INV-DOC-007 — collided with an existing entry of that number later in the file; renumbered same day). Full CJK glyph rendering (an embedded Unicode font via `fontkit`) deliberately out of scope — this is a crash-safety net, not a rendering feature.
- **Medium**: the "all invoice PDFs failed to fetch" 502 check discarded an already-successfully-drawn cover page (Supabase-only data, independent of the live QuickBooks PDF-fetch loop) whenever every individual `fetchInvoicePdf`/`fetchCreditMemoPdf` call failed (e.g. a transient QB outage) — now only hard-fails when the cover page ALSO didn't make it; otherwise returns the real (if invoice-pages-incomplete) Statement rather than nothing.
- **Medium**: the cover page resolved its customer independently (via `computeSoaRows()`'s own fuzzy match against the free-text `companyName` query param) from the invoice-merge match a few lines above it — a theoretical two-similarly-named-customers collision could have shown one company's aging summary above a DIFFERENT company's actual invoice pages, silently. Fixed by resolving the cover page against the EXACT raw `customer_name` the invoice-merge match already settled on, not re-running an independent fuzzy search.
- **Low**: the cover page used "SGD " where every other money display in this app (on-screen list, Excel export, chat cards) uses "S$" — made consistent.

`npx tsc --noEmit` / `npm run build` both clean throughout (initial implementation and every review-driven fix). Also manually verified the core crash-fix mechanism against real `pdf-lib` (a throwaway script confirming the raw crash reproduces, and that `safeText()` prevents it for mixed/all-CJK/pure-ASCII inputs alike, deleted after use).
...
**UI**: `app/billing/soa/_components.tsx` — new shared `SoaDraftPopover` component reused from BOTH the List row's own inline Mail icon (`variant="icon"`, new trailing 36px grid column, previously nothing was there) and the detail modal's own button (`variant="button"`, replacing its old one-click-send teal button) — one implementation, not two independently-styled ones. Fetches `email_senders` + `email_templates?type=soa` (new `useSoaDraftPickers()` hook, shared by both usages), matching Billing Drafts' exact sender/template `<select>` pattern (`app/billing/page.tsx`'s own popover). Sending itself unchanged — `buildSoaDraft()` (`lib/soa-actions-client.ts`) still merges the PDF/creates the campaign; the parent still owns showing `OutlookStyleSendModal` once (same reasoning Billing Drafts already uses for keeping one instance, not one per row).

**New auto-computed merge fields** (Vincent: "新增自动计算的合并字段", after being asked how the docs' `<Date>`/`<Day>` placeholders should be handled): `{{daysOverdue}}` (days since the oldest unpaid invoice's real due date) and `{{lastReminderDate}}` (when a prior 'soa' reminder was last actually SENT to this exact company). Both computed server-side in `app/api/client-communications/campaigns/route.ts` (creation) and `.../drafts/refresh-amounts/route.ts` (recomputed fresh on refresh, not carried forward — daysOverdue is inherently live, and a refresh naturally excludes the still-pending draft itself from "last sent"). New `lib/client-comms-resolve.ts` function `loadLastReminderSentAt()` (same shape as the existing `loadAlreadySent()`); `InvoiceRef` (`lib/email-merge.ts`) gained an optional `dueDate` field, populated only from the AgedReceivableDetail snapshot path (`loadArAgingSnapshot()`) — the legacy `quickbooks_invoices` fallback table has no `due_date` column at all, a real, confirmed gap, so a company scoped to that fallback simply gets an empty `{{daysOverdue}}` rather than a guessed one.

**Payment/bank image — no new work needed.** Asked Vincent how the docx templates' embedded "PAYMENT METHOD" bank/QR image should be handled; he said it's the same one Billing Drafts' own templates already use. Checked the real `email_templates` rows: none of them embed an image at all — `components/client-communications/OutlookStyleSendModal.tsx` already auto-attaches `/assets/payment_options.png` (+ a standing "Bank Details 2026 - Tassure Group.pdf") whenever a draft's body contains the literal text "PAYMENT METHOD" (`showsPaymentImage = editedBody.toUpperCase().includes(PAYMENT_MARKER)`) — confirmed the existing default 'ar' template ends in that exact marker with no image markup of its own. The 3 new templates just needed to keep that same literal marker (which the source docs already had verbatim) — zero image-handling code needed.

**Templates seeded via `scripts/seed-soa-reminder-templates.js`** (new, safe to re-run) rather than asking Vincent to hand-retype long bilingual text through the Templates admin UI — connects directly via the admin Supabase client (same pattern as existing `scripts/backfill-*.js`), inserts 3 new `email_templates` rows (type='soa', additive — the 2 existing placeholder SOA1/SOA2 templates are untouched, still selectable), makes "1st Reminder" the new default. Verified the exact stored body text round-trips byte-correct (no template-literal escaping artifacts, `{{totalAmount}}`/`{{daysOverdue}}`/`{{lastReminderDate}}`/`PAYMENT METHOD` all present verbatim) via a direct DB read-back.

`npx tsc --noEmit` / `npm run build` clean throughout. `docs/FEATURE_MAP.md` updated (Client Communications core-features row + a new `lib/email-merge.ts` high-risk-shared-logic row, since its 2 real call sites — campaign creation and refresh-amounts — must stay logically consistent, not byte-identical, on how they compute the same 2 new fields).

Previous entry follows.

Last updated: 2026-09-16 (CORRECTION, no code change: "Rainbow" — the internal file share the NAS document search feature below targets — is NOT a NAS appliance. Vincent: "我要先验证，避免后面白做工" ("verify first, to avoid wasted work"), so before writing the NAS-side indexing script, tested real connectivity directly from this machine (which already has the share mapped as `Q:` under Vincent's own account) rather than continuing to plan blind.

Real findings: `Q:` maps to `\\Rainbow\RainbowData`, hostname `Rainbow.sg.tassure.com` (AD-style internal DNS). `net view \\Rainbow` shows it also hosts **NETLOGON** and **SYSVOL** shares ("Logon server share") — these only exist on a Windows **Active Directory domain controller**. Port scan: 445/3389 open, 5000/5001 (Synology DSM) and 8080/8443 (QNAP etc.) all closed — not a NAS-brand device's fingerprint. So "Rainbow" is a Windows Server that is also the company's domain controller; the file shares (RainbowData/AuditHR/cctvfootage/etc.) are incidental, not its primary role. Also confirmed content extraction is technically feasible without Word installed — unzipped a real `.docx` (`Forms And Learning Hub\...\Training attendance sheet.docx`) and read `word/document.xml` directly, extracted 712 real characters of text via plain regex tag-stripping. Folder structure confirmed too: "All Clients Profile" → A-Z letter folders → `"<code>_<COMPANY NAME>"` folders (e.g. "CA029_ACG INTERIOR AND EXHIBITION PTE. LTD") — the company-name part matches `lib/company-name.ts`'s `normalize()` fine, but the NAS-side script will need to strip the `CA029_`-style numeric prefix first, a real detail the original plan hadn't accounted for.

Asked Vincent directly (`AskUserQuestion`) whether to accept running the indexing script directly on Rainbow (simplest, but installing extra scheduled tasks/scripts on a domain controller is against normal security practice — it's the whole company's login/auth core) versus finding a separate always-on machine that reaches `\\Rainbow\RainbowData` over the network instead (same access pattern just verified, never touching the DC itself). **He chose the separate-machine option.** Still open: which specific always-on machine — needs Vincent/IT to designate one. Plan file (`C:\Users\vincent\.claude\plans\atomic-wandering-locket.md`) updated with this correction; the cloud-side code shipped earlier today (see entry directly below) is unaffected — it only cares about receiving batches at `/api/nas-index/ingest`, not about which machine sends them.

Previous entry follows.

Last updated: 2026-09-16 (CLOUD-SIDE SCAFFOLDING SHIPPED, feature not yet live end-to-end: internal-network NAS document search for the AI assistant.

Vincent: "假设我们有一个内网server 能不能设置系统链接内网资源方便我们后续chatbot 协助找出相关资料文件" — wants files on the office's internal NAS (`\\Rainbow`, SMB share with "All Clients Profile"/"Finance"/"Marketing"/etc top-level folders) searchable by the existing AI assistant, including content, not just filenames ("连内容都索引"). Worked through the architecture with him in conversation first: this app runs on Vercel and cannot reach into the office LAN — a NAS share is only reachable from inside the office network/VPN, the opposite direction of how QuickBooks/TeamWork sync today (this app pulls FROM the public internet; a NAS needs to push TO us). Confirmed with Vincent that privilege level and network reachability are two separate things — domain-admin credentials don't make an unreachable machine reachable. The cleanest "not dependent on any one employee's computer" answer is an indexing script running on the NAS device itself, since it must already be on 24/7 to serve files day-to-day — Vincent confirmed: "就先按照它是NAS为前提去计划方案".

Planned via Plan Mode (2 Explore agents grounding the design in this app's existing conventions) before writing any code — full plan at `C:\Users\vincent\.claude\plans\atomic-wandering-locket.md`. Shipped the cloud side of that plan:

- **`scripts/add-nas-document-index.sql`** (new, not yet run by Vincent) — `nas_documents`: upsert-by-`file_path` (NOT a replaced-snapshot table like `quickbooks_ar_aging_detail` — NAS files change far less often than a daily financial report), `content_tsv` a generated `tsvector` column with a GIN index for Postgres full-text search, `company_id` resolved at INDEX time (not query time) via the existing `lib/company-name.ts` `resolveCompany()`, left null + `raw_folder_name` kept when nothing resolves cleanly rather than guessing.
- **`app/api/nas-index/ingest/route.ts`** (new) — receives batches (max 20 files) from the NAS-side script. Authenticated like the QuickBooks webhook (`proxy.ts`'s `PUBLIC_PATHS`, HMAC-SHA256 of the raw body via `timingSafeEqual`, a new `NAS_INDEX_SECRET` env var — not yet set), not like the `CRON_PATHS` bearer bypass (that one is GET-only; this is an external POST-with-payload push). Wrapped in `withAutomationRun('nas_index', ...)` so it shows up on the same Automation Health dashboard as every other sync source (also added `'nas_index'` to `lib/automation-sync.ts`'s `AutomationSource` union and `app/api/automation/health/route.ts`'s separate `SOURCES` array — that array doesn't follow the union automatically, a known existing gap).
- **`lib/document-search-lookup.ts`** (new) — `searchDocuments()`, Postgres full-text search (`plainto_tsquery`) over `nas_documents`. Never returns a matched file's full `content_text` to the caller — only a short snippet around the match (own local extraction, not `ts_headline()`, to avoid needing a second migration just for an RPC function) — per INV-DATA-028's existing curation-at-the-boundary principle, so a whole sensitive document's contents never land in the assistant's context.
- **`app/api/assistant/route.ts`** — new `search_documents` tool, wired the same 3-piece way (`CLAUDE_TOOLS` schema entry + `runTool()` dispatch + `documentSearchTool()` wrapper) as every other tool in this file, plus a routing line in the system prompt distinguishing it from `company_deep_lookup` (files vs. company records).
- **`lib/deep-links.ts`** — new `companyDeepLink(companyId)`, the first deep link in this file keyed by `companies.id` directly rather than a fuzzy-matched name (every other one here is name-based, for pages this app itself serves; a NAS document's `company_id` is already resolved, and the document itself lives on the NAS, which the cloud can never serve — a search result links to that company's Company 360 page instead, where staff already know to go find the real file from a machine on the office network).

**Explicitly NOT live yet — three real pieces still missing**, all called out in `docs/CURRENT_STATE.md`'s new Active Issues entry: (1) Vincent hasn't run the migration yet, (2) `NAS_INDEX_SECRET` isn't set (the ingest route 503s until it is), (3) the NAS-side indexing script itself (walks the real folders, extracts text, POSTs batches) hasn't been written — deliberately deferred pending Vincent confirming the NAS device's actual model/OS (Synology/QNAP/other), per the plan's own scoping. Deliberately deferred out of scope for this round entirely (per the plan): semantic/vector search (Postgres full-text only for now), a dedicated result-card UI (plain markdown links), and opening/downloading the file itself from chat (links to Company 360 instead).

`npx tsc --noEmit` / `npm run build` both clean. `docs/FEATURE_MAP.md` (new Core Features row) and `docs/CURRENT_STATE.md` (new Active Issues entry) updated in this change.

Previous entry follows.

Last updated: 2026-09-16 (SHIPPED: SOA/Outstanding now refreshes near-real-time via the QuickBooks webhook, not just once a day.

Vincent reported Taiyau Trading Pte. Ltd. showing an old balance on the Outstanding List despite being fully paid across TAB/TAC/TAO that same day. Investigated with a background Workflow (2 parallel agents: one checking live QuickBooks directly, one checking our synced snapshot + sync timestamps) — confirmed QuickBooks itself was genuinely $0.00 in all three books (Payment #25314 $1,120 TAB, #4407 $4,000 TAC, #11214 $700 TAO, each exactly zeroing its invoice), while the synced `quickbooks_ar_aging_detail` snapshot still held the pre-payment amounts because the daily sync (19:30 UTC / 03:30 SGT) had already run hours before the payments were entered — a real timing gap, not a code bug. Manually triggered a fresh sync to fix Taiyau immediately (verified 0 rows for Taiyau in the snapshot afterward).

Vincent's direct follow-up: "这部分是有需求的所有需要实时的收到QB的更新，不能一天更新一次" — real-time updates are a real requirement, once-daily isn't enough. This app already had a full QuickBooks webhook pipeline (`app/api/quickbooks/webhook/route.ts` → `lib/quickbooks-webhook-queue.ts` → CDC-based invoice sync), but it hard-filtered to Invoice entities only — every Payment/CreditMemo/JournalEntry/Deposit webhook event (the exact 5 types INV-QB-017 confirmed actually affect this business's real AR) was silently discarded before even being queued, a gap INV-QB-015 itself already flagged as "deferred, not done."

Used Plan Mode with an independent Plan agent to design + adversarially review the fix (the agent verified 3 of my own claims against the real code before I trusted them: the QuickBooks API's real entity-name spelling — "CreditMemo", no space, confirmed against this app's own `SELECT * FROM CreditMemo` — differs from the AgedReceivableDetail report's display wording "Credit Note"; `AutomationRun`'s lock is keyed by source alone, not per-company, so no new concurrency guard was needed; `QB_WEBHOOK_VERIFIER_TOKEN` is genuinely absent from this sandbox's `.env.local`, ruling out a real signature-verified webhook simulation here).

**Not a return to per-entity-type sync** (the exact pattern INV-QB-017 rejected) — the webhook only decides WHEN to trigger a re-sync; the re-sync itself still pulls QuickBooks' own comprehensive AgedReceivableDetail report, so a future 6th AR-relevant entity type needs zero new sync logic. `syncAgedReceivableDetail()` moved from a private function in `app/api/quickbooks/sync/route.ts` to an exported one in `lib/quickbooks-ar-aging.ts` so both the daily cron and the webhook path share the exact same implementation. `app/api/quickbooks/webhook/route.ts`'s entity-type allowlist widened from Invoice-only to the 5 AR-relevant types. `lib/quickbooks-webhook-queue.ts` now also triggers `syncAgedReceivableDetail()` per company whenever that company's webhook batch includes an AR-relevant event, debounced to a 60-second minimum interval per company (checked against `quickbooks_ar_aging_sync_state.last_synced_at`, not inside the sync function itself, so the daily cron and manual-trigger route stay fully unconditional) — independent try/catch from the existing Invoice CDC sync, so a problem in one never gets marked against the other's own processed/failed status.

**One real, unverifiable-from-here dependency flagged explicitly, not assumed either way**: whether QuickBooks actually delivers the 4 new entity types to this app's webhook endpoint depends on the Intuit Developer Dashboard's subscription configuration (external, not in this repo) — since the discarding happened in OUR OWN code, it's plausible QuickBooks was already sending these events and we were just dropping them, but this needs to be observed post-deployment (watch `quickbooks_webhook_events` / the new `console.warn` logs for real Payment/CreditMemo/JournalEntry/Deposit activity), not assumed.

`npx tsc --noEmit` / `npm run build` clean. `docs/INVARIANTS.md` INV-QB-021 (new, plus a "CLOSED" note appended to INV-QB-015), `docs/FEATURE_MAP.md`, `docs/REGRESSION_CHECKLIST.md` REG-021 (new), `docs/CURRENT_STATE.md` all updated in the same change. Full design plan at `C:\Users\vincent\.claude\plans\atomic-wandering-locket.md`.
...
Verified against live data: 380 companies keep a real positive balance across the 3 books; 34 net to exactly $0 and 50 net negative (real examples: Cyber Quantum Pte Ltd (USD) -$22,040.26, Cyber Quantum Pte Ltd -$16,131.11, down to sub-dollar rounding cases like -$0.50) — 84 total now hidden from the list/export, down from the original 464 "Clients With a Balance."
...
**2. SOA Excel export — same bug the on-screen list had, plus a real wrong-total bug.** Vincent, after seeing the on-screen ACCADIA MANAGEMENT SERVICES example (91+ bucket: 7 real line items netting to exactly $0.00): "这样Export Full Workbook 那边也是要更新一下内容显示了". `lib/soa-export.ts`'s `renderAgingTable()` had the SAME `> 0` gate the on-screen list was fixed for yesterday, independently never updated — a bucket cell showed blank whenever its net was zero or negative, hiding real activity. Worse, found while fixing it: the sheet's own bottom TOTAL row summed each bucket column using that SAME `> 0` filter on every company's contribution BEFORE summing — meaning the printed grand total for that column was actually wrong (silently excluding every company whose net in that bucket was zero or negative), not just visually incomplete.

Fix: a bucket cell now shows the real net (`aging[bucket]`, the one already-computed source of truth — never re-derived from line items) whenever the bucket has any real line item behind it (checked via `lineItems.length`, not the net's sign); since an Excel cell can't stack lines the way the on-screen table now does, the itemized breakdown (type/reference/signed amount, one per line) goes into that cell's own Excel comment/note instead — verified this actually persists through a real save-and-reopen `.xlsx` cycle, not just in-memory before save. The TOTAL row's sum lost its `> 0` filter too, fixing the real wrong-total bug. A new shared `SoaExportRow` type replaces 4 separately hand-maintained copies of the same row shape across `lib/soa-export.ts` and `app/api/billing/soa/export-all/route.ts`.

`npx tsc --noEmit` / `npm run build` clean throughout both fixes. `docs/INVARIANTS.md` INV-QB-020 (new), `docs/REGRESSION_CHECKLIST.md` REG-020 (new) added for the export fix.

Previous entry follows.

Last updated: 2026-09-16 (SHIPPED AND VERIFIED: real double-billing risk in ND renewal period calculation, found via a Billing Drafts screenshot — "这边的开单Period 好像有点判断失误呢".

Siehi Shipping Pte. Ltd.'s (TAC) most recent ND invoice, #02580282 (PAID), billed two lines in the SAME invoice: "Nominee director service for one year (Aug 2025 - Dec 2025)" $1,250 (a "primary" QB line, `product_service` "Secretary:Nominee Director Fees - LJW") and "(Jan 2026 - Jul 2026)" $1,750 (a "deferred" QB line, "Deferred - ND Fees - LJW") — together one real 12-month renewal covering Aug 2025 through Jul 2026. Billing Drafts proposed the NEXT ND period as "Jan 2026 - Dec 2026" — re-billing Jan-Jul 2026, already paid for in that same invoice's deferred line.

Root cause: `lib/invoice-period.ts`'s `compareRenewalPeriodProductLines()` always ranks a "primary" QB line above a "deferred" one when picking which line's `period_end` is authoritative — correct when comparing lines from DIFFERENT invoices (guards against an unrelated ad-hoc line, e.g. a one-off CPF submission, coincidentally outranking the real renewal), but wrong within the SAME invoice, where Tassure's own ND billing convention sometimes splits one renewal into a primary stub period + a deferred continuation period covering DIFFERENT months, not the same period twice. The primary-first rule picked the EARLIER (primary) line's `period_end` (Dec 2025) as the invoice's true coverage end, silently understating it.

Fix: compare by `period_end` alone (latest wins) when the two lines share the same `invoice_no`; only fall back to primary-first ranking across different invoices. Both real call sites (`app/api/billing/renewals/route.ts`'s renewal-status computation, and `app/api/quickbooks/create-invoice/route.ts`'s `validateRenewalPeriods` overlap check that runs when actually generating an invoice) share this one function, so both are fixed together.

**Real scope, not a one-off**: a full paginated scan of `quickbooks_invoice_items` (the default Supabase 1000-row cap would have badly undercounted this — paged properly to 3,636 real rows) found this exact same-invoice primary+deferred split-into-different-sub-periods pattern on **25 real companies**, all ND, all previously mis-sorted the same way. Verified the fix against real data: Siehi Shipping's own next proposed ND period corrected from the wrong "Jan 2026 - Dec 2026" to the true "Aug 2026 - Jul 2027".

`npx tsc --noEmit` / `npm run build` clean. `docs/INVARIANTS.md` INV-QB-019 (new), `docs/FEATURE_MAP.md` (new `lib/invoice-period.ts` row), `docs/REGRESSION_CHECKLIST.md` REG-019 (new) all updated in the same change.

Previous entry follows.

Last updated: 2026-09-15 (SHIPPED AND VERIFIED: four more SOA fixes from real Vincent screenshots/questions, same investigation thread as the entries below.

**1. Multi-currency, rigorously re-verified, not re-asserted.** Vincent pushed back directly on the earlier "the report already SGD-converts" claim ("在QB内我们也是有记入 USD/SGD和人民币的，这些不能把全部的货币都当成新币(SGD)") — asked for real proof, not a repeated assumption. Went to live QuickBooks and checked with exact arithmetic on 2 independent real transactions: Cyber Quantum Pte Ltd (USD)'s Journal Entry — raw 16,040.26 USD × its own ExchangeRate 1.3740588 = 22,040.26, matching this app's stored figure exactly; FAITH CAPITAL GLOBAL FUND VCC's Invoice — raw 507.55 USD × ExchangeRate 1.2861 = 652.76, matching QuickBooks' own `HomeBalance` field AND this app's stored figure exactly. Confirmed: MultiCurrencyEnabled is true on all 3 books (TAB/TAC/TAO), the `AgedReceivableDetail` report's `Header.Currency` reads `"SGD"` on all 3, and only USD customers currently exist anywhere (TAB 5, TAC 2, TAO 0) — no RMB/CNY customer exists today despite Vincent's concern, though the conversion mechanism itself is currency-agnostic (per-transaction `ExchangeRate`, not USD-specific) so it holds the moment one appears. INV-QB-016 updated from "not yet fixed" to closed, with this evidence.

**2. Real bug: SOA detail modal / merged PDF empty for companies whose display name differs from their real QuickBooks name.** Vincent found it via ACG Interior & Exhibition Pte Ltd — the on-screen Outstanding row showed a correct S$4,540.00 total, but opening the detail modal showed zero rows. Root cause: `app/api/billing/soa/detail/route.ts` and `app/api/billing/soa/pdf/route.ts` both pass their `?companyName=` query param (a full, already-fuzzy-resolved display name, e.g. "ACG Interior and Exhibition Pte. Ltd.") straight through to `loadArAgingSnapshot()`, which uses it as a literal SQL `ilike '%...%'` substring filter against the real QuickBooks `customer_name` ("ACG Interior & Exhibition Pte Ltd") — "&" vs "and" alone breaks literal substring containment even though the two names fuzzy-match fine everywhere else in this app. Fixed at the shared choke point: `loadArAgingSnapshot()` and `legacyComputeSoaRows()` now both reduce any `customerNamePrefilter` through `significantWord()` (relocated from `lib/company-360.ts` to `lib/company-name.ts` to avoid a circular import) before using it, idempotently, so callers that already pre-reduced their own input are unaffected. Verified against live data: old behavior 0 rows, new behavior 2 rows for ACG. New INV-QB-018.

**3. Aging-bucket cells redesigned to show every line item, not a net.** Vincent, separately: "当一个列里面出现多过一个单逾期的时候，这些欠款都应该要出现在List...我一行一个数字" — a bucket cell used to show only ONE net number, which hid real activity two ways: a bucket that happened to net to a coincidental value looked like a single transaction, and (worse) a bucket where several real line items net to exactly $0 showed nothing at all. Concrete example Vincent gave: ACCADIA MANAGEMENT SERVICES PTE.LTD.'s TAO 91+ bucket has 7 real line items (5 invoices + 2 Journal Entries) netting to exactly $0.00 — the list showed a bare dash. `app/billing/soa/_components.tsx`'s aging-bucket cells now render every `lineItems` entry in that bucket on its own line (oldest-due-first, matching the existing convention), each colored/tagged by its own sign — not a bucket-level net tag anymore, so the two now-superseded helper functions from the earlier tag fix were removed rather than left unused. Row `alignItems` changed from `center` to `start` to match Company 360's existing multi-line-cell precedent, since rows can now be genuinely taller. Verified against ACCADIA's real data: renders exactly the 7 lines Vincent listed.

**4. Detail modal rows are now clickable to view each transaction's own PDF.** Vincent: "这些单都要是可以点开查看PDF的". `app/api/quickbooks/invoice-pdf/route.ts` gained an optional `?docType=creditmemo` param (purely additive — every existing caller omits it and is unaffected) so one shared route serves both Invoice and Credit Note PDFs via QuickBooks' parallel `/invoice/{id}/pdf` and `/creditmemo/{id}/pdf` endpoints. Detail-modal rows with a real document (`type !== 'other'`) open that PDF in a new tab on click; Payment/Journal Entry/Deposit rows (`type === 'other'`) stay plain, non-clickable text with a title explaining why — consistent with the existing "Other Adjustments" PDF-page reasoning that these transaction types genuinely have no official QuickBooks document to show, not a gap to paper over.

**5. Abbreviation tooltips** (Vincent: "这种有简写的好像稍微要有一个窗口描述到底是什么") and **Owner→Main PIC rename** (Vincent: "换成类似于Main PIC会不会比较好呢") — both already shipped earlier today, see below.

`npx tsc --noEmit` / `npm run build` clean throughout. `docs/INVARIANTS.md` INV-QB-018 (new) and INV-QB-016 (updated), `docs/FEATURE_MAP.md`, `docs/REGRESSION_CHECKLIST.md` REG-018 (new) + REG-017 (extended) all updated in the same change.
...
Re-verifying against QuickBooks' own AgedReceivables report after the real sync found the fix cut the system-wide gap roughly in half ($185,722.57 → $90,168.88) but did NOT fully close it — TAC landed almost exactly right ($3,689.02 left), TAO had $18,586.20 left, TAB still had $67,533.66. Built a precise customer-by-customer diff (our net total vs QuickBooks' own per-customer AgedReceivables row, same `normalize()` matching key `computeSoaRows()` already uses) instead of continuing to guess at QuickBooks entity types abstractly — this pinpointed TAB's gap to 2 customers covering 97% of it:

1. **TASSURE PAC** ($27,413.06 overstated) — Vincent confirmed this is an internal inter-company settlement account (TAB/TAC/TAO are 3 different companies; PAC sometimes pays one company's expenses on another's behalf), not a real client. **Fixed**: excluded from `computeSoaRows()` (`lib/soa-data.ts`'s new `INTERNAL_ACCOUNT_NORM_NAMES` set) — the one shared computation, so every consumer (SOA pages/exports, Company 360, AI assistant tools, collection emails) now correctly never shows PAC as a client owing money.
2. **Cyber Quantum Pte Ltd / "Cyber Quantum Pte Ltd (USD)"** ($38,171.37 overstated combined) — real negative balances sit on the QuickBooks `Customer` record itself, but this customer has ZERO Invoice/CreditMemo/Payment/RefundReceipt/SalesReceipt records of any kind, at any date — checked directly against live QuickBooks, not assumed. Confirmed as a genuine multi-currency case: this Customer is a distinct USD-denominated record in QuickBooks. **Not fixed** — where this specific balance actually originates (likely a legacy/migrated balance or a manual adjustment mechanism not yet identified) needs either QuickBooks-side research or Vincent's own institutional knowledge of this customer's history.

Vincent then confirmed the broader pattern: "这个就是我们财务说的货币问题，因为我们的记录基本是靠SGD的，但是有一些公司是给美金和人民币的" — a known finance-team issue, not isolated to Cyber Quantum. Full customer-by-customer discrepancy lists for TAB (38 rows)/TAC (7 rows)/TAO (23 rows) were handed to Vincent directly (not filed anywhere else) for him to work through against QuickBooks himself. Multi-currency handling (capturing `CurrencyRef`/exchange rate at sync time, deciding conversion-vs-separate-display) is explicitly NOT started — a separate, properly-scoped piece of work, not a quick follow-on to the CreditMemo fix. TAO's remaining $18,586.20 gap is more diffuse (no 1-2 dominant customers the way TAB had) and has not been root-caused yet.

Added `docs/INVARIANTS.md` INV-QB-016 documenting both findings (the PAC exclusion and the not-yet-fixed multi-currency gap) so neither gets silently rediscovered or silently "fixed" by broadening the CreditMemo logic incorrectly.

Verified: `npx tsc --noEmit` / `npm run build` clean after the PAC exclusion and the aging-bucket display fix.

Previous entry follows.

Last updated: 2026-09-15 (Phase 1 shipped: SOA/Outstanding Balance now nets QuickBooks CreditMemo (Credit Note) balances, fixing a confirmed $185,722.57 (~42%) system-wide overstatement of what clients owe.

Vincent noticed a "Credit Note" concept on the SOA side that looked like it wasn't being deducted from outstanding balances. Investigated with real production data before writing any code (not guessed): the daily QuickBooks sync (`app/api/quickbooks/sync/route.ts`) has only ever pulled `Invoice` objects, never `CreditMemo`. QuickBooks itself does not reduce an invoice's own `Balance` when a CreditMemo is left "Unapplied" (applying is a separate manual step), but QuickBooks' own official Aged Receivables report DOES net an unapplied CreditMemo into that customer's total. Confirmed via QuickBooks' own report API (`/v3/company/{realmId}/reports/AgedReceivables`) — the TAC number matched a real Excel export Vincent pulled directly from QuickBooks exactly, validating the method — that this system was overstating total receivables by $185,722.57 (~42%) across TAB ($112,878.67 over, ~68%) / TAC ($29,957.70 over, ~26%) / TAO ($42,886.20 over, ~26%) combined. Real example: Ligang Limited (TAC) showed $790 owed; QuickBooks' own report nets it to **-$970** once its $1,760 unapplied CreditMemo (CN268021, memo explicitly "CN TAC 02680170") is counted. Directly queried all 3 QuickBooks companies' CreditMemo data too: 478 total, 59 unapplied (~$85K), most with memos explicitly referencing an invoice number — read as a process gap ("created the CN, forgot to click Apply"), not intentional, and NOT something this fix depends on anyone cleaning up manually — the system now computes the correct net number regardless of whether any of those 59 ever get formally applied in QuickBooks.

Used two Explore-agent research passes (outstanding-balance consumer map, QuickBooks sync architecture) to find the real fix surface before writing a plan: `computeSoaRows()`/`computeAllSoaRows()` (`lib/soa-data.ts`) is the ONE shared computation feeding 6+ features (SOA pages/exports, Company 360, 3 assistant chat tools, firm-pulse, chat-export, company-deep-lookup) — confirmed via `docs/INVARIANTS.md`'s existing INV-DATA-022 — but 3 independent paths bypass it entirely with their own direct `quickbooks_invoices` queries and needed separate fixes: `app/api/billing/soa/detail/route.ts` (drill-down modal), `app/api/billing/soa/pdf/route.ts` (**the actual merged SOA PDF sent to a client**), and `lib/client-comms-resolve.ts` (SOA collection-email candidate list + line items — without this fix a client with a real net credit could have received a collection email for money they don't owe).

**Shipped this pass**: new `quickbooks_credit_memos` table (`scripts/add-quickbooks-credit-memos.sql`, Vincent needs to run this once in the Supabase SQL Editor — no DDL access from this sandbox); `syncCreditMemoYear()` added alongside the existing `syncYear()` in the daily cron (3 companies × 3 years, same pagination/dedup/batch-upsert/stale-row-reconcile pattern, deliberately simpler — no line-items table, no revenue/period classification, since CreditMemo's only role here is balance netting); `computeSoaRows()` nets CreditMemo balances into the same customer bucket (keyed by the same `normalize(customer_name)` Invoice already uses — no new matching scheme invented), bucketed into the CreditMemo's OWN aging bucket (matching the real Ligang example exactly: -1760 in the 31-60 column, 790 separately in 91+over, only the Total column nets to -970); all 3 bypass paths fixed (the PDF route now also merges in each unapplied CreditMemo's own official QuickBooks PDF via the symmetric `/creditmemo/{id}/pdf` endpoint, rather than computing a number itself — the most transparent, least-guessable fix for a client-facing document). Added `docs/INVARIANTS.md` INV-QB-015 and a new `lib/soa-data.ts` row in `docs/FEATURE_MAP.md`'s high-risk shared logic table (a real, pre-existing documentation gap — the function already fanned out to 6+ features with no entry there).

**Deliberately deferred (Phase 2, not done)**: the webhook/incremental real-time CreditMemo path (`lib/quickbooks-invoice-incremental.ts`'s CDC query hardcodes `entities=Invoice`; `app/api/quickbooks/webhook/route.ts`'s receiver hard-filters to `'invoice'` entity type only and would silently drop a CreditMemo webhook event even if Intuit sends one; `lib/quickbooks-webhook-queue.ts`'s processor doesn't even select `entity_name` from queued events). CreditMemo data is therefore only as fresh as the daily cron for now — the same "up to a day stale" tolerance `INV-QB-014` already accepts for Invoice data itself, not a new risk class.

Verified: `npx tsc --noEmit` / `npm run build` clean. Real-world verification still pending Vincent running the migration + a sync cycle: re-check the QuickBooks AgedReceivables-vs-system total comparison (should land near $447,149.03 combined instead of $632,871.60), and confirm Ligang Limited nets to -$970.
...
**Correction, then a correction of the correction (same day):** first thought Supabase was in Tokyo (the long-standing assumption), then a Supabase world-map tooltip screenshot appeared to say "Southeast Asia (Singapore)" so that got written up as the real answer — then a SECOND, more specific screenshot of the actual "Primary Database" info card settled it properly: **Northeast Asia (Tokyo), ap-northeast-1, t4g.nano** (NANO tier — too small a project to have a multi-region replica, so this is the one real location; an explicit AWS region code on the database's own info card beats a world-map tooltip). The original "Tokyo" note was right all along.

Net effect: Vercel's Function Region is now Singapore (`sin1`) — chosen to match the existing 12+2-route code convention, not because anyone had confirmed it was the closest option — which already fixed the dominant lag problem (P75 20s→1.83s, timeouts gone) since Singapore is still reasonably close to Tokyo. It is NOT the same region as the database, though. Switching Vercel's Function Region to Tokyo (`ap-northeast-1`/`hnd1`) instead would close that last, much smaller remaining gap — offered to Vincent as an optional further optimization, not yet done, not urgent given the current result is already good.
...
Enriched the 22 (of the previously-flagged 33) rows that had zero real source material anywhere and were the most clearly describable from general regulatory knowledge: PC1-a/b/c/d/e/g/h (particulars/address/secretary/business-activity/auditor/officer/dividend changes), PC2-a/b/c/d/e (name change, constitution amendment, share certificate split/replace, charge removal, EGM for bank facilities), PC3-a/b/c (share allotment, share transfer, share transfer with corporate shareholder), PC4-a (share capital deduction), PC6-a/b (bank attendance, court attendance for tax), PC9-a/b/c/d (CTC documents, register extracts, RORC update lodgment, KYC/CDD re-verification). Left T2-2/T2-3/T2-4, T3-2 through T3-5, and T1-5D/T1-7/T1-10/T1-11 alone — these describe Tassure's own recurring/bundled services rather than a generic statutory procedure, so a generic description would be more likely to mislead than help; better to wait for Vincent's real material there.

Critical difference from every prior enrichment pass in this thread: this content is NOT sourced from any Tassure document. Every touched row's `remarks_en`/`remarks_cn` got an explicit, verbatim-repeated label appended — "GENERAL KNOWLEDGE, NOT TASSURE-VERIFIED: ... confirm Tassure's own specific execution steps before relying on this to actually perform the task." — so the provenance distinction Vincent's "AI employee executing real tasks" end goal requires (仿真员工执行真实任务前必须知道这是通用知识还是Tassure真实流程) survives in the data itself, not just in this chat. A couple of rows (e.g. PC1-e, Change of Auditor) got extra hedging in the description text itself where the general procedure is genuinely more involved/risky than a routine filing (removing an auditor mid-term needs Companies Act special-notice procedure, not just a board resolution) — flagged as needing Tassure's own legal/compliance input, not presented as a simple administrative step.

Applied live via direct Supabase REST API PATCH (fetch-current-value-then-concatenate-in-JS, sidestepping the `NULL || text = NULL` footgun structurally), verified every row by re-query afterward (not by trusting the script's own 200 status alone) — same reliable path established after the two SQL-editor silent-failure incidents. `scripts/add-service-pricing-updates-5.sql` written for repo history/fresh-environment replay: idempotent (guarded on `remarks_en NOT LIKE '%GENERAL KNOWLEDGE, NOT TASSURE-VERIFIED%'`), validated for balanced parens/quotes and exactly 22 real statement-terminating semicolons before committing.

Remaining gap: 11 rows still thin (T2-2/T2-3/T2-4, T3-2/T3-3/T3-4/T3-5, T1-5D/T1-7/T1-10/T1-11) — these need Vincent's own description of what Tassure actually does, not general regulatory text, since they're Tassure-specific recurring services rather than one-off statutory procedures.

Previous entry follows.

Last updated: 2026-09-11 (Resolved the trademark price conflict; ingested a deep company-incorporation process guide; applied writes directly via the Supabase API instead of risking a third SQL-editor failure.

Vincent: "以这个为准（$1,000/类别（多一类加$500）)" — resolved the trademark conflict from the prior entry. While applying it, found `scripts/add-service-pricing-updates-2.sql` had NOT actually been run yet (`updated_at` still equalled `created_at` on the live rows) — folded the price fix directly into that script rather than shipping a third one.

After Vincent ran that script, spot-checking (never just trust "SQL好了") found it had PARTIALLY failed silently, again: PC7-a/PC7-b's description stayed NULL, and PC5-a's price hadn't taken the fix. Root-caused properly this time: `description_en || text` evaluates to NULL in PostgreSQL when `description_en` is already NULL (true for PC7-a/PC7-b) — a silent no-op, not an error. PC5-a's issue was a stale editor draft (its description update, present in every version, DID apply; only the price fields added in the later edit hadn't). Rather than risk a third failed round-trip through the web SQL editor, applied both fixes DIRECTLY via a Supabase REST API PATCH (the same service-role credential already used for read verification) and confirmed via re-query. Fixed the script file too (COALESCE + idempotency guards) so it's correct if ever replayed on a fresh environment, without duplicating the now-live data.

Then ingested a 4-page client-facing "Singapore Company Registration Process" guide (Tassure's own material) — the size-check tool had misreported it as "234 pages, 22.8MB"; verified independently via pdfjs-dist directly (`doc.numPages`) that it's genuinely 4 pages, not a truncated read. Real content: full KYC/CDD document checklists (individual vs corporate shareholder, UBO 25%+ threshold), the RORC 30-day filing rule, e-signing workflow, a realistic incorporation timeline, a 6-item post-incorporation checklist (board meeting, bank account 2-6 weeks, secretary within 6 months, auditor within 3 months with exemption criteria, RORC, industry licences), a full Pte Ltd vs LLP vs Sole Proprietorship comparison table, and Tassure's own quick-reference mnemonic ("两个本地/两个范围/三个名称..."). Appended the KYC/timeline detail onto the existing Company Incorporation (T1-1) and Bank Account (T1-3) rows' descriptions, and added 3 new `company_service_terms` topics for the content that doesn't map to one priced line item — applied live via the same direct API approach, `scripts/add-service-pricing-updates-4.sql` written for repo history/fresh-environment replay.

Vincent's framing for all of this: "以后可以通过这些数据和员工习惯去模拟出一个AI员工去执行每天繁琐重复的任务" — this knowledge base is the first layer toward an eventual AI-employee that executes real tasks, which is exactly why process depth (not just price) has been the priority throughout, and why fabricated-sounding process detail was refused (asked for real source material instead) rather than guessed from general knowledge. 39 of 66 service_pricing rows are still "thin" (price only, no real process) — mostly the Post-Incorporation-Changes transactional items — flagged back to Vincent to supply source material for, rather than invented.

Verified: row counts and specific field values re-queried directly from production after each write (66 service_pricing rows, 6 company_service_terms rows) — not assumed from "the script ran."
...
**Root cause #1 — two templates in the repo were stale.** `md5sum`'d every file in `templates/post-incorporate/` against his Desktop `Post Incorporate - Tassure` folder: `12 ND_AGREEMENT` and `05 Engagement_Letter` had drifted. 05's diff was cosmetic (a decorative image); 12's was a real, deliberate business change — the template now names only the LARGEST shareholder as "the Shareholder" party (new `{{largest_shareholder_name/_identification_number/_address}}` placeholders), not every shareholder. Confirmed via the old Python tool's source: it already has a manual "最大股东" (largest shareholder) selector defaulting to whoever holds the most shares. Ported both: new `PostIncorporateCompany.largestShareholderName` (optional override) + `largestShareholder()` helper (auto-picks by share count, ties broken by entry order — deliberately not the old tool's random tie-break) in `lib/docx-post-incorporate.ts`, and a new "最大股东 Largest Shareholder" dropdown in `app/post-incorporate/page.tsx`, shown only when ND service is YES, matching where it sits in the old tool. Copied both refreshed template files into the repo.

**Root cause #2 — a real, systemic gap in the doc-generation engine.** One screenshot showed a literal unresolved "{{ND_name}}" in a small table — traced to `word/footer1.xml` inside the ND_AGREEMENT template (a page footer, not body content). `loadTemplate()`/`renderDoc()` only ever touched `word/document.xml`; ANY placeholder living in a Word header or footer was copied through completely unprocessed, for every one of the 16 templates, silently, this whole time — this specific template just happened to be the one that actually uses one. Fixed `renderDoc()` to also run the same placeholder-fill/marker-strip pass on every `word/header*.xml`/`word/footer*.xml` part it finds, applied to all 14 call sites (not just ND Agreement) so a future template edit adding a footer placeholder elsewhere doesn't reproduce this silently. `test-orchestrator.ts`'s `fullText()` also now scans header/footer parts — it structurally could not have caught this bug before, since it only ever read the body. INV-DOC-008, INV-DOC-009.

**Still open, not yet fixed** — flagged back to Vincent with specifics rather than guessed at: the "05 Engagement Letter" signature block's floating "⬅ Sign / 签字" text box overlaps a wrapped director name (already-known Word-template-layout issue, confirmed still present in his refreshed Desktop copy too — needs a manual reposition in Word, not a code fix); Bizfile PDF parsing issues (currency sometimes truncated e.g. "UNITED STATES OF" missing "AMERICA DOLLAR"; a new manually-added shareholder's currency field isn't editable; FYE shows month only when it should show day+month) — investigation started (`lib/bizfile-parse.ts` already has a hard-won real fix from 2026-09-09 for multi-page shareholder tables on this exact company, LAKEFILL VENTURES, so the "only 2 of 5 shareholders detected" report needs the actual source PDF to confirm whether that fix covers this case or a new one — asked Vincent for it rather than guessing blind.

Verified: `npx tsc --noEmit` / `npm run build` clean; `test-orchestrator.ts` all passing, including 2 new checks (largest-shareholder clause names the right person with his own ID; footer placeholder is actually filled, not left literal).
...
Bill To panel refinements after Vincent used it: translated to English (the Billing page's own language), collapsed by default with a summary chip so a configured c/o still reads at a glance while closed, and the edit path fixed — update-invoice never received `billTo`, so the fields silently did nothing on a row already editing an invoice, which is the normal state for any company invoiced this cycle. The parent-link conflict now warns inline as soon as a c/o is typed, instead of only as a note after generating.

Vincent asked whether the c/o "is generally the parent company", i.e. whether the placeholder should name it. Checked against the real data rather than agreeing: of the 7 clients who have ever had a c/o printed, ZERO have a parent link set; 3 of the c/o parties are not companies of ours at all (Han Kun LLP is a law firm, SMARTEE Singapore and Greenwich Peninsula Partners are an overseas affiliate and a management firm); and only 1 of 947 companies has a parent link at all. So c/o is NOT the parent in this data, and a parent-named placeholder would assert something false. Instead, where a parent IS linked, the panel offers a one-click "Use Bill-To parent: X" fill. Worth Vincent's attention separately: the parent-company Bill-To feature is used by exactly one company out of 947.

Assistant — person-activity permission model (2026-09-10). Vincent specified a rank ladder for who may ask the assistant about whom's activity/whereabouts: owner (Vincent) > partners > leaders (Jay, Hoe Chyi, Seng Xin, Clarence, Lina, Felicia) > staff (incl. Chelsea — he moved her here from a brief 'transparent' tier). Vincent sees all; a partner is visible only to Vincent and not to other partners; a leader is visible to owner/partners/other leaders; staff see each other and nothing above. It gates ONLY person-centric tools — client data, invoicing, arrears, deadlines stay open to everyone, and a blocked "what did <leader> do" still allows "what changed on <company>". team_activity / active_users_today now filter to the visible people (with a hidden-count note) instead of a hard management-only refusal, so a regular staff member finally gets a real team-activity answer about their peers. firm_pulse opened to all (deadlines + money). lib/person-visibility.ts + test-person-visibility.ts (matrix pinned). INV-DATA-051.

Assistant — department roster (2026-09-10). Vincent: "你可以分出各部门人员有谁吗？" got "no such tool". The org structure was already in the codebase — as section comments on lib/staff-directory.ts (Partners / Corporate Secretarial / Accounting / Tax / Audit ...). Promoted those to a real `team` field (35 people), cross-checked against the live PIC distribution (Accounting names do appear in ar_reminder.acc_pic, Tax in tax_pic, etc.). New `team_roster` tool (34 tools) returns each team's members plus a live per-person company load from ar_reminder, and the separate Nominee-Director roster with active appointment counts. Also: the reply that prompted this had leaked a raw tool name (`list_companies`) — HOW TO ANSWER now forbids naming a tool/function in a reply.

Answer layout (2026-09-10).  Vincent, after the data reached field level: "已经很完美了，但是我嫌弃回答太机械化，应该要好像完整的 ai 那样会认真分析过后回答". This was prompt, not data — the model was transcribing tool results row by row ("（4 项，最新 15:06）", every field listed). Added a HOW TO ANSWER block at the top of the static prompt: a tool result is raw material, not the reply; lead with the takeaway, write flowing sentences, call out what's notable (a PIC reassigned, a value that looks wrong), match depth to the question, and never restate the tool's own counts/groupings/field-names as if that were the answer. The team_activity note was aligned to it. On Sonnet this should land; on Haiku it could not have.
...
**The real gap it closes:** staff generate the invoice here and then retype the c/o inside QuickBooks every cycle, because this system omits `BillAddr` entirely and QuickBooks refills it from the customer default. Confirmed live on Kelun Health (TAB #02610648 — generated by this system, then hand-patched) and 吉木锌国际贸易（上海）.

**Two levels, per Vincent** ("又要跟着客户走，又要每单选"): a stored per-company default on `companies`, and a per-invoice override in the create-invoice request body that wins for that invoice only and writes nothing back. The address under the c/o line is an explicit choice — 'b' the c/o party's own address (default), 'a' the client's own, 'custom' typed — because both conventions exist in the real data and they mean different things (send it to the agent vs. send it to the client noting the agent).

**Precedence:** c/o beats the parent-company override, and when a company has both, the invoice result carries a note saying which was used. INV-QB-0CO2.

Verified against live QuickBooks, composing (not sending) every branch: the 'b' case reproduces the real staff-typed invoice TAC #02680288 line for line, and correctly notes that "Novix Ai Global" exists but has no address so the client's own was used — which is exactly why the real invoice looks that way. Empty-custom, Attn-only, c/o+Attn and 5-line overflow all degrade with a visible note. **Nothing configured sends no BillAddr at all**, leaving ~99.9% of invoices byte-identical.

**Still to do:** run `scripts/add-companies-bill-to-care-of.sql` (until then `loadCareOfSettings` returns all-null and invoicing is unchanged — verified against production), and build the per-invoice Bill To fields into the Billing Drafts row (the API contract accepts them; no UI sends them yet). Setting the per-company default is available now through chat (`preview_company_update`, `billto:*`).

**Also found, not built:** Cindy's mockup includes a `UEN:` line. No invoice in any book has ever printed a UEN (0 of 6,544) — that is a new request affecting every invoice, not something QuickBooks already does, and is left for Vincent to decide separately.

Previous entry: Assistant — 互通 phase 7: the full AR Reminder record opens in chat.
...
**2. Singapore time everywhere else too** (Vincent: "一切以新加坡时间为准"). Audited every `now`-based date computation. Vercel functions run in UTC, which is the PREVIOUS day until 08:00 SGT and the previous YEAR until 08:00 on 1 Jan. Fixed: trademark renewal window, the fallback intent-router's deadline window, chat export stamps, three SOA export/PDF filenames, Reports `generatedAt`, and — the one that had reached a real financial record — **`create-invoice`'s txnDate default**, which dated an invoice raised before 08:00 SGT to the previous day in QuickBooks. Nine `getFullYear()` call sites (billing/FYE/QB-sync year) moved to `thisYearSGT()`. Only defaults changed; an explicitly supplied date is still honoured exactly as passed. `deadlines-lookup` and `renewals` were already correct. INV-DATA-036.

**3. Real SOA actions in chat.** Vincent on the previous SOA answer: "还是非常简陋，功能不齐全", and then, on the general principle: "我已经说很多次了要有实际功能，只是在每次真正要操作实际功能的时候，敏感操作，需要跳出弹窗获得用户点击同意AI助手协助执行". The old reply was prose plus two markdown links pointing at the SOA page — leaving the actual job (download the statement, send it to the client) undone. There is now a real `SoaCard`: per-book breakdown with a working **下载 SOA PDF** button and a working **起草邮件** button that builds the campaign draft with the merged PDF attached and opens the same Outlook send window the page opens. Sending stays a human click in that window. Crucially the card does not reimplement any of it — `downloadSoaPdf`/`buildSoaDraft`/`loadSoaActor` were EXTRACTED from the SOA page into `lib/soa-actions-client.ts` and the page now calls them too, so there is one implementation of the client-facing money email, not two that can drift (`git diff` on `_components.tsx` is a clean mechanical move: 4 insertions, 57 deletions, no behavior change beyond the SGT filename date). The tool's note was rewritten to stop it pasting soa_link markdown and re-listing what the card already shows. INV-DATA-037.

Verified: `npx tsc --noEmit` / `npm run build` clean; SOA card data checked against the exact company in Vincent's screenshot (1V CAPITAL — TAB S$1,000 #02610894 due 2026-07-31 / TAO S$2,650 #02660519 due 2026-07-08, total S$3,650, matching line for line). The two SOA buttons themselves have NOT been exercised end-to-end in a browser.

Previous entry: Assistant — making it usable by a human, not just capable. Vincent: "你再思考思考人类的使用方式，怎么样让AI chat 更简单易懂人类的提问，怎么样更实用" → then "全部做完" on the resulting 5-item list. All five are done; the last three in this entry.

1. **Ambiguous names no longer read as "we don't have this client."** `findUniqueBestMatch()` already reported `ambiguous:true` when several names tied, but every caller read only `.value` and answered "No company matched" — confirmed on production: typing "remobie" (3 real companies) produced a flat denial. New `resolveCompany()` in `lib/company-name.ts` returns exact / best / ambiguous / none, de-duplicates candidates by normalized name, and falls back to substring for the short brand names people actually type; the tools now ask *which one*. Verified: "remobie" → asks between 2 deduped candidates, "1v capital"/"lakefill" still resolve, garbage still returns none.

2. **Collections worklist.** `getCollectionsWorklist()` answers a collections person's most common question — "who owes us money on MY list" — grouped by the owner the SOA page itself assigns (confirmed `soa_owners` pick → Class/Location suggestion → sole-PIC fallback). Verified: Chelsea 3 companies/S$3,405.50, Kah Ye 50/S$74,154, whole firm 419/S$645,288.10.

3. **`firm_pulse` — the app's own headline prompt was a dead end.** The most prominent suggested prompt is "What should I prioritize today?", and the only tool behind it (`my_tasks_summary`) shows the CALLER's own AR/Late Filing rows — verified on production that for Vincent this returns `everAssigned:false, total:0`, i.e. the app's front-and-centre question answered "you have never been assigned anything." New `lib/firm-pulse.ts` composes the existing deadline/late-filing/outstanding computations into one firm-wide "what needs attention right now" (management-only), so the numbers always agree with the individual tools. Its note also warns about two things a human spots and an LLM does not: the biggest debtors are routinely Tassure's OWN group entities (intercompany, not client debt), and the worst overdue AR filings are years old and usually already being struck off — Late Filing is the list actually worth chasing. Verified live: 25 overdue AR filings, 12 overdue AGMs, 16 active late filers, S$645,288.10 across 419 customers.

4. **"Hand me the list" — chat answers are now downloadable.** Chat could finally answer list questions but a person cannot work from "16 家逾期" or the first 40 of 419 names in a bubble; the honest end of every such answer was "go to the page and re-apply the filter yourself." A list-shaped tool now attaches an `_export` offer, the UI renders a real download button (`ListExportCard`), and the click POSTs to the new `/api/assistant/export`, which **re-runs the same tested lookup server-side** and builds the .xlsx from that — so nothing the model wrote can reach the file, and the export is never truncated the way the reply is. Four kinds: company list (any filter), collections worklist, deadlines, late filing. Permissions mirror the page each list belongs to via the same `isWithinRestriction()` check the billing tools use. The offer is stripped from the tool result before the model sees it — a model that can see an export descriptor narrates it ("I've exported it for you"), the same false claim the preview→confirm pattern exists to prevent. INV-DATA-034. Verified against production: 370 FYE-December companies, 377 address-service, 419 collections rows / S$645,288.10, 62 deadlines, 16 late filing — every figure matching the corresponding tool.

5. **Discoverability — nothing in the UI said what the assistant could be asked.** All the suggested prompts dated from when it could only explain the system ("怎么开单？", "数据什么时候更新？"), so 29 real data tools were invisible. Every page's suggestions were rewritten to lead with real data questions, under two hard rules recorded in the code: a click sends the text VERBATIM (so no "XX 公司" placeholders — clicking one literally asks about a company called XX), and every suggestion must be really answerable. The static prompt also gained a concrete answer for "你能帮我做什么" — grouped by the questions a person would actually ask, not by tool name. INV-DATA-035.

Note for later: `list_companies` itself has no per-account restriction check, so a restricted (AR-only) account can see up to 200 companies in chat but is correctly refused the download. The export route is the stricter of the two; the chat tool's own gap predates this change and was left alone rather than narrowed silently.

- New: `lib/firm-pulse.ts`, `lib/chat-export.ts`, `app/api/assistant/export/route.ts`, `ListExportCard`.
- Extended: `lib/company-name.ts` (`resolveCompany`), `lib/outstanding-lookup.ts` (`getCollectionsWorklist`), `lib/company-list-lookup.ts` (`unlimited`, reachable only from the export path), `app/api/assistant/route.ts` (29 tools; capability + export guidance), `components/AssistantWidget.tsx` + `app/my-tasks/page.tsx` (suggestions, export card).
- `docs/INVARIANTS.md`: INV-DATA-034, INV-DATA-035.

Verified: `npx tsc --noEmit` / `npm run build` / `test-orchestrator.ts` all clean; every export kind run against real production data. The download itself is behind the user's click and has not been exercised end-to-end through the HTTP route in a browser.

Previous entry: Assistant — finishing the round-2 sweep: the AR read/write coherence gap, and tool routing.
...
1. **Chat couldn't recognise former clients at all.** A struck-off/terminated company is routinely deleted from `companies` while master_list keeps its record — 4 of 6 sampled struck-off companies had no `companies` row, so company_deep_lookup answered "no company matched" for real clients the firm served for years. Added a master_list fallback (`recordSource: 'master_list_only'`) returning what the company WAS. Related: the tool read only `companies.tw_status`, which can disagree with master_list's lifecycle category (268 terminated / 263 struck off) — it now returns that category and is told to answer "is this still our client" from it. INV-DATA-030.
2. **ar_batch had the same filing-vs-money confusion that caused a real wrong answer before.** It returned only Annual-Return FILING status, so "4月有几家没开单" (a billing question) had no correct answer. Now returns filing_status and billing_status separately. INV-DATA-031.

Then the gaps, all built and verified against real data:
3. **list_companies** — the biggest structural hole: every tool answered about one named company or gave one global number; nothing returned a filtered LIST. Now answers "Chelsea 负责哪些公司" (16), "哪些公司用注册地址" (377), "12月FYE" (370), "XBRL" (35).
4. **upcoming_deadlines** — unifies AR filing, AGM and trademark-renewal deadlines plus what's already overdue (25 AR overdue, 12 AGM overdue). Verified that ar_reminder.due_date already carries the EOT-revised date, so it's used directly and the original is shown as context — 20 real companies have an extension that was invisible to chat.
5. **customer_profile_summary** extended from 1 of Reports' ~5 leadership outputs to all of them — service mix, customer source, client-lifecycle roll-up, address-service footprint by office, and this year's flow (+148/-88).
6. **recent_changes** — the audit log (4,364 rows of field-level "who changed what") had zero coverage; now resolves row ids to company names, formats SGT, and separates human edits from automated syncs (7d: 26 human vs 1032 system).

7. **Real execution (H)** — new `preview_ar_update`, the fifth confirm-gated action: mark an AR cycle prepared/sent/received/AGM-held/FILED, assign a Secretary/Accounts/Tax PIC, or set remarks. Follows the established preview → user-click-Confirm → execute pattern exactly: the tool is read-only, the card shows a real before/after, and only the click PATCHes the real conflict-safe /api/ar-reminder endpoint. It sends the previous value, so a value someone else changed meanwhile is REJECTED (409) and shown to the user rather than silently overwritten, and the chat-writable field list is deliberately narrower than that endpoint's own allowlist. INV-DATA-033.

Three real bugs caught while verifying my own new code: the address-service count silently dropped an active client with no UEN (376 vs the true 377); the audit totals reported Supabase's 1000-row fetch cap as the real count (true 1058, INV-DATA-032); and the first version of the deadline walk would have mis-stated EOT dates had `due_date` not already carried them (checked rather than assumed).

- New: `lib/company-list-lookup.ts`, `lib/deadlines-lookup.ts`, `lib/audit-lookup.ts`, `lib/ar-update-lookup.ts`; `ArUpdateCard`/`ArUpdateConfirmModal` in `components/assistant/ChatCards.tsx`.
- Extended: `lib/company-deep-lookup.ts` (lifecycle/contact/former-client), `lib/customer-profile-lookup.ts`, `lib/reports-data.ts` (parseFlexibleDate/computeClientFlow extracted from the Reports route so both share one computation), `app/api/assistant/route.ts` (5 new tools + guidance), both chat surfaces, `lib/ai-conversations.ts` (new persisted preview type).
- `docs/INVARIANTS.md`: new INV-DATA-030 through INV-DATA-033.

Verified: every new tool run against real production data before wiring in; `npx tsc --noEmit`/`npm run build`/`test-orchestrator.ts` all clean. The AR update path was verified read-only (preview returns the real current PIC/filing date and writes nothing) — the write itself is behind the Confirm click and has NOT been exercised against production data.

Previous entry, same day: Assistant — full data-coverage sweep. Vincent: "你再认真思考分析一下目前系统的数据还可以怎么样补充利用和理解，不要每次我随便问一个东西，chat 都不能理解" (seriously analyze what more of the system's data could be surfaced to chat — stop letting every casual question hit a gap). Reviewed all ~20 lib data modules against the (then-)17 registered chat tools before writing any code, found the single highest-leverage gap plus 5 smaller ones, presented the full list with priority tags, and — on "全部都要做" (do all of them) — built all 6:

1. **`company_deep_lookup` — the highest-leverage fix.** `search_company` only ever exposed a thin slice of what Company 360 actually knows (name/UEN/FYE/status/3 service flags/PIC/2 AR rows). Since "ask about company X" is the single most common shape of casual question, this was the biggest source of "chat doesn't understand." New tool reuses `getCompany360()` directly (never a second, re-derived notion of "everything about this company") and curates it into a chat-safe summary — directors/secretary/shareholders, trademarks, invoice history, Post Incorporate documents generated, ND appointments, outstanding balance, communications draft count. Deliberately DROPS personal ID numbers/DOB/home address/personal contact info from the officials/shareholders roster before it ever reaches the LLM — a casual "who's on the board" question only needs names and roles. Verified against the same real 1V Capital company used earlier this session — outstanding total matched exactly (S$3,650), real directors/secretary/document history all correct.
2. **Trademark — was a complete chat blind spot** (zero tools, confirmed by a real search of the codebase). New `trademark_summary` (company-wide: registered vs. in-progress counts, marks expiring within 180 days) — company-specific trademark data now also comes for free through `company_deep_lookup` above, reusing the exact same `trademark_records` fuzzy-match `getCompany360()` already does.
3. **Late Filing had no aggregate view** — only a single-company resolve-preview existed. New `late_filing_summary` (total active overdue, broken down by severity and by which PIC currently has the most). Extracted the real page's own `categorize()` function into a new shared `lib/late-filing-categorize.ts` (pure, dependency-free) so the chat tool classifies rows the EXACT same way the page's own metric cards do, rather than re-deriving the rule — caught a real client/server module-boundary risk while doing this (the same class of bug fixed once already this session for `formatDisplayDate`) and avoided it up front this time.
4. **ND roster had no capacity view** — `nd_lookup` could only answer "which companies is person X on." New `nd_roster_capacity` runs the same per-person appointment count across the whole real 14-person roster (154 total active appointments; real spread from 0 to 27 per person) — deliberately reports relative bandwidth (sorted fewest-first) rather than inventing a "N slots free" figure, since no fixed capacity-per-person rule exists anywhere in this system.
5. **Client Communications had no company-wide view** — only `check_email_status` (one company). New `communications_summary` (real sent-count over a recent window, plus all-time status/campaign-type breakdown) reusing the exact same `email_drafts` join `lookupEmailStatus()` already uses.
6. **Reports' revenue trend / PIC workload weren't reachable from chat** even though already computed on that page. Extracted `computeRevenueTrend()`/`computePicWorkload()` out of `app/api/reports/route.ts`'s own inline code into `lib/reports-data.ts` (mechanical, verbatim — Reports' own output re-verified unchanged against real data after the move) so a new `revenue_workload_summary` tool (same `canViewReports` gate as `customer_profile_summary`) could reuse them directly instead of a second copy.

**Also fixed while building #1 above**: found and fixed the SSIC-casing data-quality bug (`docs/INVARIANTS.md` INV-DATA-027) in the ONE place flagged as still-open from the previous entry — `app/reports/page.tsx`'s own "Explore" section pivot now also normalizes to uppercase before grouping. Checked `app/api/reports/export/route.ts` too and deliberately left it alone: it's a flat per-company row export with no aggregation, so the casing inconsistency never silently splits a count there — normalizing would just hide the real raw value from someone who might want to go clean up the source data.

- **`lib/company-deep-lookup.ts`** (new): `lookupCompanyDeep()`.
- **`lib/trademark-lookup.ts`** (new): `getTrademarkSummary()`.
- **`lib/late-filing-categorize.ts`** (new): `categorizeLateFilingRow()`, moved from `app/late-filing/page.tsx`'s local `categorize()` (page now aliases to the shared one).
- **`lib/late-filing-lookup.ts`**: new `getLateFilingSummary()`.
- **`lib/email-status-lookup.ts`**: new `getCommunicationsSummary()`.
- **`lib/reports-data.ts`**: new `computeRevenueTrend()`/`computePicWorkload()`, extracted from `app/api/reports/route.ts` (which now calls them instead of its own inline copies).
- **`app/reports/page.tsx`**: SSIC `DIMENSIONS` entry normalizes case before grouping.
- **`app/api/assistant/route.ts`**: 6 new tools registered (`company_deep_lookup`, `trademark_summary`, `late_filing_summary`, `communications_summary`, `nd_roster_capacity`, `revenue_workload_summary`) + matching static-prompt guidance.
- **`docs/INVARIANTS.md`**: INV-DATA-027 updated (Reports page fixed, export route deliberately not); new INV-DATA-028 (PII minimization rule for any future chat tool touching personal roster data) and INV-DATA-029 (pure cross-boundary functions need their own dependency-free module — 2nd real instance of the INV-DOC-006 risk).

Verified: all 6 new tools run against real production data before wiring into chat — `company_deep_lookup` (1V Capital, matched known-good outstanding figures exactly), `trademark_summary` (65 registered/6 in-progress, real company names), `late_filing_summary` (16 active overdue, real severity split — genuinely ALL currently PIC-unassigned, a real finding not a bug), `communications_summary` (57 real sends in 30 days, 1971 all-time sent), `nd_roster_capacity` (14-person roster, 154 total appointments), `revenue_workload_summary` (real dollar figures, real staff names) — and `computeRevenueTrend`/`computePicWorkload` specifically re-verified to produce identical output before/after the extraction. `npx tsc --noEmit`/`npm run build`/`test-orchestrator.ts` all clean.

Previous entry, same day: Assistant — Vincent's continued live testing surfaced 3 more real chat-assistant gaps, unrelated to Post Incorporate:

1. **SOA conflated with Billing Drafts.** Asked "我要开SOA", the assistant offered to preview a NEW invoice draft instead — wrong. SOA is a real, different feature: a PDF of unpaid invoices downloaded from a company's own QuickBooks book (`/billing/soa/tab|tac|tao`), with a real "Draft Email" button right there. Fixed with a new `soaDeepLink()` (`lib/deep-links.ts`) and real `openCompany` auto-open support added to `app/billing/soa/_components.tsx` (same convention `/billing` and `/late-filing` already use) — `check_outstanding_balance`'s result now carries a real, clickable `soa_link` per QB company. Caught a real Suspense-boundary build error along the way (`useSearchParams()` in the app router needs one) — fixed the same way `late-filing`/`billing` already do, wrapping once in `_components.tsx` so all 4 pages that render it (tab/tac/tao/all) get it for free.
2. **"客户最大是什么类型/行业" — a real capability gap**, asked twice, both times honestly told "no such tool, browse Companies/Active Client yourself." The Reports page's own "Explore" section already computes exactly this (`lib/reports-data.ts`'s `buildReportsCompanyRows()` — real company-type + SSIC industry data, backfilled for the whole roster). New `customer_profile_summary` tool (`lib/customer-profile-lookup.ts`) reuses it directly rather than re-deriving, gated on `canViewReports` same as the page itself. Found and fixed a genuine data-quality bug while building it: `ssic_description_1` has inconsistent casing in production ("WHOLESALE TRADE..." x140 vs "Wholesale trade..." x12 — same industry, silently split by any raw-string grouping) — now normalized to uppercase before counting. **Not yet fixed**: Reports' own Explore section and its export route have the same casing-split risk — flagged in `docs/INVARIANTS.md`, out of scope for this chat-assistant fix.
3. **Activity timestamps: two distinct real bugs.** (a) Asked "Jay 今天做了什么", the reply's underlying data was right but showed a raw UTC timestamp ("2026-09-09 02:04:08 UTC") the user didn't recognize as the "10点" (10am) they remembered — `lib/activity-data.ts`'s `getPersonActivitySummary`/`getCompanyActivitySummary` never converted to Singapore time before handing timestamps to Claude. Fixed with a new shared `formatSgtDateTime()` (`lib/date.ts`, same convention `app/page.tsx`'s own local formatter already used) applied in `recentActivitySummary()`/`myActivityPattern()`. (b) `active_users_today`'s `days=1` default ("today") was actually a ROLLING 24-hour window, not the Singapore calendar day — a reply describing itself as "过去24小时" while the user asked about "今天" is a real, if subtle, mismatch. Fixed with a new `sinceFor()` in `lib/activity-data.ts`: `days<=1` now uses the real SGT midnight boundary; `days>1` keeps rolling-window behavior (the only semantic that makes sense for a genuine multi-day trend).

- **`lib/deep-links.ts`**: new `soaDeepLink()`.
- **`app/billing/soa/_components.tsx`**: `openCompany` auto-open (mirrors `app/late-filing/page.tsx`'s own pattern exactly, including the `triedAutoOpen` guard against re-opening after a manual close); wrapped in `<Suspense>` for `useSearchParams()`.
- **`lib/activity-data.ts`**: new `sinceFor()`, used by both summary functions.
- **`lib/date.ts`**: new `formatSgtDateTime()`.
- **`lib/customer-profile-lookup.ts`** (new): `getCustomerProfileSummary()`.
- **`app/api/assistant/route.ts`**: `checkOutstandingBalance()` now attaches `soa_link` per line; new `customerProfileSummary()` tool + `customer_profile_summary` registration; `recentActivitySummary()`/`myActivityPattern()` format timestamps before returning; `active_users_today`'s tool description/note corrected ("today" not "rolling 24h"); static prompt gained SOA-vs-Billing-Drafts guidance and a note that activity timestamps arrive pre-formatted.
- **`docs/INVARIANTS.md`**: new INV-DATA-024 through INV-DATA-027.

Verified: `npx tsc --noEmit`/`npm run build` clean (the Suspense fix specifically required a real build, not just tsc, to catch — same lesson as the previous entry's module-boundary bug); `getCustomerProfileSummary()` run against real production data (911 active clients, "Exempt Private Company Limited by Shares" the largest legal-entity type, "WHOLESALE TRADE..." the largest industry at 152 after the casing fix, ~86% of active clients have a real SSIC on file); `soaDeepLink()` verified against the real 1V Capital outstanding-balance data from an earlier entry (correctly produces both its TAB and TAO SOA links); `test-orchestrator.ts` (unrelated to this entry, Post Incorporate) still passes.

Previous entry, same day: Post Incorporate — Vincent supplied the REAL Bizfile PDF (LAKEFILL VENTURES) for the first time this round, which exposed that the previous entry's page-merge fix was still wrong in 2 more ways, plus 3 smaller real issues from his continued live testing:

1. **Multi-page merge walk-rule was wrong.** The previous "walk until the section's terminator appears" rule assumed a terminator (next heading, or the "Includes nationality..." footnote) appears exactly ONCE, at the true end. Tested against the real 2-page Shareholder(s) table: ACRA reprints BOTH the section heading AND that footnote on EVERY page the table spans, so the old rule stopped one page early (3 of 5 shareholders, not 2 — better than before, still wrong). Real, confirmed-reliable signal: a page belongs to the same table's run if and only if it repeats the exact same section heading text; stop at the first page that doesn't. Rewrote `collectSectionItems()` around this rule.
2. **`sectionBand()` picked the FIRST terminator match, not the last.** Once a terminator label can legitimately repeat per-page (see #1), anchoring the table's lower boundary to the first occurrence wrongly excludes every row on a later merged page. Now takes the LOWEST (last) occurrence's Y instead.
3. **Header/footer/footnote boilerplate leaking into row data across page boundaries.** With multi-page merging now correctly walking further, 3 distinct kinds of repeating boilerplate needed exclusion — found one at a time by testing the real PDF, each confirmed necessary: the disclaimer header block every page repeats at its top (`HEADER_ITEM_PATTERNS`, stripped from every page but the section's own first); the table's own column-header row repeated on continuation pages (`TABLE_HEADER_LABELS`, same rule); and bare 1-2-digit reference-number superscripts floating just above either a footnote or a header label (`stripSuperscriptNoise()`, stripped from EVERY page including the first, since a table's own first-page footnote/header superscript is also not real data once more pages follow). Verified end-to-end against the real PDF: all 5 officers and all 5 shareholders now parse correctly with clean fields, share counts summing to exactly 10,000 (matching the Capital section's own total). `docs/INVARIANTS.md` INV-TW-019/020/021 rewritten to describe the actual final mechanism, not the first (wrong) attempt.
4. **FYE format was wrong even once matching worked.** Vincent: the field should show "31 December" (spelled-out month, matching Incorporation Date's own convention and what actually goes into generated documents), not "31/12" — despite the field's own label saying "(DD/MM)". Changed the format accordingly.
5. **Incorporation Date "format" complaint.** Native `<input type="date">` displays in the browser/OS's own locale (e.g. "21/08/2026"), which Claude Code cannot control — added a small formatted label alongside it (`formatDisplayDate()`, "21 August 2026") so the spelled-out form Vincent expects is visible without changing the underlying ISO-bound picker (still needed for real date math downstream).
6. **New feature: drag-and-drop for the Bizfile PDF upload**, alongside the existing click-to-upload — same `dragActive`/`onDragOver`/`onDragLeave`/`onDrop` pattern already used for My Tasks chat's attachment drop zone, reused rather than re-derived.

One real build-breaking bug caught before shipping: importing `formatDisplayDate` (for #5) from `lib/docx-post-incorporate.ts` into the client component broke `next build` — that module pulls in `fs`/`pizzip` for real document generation, and even ONE plain value import (not `type`-only) drags the whole thing into the browser bundle. Fixed by moving the function to the already browser-safe `lib/date.ts` (already imported by several other client pages) instead. `docs/INVARIANTS.md` new INV-DOC-006.

- **`lib/bizfile-parse.ts`**: `collectSectionItems()` rewritten (heading-repeat walk rule); `sectionBand()` fixed (lowest-occurrence terminator); new `HEADER_ITEM_PATTERNS`/`TABLE_HEADER_LABELS`/`stripSuperscriptNoise()`; new shared `headerBlockBottomY()` (the header block's true bottom line, not just "Name"'s own line — a wrapped 2nd/3rd header line's own superscript was slipping past the old fixed buffer) used by both `extractOfficersFromItems`/`extractShareholdersFromItems`; `extractShareholdersFromItems`'s `changedHit` lookup split onto its own `topHeaderY` so it isn't affected by the header-bottom change.
- **`lib/date.ts`**: new `formatDisplayDate()` (moved from `docx-post-incorporate.ts`, byte-for-byte unchanged behavior).
- **`lib/docx-post-incorporate.ts`**: `formatDisplayDate` now imported from `./date` instead of defined locally; no longer re-exported (client code imports `lib/date` directly).
- **`app/api/post-incorporate/enrich/route.ts`**: FYE format `DD/MM` → `D Month`.
- **`app/post-incorporate/page.tsx`**: formatted Incorporation Date label; Bizfile upload drop zone.

Verified: real Supabase-sourced Bizfile PDF (LAKEFILL VENTURES) run through the actual `parseBizfilePdf()` pipeline end-to-end — company/officers/shareholders all match the real document exactly, share counts sum correctly; full `test-orchestrator.ts` Post Incorporate suite (13 document types) still passes unchanged; `npx tsc --noEmit`/`npm run build` clean (the module-boundary bug above was caught specifically by running the real build, not just `tsc`).

Previous entry, same day: Post Incorporate — Vincent's own real-data re-test of the previous entry's fixes surfaced 2 more real issues, one of them a genuine conceptual bug baked in since the Post Incorporate port itself:

1. **FYE still showed month-only after the previous fix.** Root-caused with a real Supabase query on the exact test company: `companies.fye_day = 31` WAS present (previous fix's premise held), but `master_list.fye = "DEC"` — a 3-letter ABBREVIATION, not "December". The previous fix's month lookup did an exact-string match against full month names, which silently failed on "DEC" and fell through to the month-only fallback. Fixed by matching on the first 3 letters instead (the same technique `lib/invoice-period.ts`'s own private `monthNumber()` already uses) — now correctly produces "31/12".
2. **ND vs. Nominator — a real conceptual bug, not the "understandable/low priority" gap it looked like in the previous entry.** Vincent: "我之前一直误解了我把ND 当成是 NOMINATOR...这部分一开始好像就不对了". Read "08 Declaration of Maintenance of ROND"'s actual template text directly to settle this: the per-nominee block is a letter FROM the Nominator TO the company ("I, the undersigned, have appointed a nominee director..."), signed by the Nominator in THEIR OWN capacity — a genuinely separate real person/entity from the ND, not the same person in two roles. The previous port had this backwards: `app/post-incorporate/page.tsx`'s Bizfile-parse handler auto-filled the "Nominator" fields with the ND's OWN bio (`nominatorIndName: d.name`) — confidently wrong data on a real legal declaration. Removed that wrong auto-fill entirely (now correctly blank after parse, same as it already was for nominee shareholders). Added the convenience feature Vincent actually asked for ("jianwei的generate tool是可以选nominator"): a "Quick-fill Nominator from an existing Director/Shareholder" picker in both the Director's and Shareholder's nominee panels — the real nominator is very often already one of the other people already entered in the same form; picking one is a one-shot copy (name/address/nationality/ID/DOB/email/phone), never a live binding, leaving "Date Became Nominator" for manual entry since it has no source anywhere.

- **`app/api/post-incorporate/enrich/route.ts`**: month matching now compares the first 3 letters (case-insensitive) instead of the full string.
- **`app/post-incorporate/page.tsx`**: removed the wrong director Nominator auto-fill (and its now-dead `nomineeDetailByName`/`nomineeDirectorDetails` local plumbing); new `nominatorCandidatesFrom()`/`nominatorFillFrom()` helpers + a picker `<select>` in both nominee panels.
- **`docs/INVARIANTS.md`**: new INV-DOC-005.

Verified: real Supabase query confirmed `master_list.fye = "DEC"` for the exact test company (LAKEFILL VENTURES) and that the fixed 3-letter matching now resolves it to "31/12"; the full `test-orchestrator.ts` Post Incorporate suite (13 document types incl. ROND/RONS nominator branches) still passes unchanged; `npx tsc --noEmit`/`npm run build` clean.

Previous entry, same day: Post Incorporate — 4 real bugs from Vincent's uploaded bug-report doc (`NEW_SYSTEM_POST_INCORP_1.docx`, SYSTEM-vs-SHOULD-BE screenshot pairs against a real test company, LAKEFILL VENTURES PTE. LTD., 5 real shareholders). Root-caused each against real code before fixing, in Vincent's own confirmed priority order:

1. **Bizfile shareholder detection: only 2 of 5 found.** `parseBizfilePages()` looped per PDF page and only ever read the ONE page containing the "Shareholder(s)" heading text (`shareholders = extractShareholdersFromItems(...)`, an assignment, not a merge) — a table that overflows onto a continuation page (no repeated heading, as ACRA's real layout does) silently lost every row past the first page. Fixed with a new `collectSectionItems()`: walks forward from the heading's page, merging in later pages until the section's real terminator appears (next heading, or — for Shareholder(s) — the "Includes nationality.../Abbreviation" footnote), offsetting each later page's Y by a large fixed step so the existing Y-based row/column logic keeps working unmodified across the page boundary. Applied to both Officer(s) and Shareholder(s) for symmetry. `docs/INVARIANTS.md` INV-TW-019.
2. **Capital table currency truncation.** `parseCapitalTable()` only read ONE line after the "Issued/Paid-Up Share Capital" heading and split it by tab assuming exactly 4 cells — a long currency name ("UNITED STATES OF AMERICA DOLLAR") wraps onto its own PDF line, so it came back truncated ("UNITED STATES OF") and Share Type (which wrapped along with it) came back blank. Fixed by reading forward across up to 5 lines as one continuous token stream: first 2 tokens are Amount/Number of Shares, last token is Share Type, everything between is Currency. INV-TW-020.
3. **New shareholder's currency wasn't editable.** `app/post-incorporate/page.tsx`'s Paid-Up Capital row showed the per-shareholder currency as a plain `<span>{s.currency || 'SINGAPORE DOLLAR'}</span>` — display text with no input at all. Confirmed via grep that this field is never consumed by document generation (only the separate top-level `company.currency` is), so this was purely a UI gap with zero generation-side risk. Now a real editable `<input>`.
4. **TeamWork-linked FYE showed month only, not day+month.** `app/api/post-incorporate/enrich/route.ts` only ever selected `companies.fye_month`, and its own comment claimed "no source in this system has the day-of-month" — wrong: `companies.fye_day` is already TeamWork-synced and already used elsewhere (AR Reminder due-date math, Company 360's own FYE display). The route just never queried it. Now selects `fye_day` too and formats `DD/MM` when both are on record, falling back to month-only (previous behavior) when the day isn't.

Two items investigated and found to need NO code fix: the reported "ND Agreement shows only 1 of 5 shareholders, no bullets" turned out to be fully explained by bug #1 above — verified by generating a real ND Agreement doc with 5 well-formed shareholders (`generatePostIncorporateDocuments`) and confirming all 5 render correctly, each as its own real Word bullet paragraph (`numPr`/`numId=3`, a valid definition in the template's `numbering.xml`) — the template and `repeatSection()` were never broken, the input just never had 5 shareholders in it before. And the ND Agreement/Engagement Letter signature block's stray "/" in a wrapped name (e.g. "ZHANG /\nWEIZENG") was root-caused to a floating "⬅ Sign / 签字" text box in `05 Engagement_Letter...docx`, absolute-positioned assuming a short one-line name — when a longer name wraps (the "Name:" paragraph has its own `w:right` indent narrowing available width), the floating box visually overlaps the wrap point. This is a Word template layout fix (reposition the floating box or widen the paragraph), not a code fix — flagged to Vincent rather than risking a blind edit to floating-drawing XML coordinates. A last item, "Nominator Nationality isn't auto-filled," Vincent himself called understandable (Bizfile genuinely has no nominator data) and asked instead for an old-tool-style nominator picker — a real but separate feature request, not implemented here, needs its own scoping.

- **`lib/bizfile-parse.ts`**: `collectSectionItems()`/`pageHasTerminator()`/`KNOWN_SECTION_HEADINGS`/`SECTION_TERMINATORS`/`PAGE_Y_STEP` (new); `parseBizfilePages()` rewritten to use them; `parseCapitalTable()` rewritten for multi-line token collection.
- **`app/post-incorporate/page.tsx`**: shareholder currency `<span>` → `<input>`.
- **`app/api/post-incorporate/enrich/route.ts`**: `fye_day` added to the `companies` select; `financialYearEndDayMonth` now built as `DD/MM` from `fye_day` + `fye_month` (new local `MONTH_NAMES`), falling back to month-only.
- **`docs/INVARIANTS.md`**: new INV-TW-019, INV-TW-020.

Verified: `npx tsc --noEmit`/`npm run build` clean; the full existing `test-orchestrator.ts` Post Incorporate document-generation suite (all 13 document types, every conditional branch) still passes unchanged. Two throwaway synthetic sanity scripts (deleted after use, per house rule) confirmed the exact failure→fix behavior for both `bizfile-parse.ts` changes: a synthetic 2-page shareholder table (2 rows page 1, 3 rows page 2 with no repeated heading) now returns all 5 in correct order, and a single-page table is provably unaffected; a synthetic wrapped-currency capital table now returns the correctly rejoined currency + previously-lost share type, and a normal single-line table is provably unaffected. **Not yet verified against the real source Bizfile PDF** — only Vincent's screenshots were available, not the PDF itself; he should re-run the same real LAKEFILL VENTURES Bizfile through `/post-incorporate` to give the final real-data confirmation these two parser fixes need.

Previous entry, same day: Assistant — a batch of real capability gaps found by Vincent stress-testing the chat, plus a second deterministic guard for the same hallucination family that hit outstanding-balance. Four distinct real issues in quick succession, same session:

1. **Permission-denial hallucination**: Vincent (real `canViewAsOthers: true`) asked "Chelsea 今天要做什么" then "Chelsea Ang" right after successfully using the exact same cross-person feature for "CKY" moments earlier — both got a fabricated "我没有权限查看其他员工的任务" refusal. `my_tasks_summary()`'s own real code only returns `permission_denied` when `!account.canViewAsOthers`, impossible for Vincent's account — so this was never a real tool result. Vincent: "你是不是傻了 我是Vincent 最大的Admin" / "它会有时候分不清楚权限". Same fix family as the outstanding-balance guard: new `claimsPermissionDenied()` scans the REPLY for a permission-denial claim; if the caller's account genuinely HAS `canViewAsOthers` and no cross-person tool call happened this turn, the claim is definitely false (ground truth is known server-side) and a corrective warning is prepended.
2. **"Who's active today" — a real capability gap, not a hallucination**: asked "今天除了Vincent 还有谁在使用这个系统", the assistant honestly said it has no such feature and asked for a specific name instead — technically true but "还是不够智能" per Vincent, because `getCompanyActivitySummary()` (the exact real computation behind Activity Insights) already tracks exactly this. New `active_users_today` tool exposes it directly (management-only, `canViewAsOthers`-gated, rolling day window default 1).
3. **"Was the email sent" — another real gap**: asked "Bao Fortune 的Email 发送出去了吗？", got told to go check the Email Activity page manually — "这个也是还判断不出来". New `check_email_status` tool (`lib/email-status-lookup.ts`) mirrors the real Email Activity/Delivery History page's own `email_drafts` query exactly.
4. **Company-wide outstanding total — the guard itself proved the gap was real**: asked "目前 TAB 的欠款总数是多少？", the assistant (correctly, this time — the new guard fired) said it had no tool for a QuickBooks-company-WIDE total, only per-company. New `outstanding_balance_summary` tool (`lib/outstanding-lookup.ts`'s `summarizeOutstandingBalance()`) sums the exact same `computeSoaRows()` with no prefilter — the real `/billing/soa` page's own computation — across every customer.

Verified all 3 new tools against real production data via a throwaway diagnostic: `active_users_today` (rolling 1 day) returned 10 real approved-account emails with real event counts (122 total events, Vincent's own account correctly dominant); `check_email_status('Bao Fortune')` returned the real sent record (status "sent", 2026-08-24, Vincent Seow, real subject line, real recipient addresses); `outstanding_balance_summary(['TAB'])` returned a real S$285,965.07 total across 236 real customers with plausible top debtors. The permission-denial guard was verified against the exact real hallucinated reply text from Vincent's own screenshots (both "Chelsea" and "Chelsea Ang" refusals detected; the real successful "CKY" reply correctly NOT flagged). `npx tsc --noEmit`/`npm run build` clean.

Previous entry, same day: Assistant — deterministic safety net after the outstanding-balance fix STILL got fooled once more. Vincent re-tested right after the `check_outstanding_balance` fix shipped: a terse follow-up naming a different company ("那么 1v capital呢", right after a genuinely correct $0 answer for another company) got a confident "✅ 确认：1V CAPITAL PTE. LTD. 没有欠款" reply with fabricated precise numbers ($0, 0 unpaid invoices) — for the SAME company that had just been verified minutes earlier to actually owe S$3,650. Vincent: "你看这个回复对吗？" The tool was never actually called for it; the model appears to have pattern-completed the previous, genuinely-correct answer's exact template rather than re-invoking the tool for the new company — proving prompt instructions alone can't be trusted to force a tool call every time, especially on a short follow-up that doesn't restate the topic in words a keyword check on the QUESTION could see.

- **`app/api/assistant/route.ts`**: new `mentionsOutstandingBalance()` scans the REPLY text itself (not the question — the real follow-up "那么 1v capital呢" contains no arrears-related word at all, confirmed by a diagnostic) for arrears/outstanding-balance language. `claudeAnswer()` now tracks whether `check_outstanding_balance` was actually invoked THIS turn (`outstandingToolCalled`); if the final reply mentions arrears/outstanding but the tool was never called, a visible `⚠️ 系统提示` warning is prepended before the reply ever reaches the user, telling them plainly not to trust it and to re-ask more explicitly. Static prompt strengthened further: explicitly states that a short follow-up naming a different company is a NEW company needing its OWN fresh call, never a reuse of the previous answer's template.
- **`docs/INVARIANTS.md`**: INV-DATA-022 extended with this second incident and the general principle — any future high-stakes factual claim (money, legal/compliance status) an LLM could plausibly pattern-complete instead of really checking should get the same deterministic, code-level output check, not just an instruction trusting the model.

Verified: a diagnostic ran `mentionsOutstandingBalance()` against the EXACT real hallucinated reply text from Vincent's screenshot (detected, correctly) and the exact real follow-up question text (confirmed it has no keyword at all — validating why the check had to move from question to reply). `npx tsc --noEmit`/`npm run build` clean. The underlying `check_outstanding_balance` tool itself was already verified correct against real data in the previous entry — this fix is specifically about making sure it actually gets CALLED, with a hard backstop for when it doesn't.

Previous update, same day: Assistant — real bug fix: the chat could wrongly claim a company has no outstanding balance. Vincent tested the new widget on a real company (1V Capital) whose real Company 360 page showed 2 unpaid invoices totalling S$3,650 (S$1,000 TAB, S$2,650 TAO) — the assistant instead answered "这家公司也没有欠款标记" (no arrears marker). Vincent: "这个回复就不对了" / "明明有outstanding". Root cause: `search_company`'s only company-status field, `ar_reminders`, is Annual Return FILING status ("Pending"/"Filed") — a completely different concept from money owed — and no tool existed at all for real outstanding-balance data, so the model read an unrelated field and answered as if it meant something it didn't, instead of saying it didn't know.

- **`lib/outstanding-lookup.ts`** (new): `lookupOutstandingBalance(companyQuery)` — finds the company (same `normalize()`/`findUniqueBestMatch()` matcher used everywhere else in this codebase), then calls the EXACT SAME `computeSoaRows()` (`lib/soa-data.ts`) Company 360's own Outstanding section and the real `/billing/soa` pages use, across all 3 QuickBooks companies (TAB/TAC/TAO) with the same `customerNamePrefilter`/company-match logic `lib/company-360.ts`'s own Outstanding section uses (its `significantWord()` helper exported for this reuse) — never a second, re-derived notion of "outstanding."
- **`app/api/assistant/route.ts`**: new `check_outstanding_balance` tool wired to it. `search_company`'s own tool description and the `ar_reminders` field it returns now carry an explicit warning not to use it for arrears questions. Static prompt gained a hard rule: any "does this company owe money / have arrears / outstanding balance" question MUST go through `check_outstanding_balance` and be answered strictly from its result — never from `ar_reminders`, general company data, or something discussed earlier in the same conversation about a different company.
- **`docs/INVARIANTS.md`**: new INV-DATA-022 — AR/AGM filing status and QuickBooks outstanding balance are unrelated concepts and must never be conflated, in this feature or any future one.

Verified against the exact real company from Vincent's screenshot: `lookupOutstandingBalance('1V Capital')` returned TAB S$1,000 (31-60 aging, invoice 02610894, due 2026-07-31, owner Chin Kah Ye) + TAO S$2,650 (61-90 aging, invoice 02660519, due 2026-07-08, owner Lee Jing Fei) = S$3,650 total — an exact match, line for line, against the real Company 360 screenshot Vincent shared. `npx tsc --noEmit`/`npm run build` clean.

Previous update, same day: Floating AI Assistant widget — full rebuild to match My Tasks chat. Vincent's spec, verbatim: "我希望它的功能和My Tasks这边的功能一样，但是它是一个小弹窗的形式，我希望这个弹窗是可以拖拽的，并且每次在弹窗内的对话都要记录成My Tasks 内的Recent (chat)" + 7 numbered rules on exactly what counts as a new conversation, what survives navigation vs. resets on reload, and where the button does/doesn't appear.

- **`components/assistant/ChatCards.tsx`** (new): the 4 agentic-chat preview cards (`InvoiceDraftCard`/`LateFilingResolveCard`/`InvoiceEditCard`/`PostIncorporateCard` + their confirm modals) and the attachment UI (`AttachmentChips`/`AttachmentThumbnails`/`AttachmentLightbox`) extracted from `app/my-tasks/page.tsx` — a mechanical move (Python line-slice with boundary assertions, then `git diff`/icon-and-label cross-checked against the original), not a retype, so a real QuickBooks/Late Filing/Post Incorporate write is never implemented twice for the two chat surfaces. One real bug caught by the cross-check during the move: `LateFilingResolveCard`'s icon had silently become `FileCheck2` instead of the original `AlertTriangle` — fixed before shipping. Also gained the shared `ChatMsg` type, `toApiMessage()` (turns a local message back into the content-block shape `/api/assistant` expects) and `storedMessageToChatMsg()` (rehydrates a saved `ai_messages` row's `preview_data` back into the right card) — both previously duplicated inline in `app/my-tasks/page.tsx`, now the one shared implementation.
- **`components/AssistantWidget.tsx`**: fully rebuilt. Draggable (mousedown on the panel's own header) and resizable (a corner handle, min 320×380, max 800×900) floating panel — position/size are sticky for the session (not persisted to storage, so a real reload resets them, matching rule 3). Three UI states (`button` idle / `open` / `collapsed`) live in the component's own React state, which the root layout (`app/layout.tsx`'s `<AppShell>{children}</AppShell>`) keeps alive across every client-side navigation since the widget renders OUTSIDE the `{children}` slot Next.js actually swaps — this is what makes rule 1 ("switch pages without clicking × and the conversation stays") true for free, no extra plumbing needed. Hidden specifically on `/my-tasks` (rule 4/5) by returning `null` from render rather than being conditionally unmounted by the parent — conversation state survives a visit to My Tasks and back. The first click each session (tracked by a `hasVisitedMyTasks` flag that also flips true just from landing on `/my-tasks` directly) redirects to `/my-tasks` instead of opening the popup (rule 4); Escape collapses rather than closes, since closing has a real consequence now (see below). Reuses `ChatCards.tsx` for cards/attachments and a fresh (not shared-hook) `sendChatMessage`/attachment implementation mirroring `app/my-tasks/page.tsx`'s own — the two surfaces' lifecycles (sidebar-driven thread switching vs. auto open/collapse/close) are different enough that a forced shared hook wasn't worth it under time pressure; the CARDS (the actual business-logic-touching pieces) are the one shared, non-negotiable part.
- Conversation lifecycle (rule 2): lazy-created on first real message sent (same pattern `app/my-tasks/page.tsx` already uses) — an opened-then-closed-with-nothing-sent popup never creates an empty row in My Tasks' Recent (chat). Once a message is sent, every exchange persists immediately via the existing `persistExchange()` (unchanged), so "closing ends and saves the conversation" is really just "closing resets local state" — the save already happened incrementally.

Verified: `npx tsc --noEmit`/`npm run build` clean; the extraction was cross-checked against the original for icon and button-label fidelity (see the `AlertTriangle` bug caught above). **Not exercised in a real browser** — drag, resize, collapse/expand, the close-then-reopen-is-a-new-conversation behavior, and the first-click-redirects-to-My-Tasks nudge are all real interaction behavior this sandboxed environment cannot click-test; Vincent needs to try the popup live on a few different pages.

Previous update, same day: My Tasks chat — click an attached image to view it full-size. Vincent, right after the drag-drop/paste attachment feature shipped: "假设我要看回去这个附带的图片，我想要点击放大查看是可以的吗？" — the sent-message thumbnails had no way to actually view them again. `AttachmentThumbnails`/`AttachmentChips` now take an `onView` callback: clicking an image thumbnail (on an already-sent message OR a still-pending one, before sending) opens a real full-size lightbox (`AttachmentLightbox` — click-outside or × to close); clicking a PDF chip opens the real file in a new browser tab instead, since a PDF doesn't "enlarge" the same way — the browser's own PDF viewer already does that job. `npx tsc --noEmit`/`npm run build` clean.

Previous update, same day: My Tasks chat — drag-drop/paste image and PDF attachments. Vincent: "我希望可以优化便利功能就是可以直接拖拽图片或者文件到对话框，或者可以在聊天框复制粘贴图片（作为此次对话的附带参考）". Anthropic's Messages API already accepts `content` as a plain string OR an array of text/image/document blocks, and `claudeAnswer()` already passed `content` straight through unchanged — so the real work was everywhere else that assumed `content` was always a string.

- **`app/api/assistant/route.ts`**: `Msg.content` widened to `string | ContentBlock[]`; new `messageText()` (extracts just the text portion — used by the intent-router fallback, title-deriving, and what gets persisted) and `attachmentSummary()` (a short `[附带 N 张图片/文件]` note). The rule-based intent-router engine can't see attachments at all — it now says so plainly in its reply rather than silently ignoring them. Added a request-time guard (max 6 content blocks, max 4.5M base64 chars per message) as defense-in-depth beyond the client's own caps. Static prompt gained one line telling Claude to actually look at an attached image/PDF when present.
- **`lib/ai-conversations.ts`**: unchanged in shape — the actual image/PDF bytes are deliberately never written to `ai_messages.content` (a plain text column; this is reference material for the live conversation, not a permanent archive), only the extracted text plus the attachment note.
- **`app/my-tasks/page.tsx`**: drag-and-drop onto the whole chat panel (a dashed-border overlay while dragging), paste-image support on both the empty-state and active-state input rows, plus a paperclip button opening a native file picker — image or PDF only, max 2 files, max 1.5MB raw each (worst case ~4.0MB once base64-encoded, kept comfortably under Vercel's ~4.5MB Serverless Function request-body ceiling — this exact threshold is also enforced server-side, kept in sync). Attachments ride along as real Anthropic vision/document content blocks on every subsequent request in the same browser session (so Claude keeps "seeing" them for as long as the 24-message history window still includes that turn), shown as thumbnail chips before sending and inline thumbnails on the sent message bubble. `components/AssistantWidget.tsx` (the floating widget elsewhere in the app) does NOT have this yet — scoped to My Tasks chat only, per Vincent's own wording.

Verified: a throwaway diagnostic re-ran the exact extraction/size-guard logic and confirmed the worst-case attachment payload (2 x 1.5MB raw -> ~4.19M base64 chars) clears the server's own 4.5M-char threshold and stays under Vercel's real ~4.5MB body limit. `npx tsc --noEmit`/`npm run build` clean. **Not yet exercised against the real Anthropic vision API from this sandbox** (no local API key — it's write-only in Vercel, never persisted) — Vincent needs to actually drag/paste an image into a real message and confirm Claude references it in its reply.

Previous update, same day: Assistant — chat reply readability pass: real markdown tables, more breathing room, a more visible deep link. Vincent screenshotted a real preview reply's own services table rendering as raw, unparsed `| a | b |` / `|---|---|` text (`components/assistant/ChatRichText.tsx` never handled markdown tables at all), shared a reference screenshot of a clean navy-header table, and asked for that look to become "the framework any future chart/table-like content in chat can reuse" rather than a one-off fix. Same message, also: "目前输出内容太紧凑了，能不能优化UI排版" (the output feels too cramped) and "深链很小也不明显，用户可能都不知道是可以点击的" (the deep link is small/inconspicuous — users might not know it's clickable).

- **`components/assistant/ChatRichText.tsx`**: `RichText` rewritten from a per-line `.map()` to a block-aware loop so a multi-line markdown table can be detected and consumed as one unit — a real table (GFM-style, with or without outer pipes) is recognized by its separator row (`|---|:---:|---|`, all dashes/colons/pipes) immediately following a row-shaped line, and rendered via a new shared `MarkdownTable` component (navy `#1d3a5c` header matching the app's own existing header convention, clean bordered grid, `overflow-x: auto` for narrow screens) — the SAME component every markdown table in chat uses, satisfying Vincent's "reusable framework" ask directly rather than a one-off. Spacing widened throughout (blank-line gap 8→14px, section-header top margin 6→16px, bullet/paragraph line margin ~2.5→4-5px) to de-densify replies. Both `app/my-tasks/page.tsx` and `components/AssistantWidget.tsx` import this same component, so the fix applies everywhere assistant replies render, not just My Tasks.
- **`app/my-tasks/page.tsx`**: the 3 preview cards' "Open in ___" deep link redesigned from an 11px plain-color centered text line into a full-width outlined secondary button (icon + label) directly under the real primary button — a standard, immediately-recognizable primary/secondary button pairing instead of a caption easy to miss.

Verified: a throwaway diagnostic re-ran the exact table-parsing logic against Vincent's real screenshotted text (5-column services table, 3 rows) and confirmed header/rows split correctly. `npx tsc --noEmit`/`npm run build` clean. Visual result not yet seen in a real browser — Vincent needs to look at a fresh reply himself.

Previous update, same day: Assistant — the 4 agentic-chat preview cards now survive a page switch/reload. Vincent, testing the smart-deep-link feature live: "我发现每次只能看到一次，当我切换了页面或者点击了接口，这个预览和深链的记录就不见了" — confirming the known, previously-flagged v1 gap: `invoicePreview`/`lateFilingPreview`/`invoiceEditPreview`/`postIncorporatePreview` were never persisted to `ai_messages`, so reopening a saved conversation always showed the plain text reply only, no card (and therefore no deep link either, since the link lives inside the card).

- **`scripts/add-ai-messages-preview-data.sql`** (new, **needs Vincent to run it** in the Supabase SQL Editor — no DB/Management API credential available in this sandbox to run it directly): `ALTER TABLE ai_messages ADD COLUMN IF NOT EXISTS preview_data jsonb`. One generic column, not 4 typed ones — `ai_messages` stays a general-purpose persistence layer agnostic to which tool produced a reply; the `{type, data}` shape is decided entirely in `app/api/assistant/route.ts`.
- **`lib/ai-conversations.ts`**: new `StoredPreview` type; `appendMessage()` gained an optional `previewData` param and is now self-healing — if the insert fails because `preview_data` doesn't exist yet on this database (migration not run yet), it silently retries without that field rather than breaking the plain-text save that has worked all along. Once Vincent runs the migration, preview persistence starts working with no further deploy needed.
- **`app/api/assistant/route.ts`**: new `toStoredPreview()` picks whichever of the 4 preview fields is set and wraps it; `persistExchange()` passes it through to the assistant's `appendMessage()` call only (never the user's own message).
- **`app/my-tasks/page.tsx`**: `openConversation()` now reconstitutes the right preview field from each message's `preview_data` when loading a saved conversation's history, so the card (and its "Open in ___" deep link) reappears exactly as it looked the first time.

Verified: `npx tsc --noEmit`/`npm run build` clean. **Not yet end-to-end verified against real data** — the migration hasn't been run on the live database yet (Vincent needs to run the one-line SQL above), so the actual "reopen a conversation and see the card again" behavior is code-complete but unconfirmed in production until then; the degrade-gracefully path (column absent) has been reasoned through but not exercised against a real Postgres "unknown column" error from this sandbox.

Previous update, same day: Assistant — reverted the same-day "which engine answered" chat indicator. Vincent asked me to check whether the Anthropic API token/credits had run out; unable to pull Vercel runtime logs (this token's scope doesn't cover them) or test the live endpoint directly (proxy.ts requires a real login session), so as a stopgap I shipped a small amber notice under any reply that came from the rule-based fallback engine instead of Claude, showing the real underlying error when there was one. Vincent's actual preference, stated directly: "很奇怪，我想要的就是（界面上完全看不出区别，回复看起来还是正常的），token 我自己会去看usage" — he wants ZERO visible difference regardless of which engine answers, and will check usage himself via the Anthropic Console. Reverted `app/my-tasks/page.tsx` and `components/AssistantWidget.tsx` back to discarding `engine`/`note` entirely — the API route itself still returns both fields (harmless, unused by the UI), only the display was added and removed. `npx tsc --noEmit`/`npm run build` clean both times.)
Local querying GIF replacement (2026-09-09): switched to Mid-Autumn Festival (5).gif. Source already has transparent alpha (green is the transparent palette color); preserved alpha instead of white-keying the gray/white robot. Cropped unused canvas (900x850 at 40,70), aspect-fit into 480x288 with transparent padding, retained 240x144 UI size and original-speed infinite loop (~6.67s). Reduced-motion still updated and blue-gray composite checked. Not yet pushed.


Local animation replacement (2026-09-09): switched querying GIF to Vincent's Mid-Autumn Festival (4).gif. Cropped the green right/bottom canvas, removed white background with the existing artwork-preserving process, and maintained aspect ratio within the 480x288 asset / 240x144 display. Original 12-second loop retained, reduced-motion still regenerated. Not yet pushed.


Previous update, same day: Local GIF transparency (2026-09-09): removed neutral white background across all 300 querying-animation frames, including enclosed background between the person's legs, while preserving pale lavender artwork and small highlights. Regenerated reduced-motion still. Asset is 480x288 for sharper 240x144 display; original 12-second loop unchanged. Checked composite against blue-gray background. Reproducible conversion in scripts/prepare-thinking-gif.mjs. Not yet pushed.

Previous update, same day: Local UI sizing (2026-09-09): increased querying GIF display to 240x144 (1.5x), with responsive width. Original playback speed unchanged. White-background removal is not included and remains pending.

Previous update, same day: Local UI replacement (2026-09-09): replaced the querying-state robot with supplied Mid-Autumn Festival (3).gif, preserving the complete robot-and-person artwork and original background. Optimized to 320x192, displayed at 160x96, looping at original speed (12 seconds per cycle), per Vincent's latest clarification. Updated reduced-motion still; empty-chat robot unchanged. Not yet pushed.
...
Previous update, same day: Assistant — smart deep links for the 3 agentic-chat preview cards. Vincent, after phases 1-4 had already shipped: "当用户点击去开单的时候你应该是带用户去到开单的接口，并且协助好找到对应的公司和点击好打开了那个发票编辑的弹窗，不只是带到 Billing draft 的接口页面就停了...思考用户真正要的便利和下一步到底可能是什么". The in-chat card+modal already lets the user finish the whole action without leaving the conversation, so this isn't a replacement for that — it's the escape hatch for when they want to do more than the compact card shows, PLUS a real fix for the not-found case (which previously really did just hand back a bare tab link).

- **`lib/deep-links.ts`** (new): `billingDeepLink(companyName, fyeMonth, fyeCycle)` and `lateFilingDeepLink(companyName)` — the one shared URL-format contract, used by both the server (assistant route, building a link for a not-found suggestion) and the client (the 3 preview cards), so the format can't drift between the two.
- **`app/billing/page.tsx`**: `CombinedPage` reads `openCompany`/`month`/`year` from the URL (and stops overwriting month/year from `/api/ar-reminder/latest` when they're present); `BillingTab` fuzzy-matches `openCompany` against `monthCompanies` (via the same `findUniqueBestMatch()` this codebase already uses everywhere for company matching — NOT by any numeric id, since the ids used inside this page are AR-Reminder-row-scoped and not stable/knowable from outside it) and calls the exact same `setExpanded()` a manual row click would. That modal already auto-detects new-draft vs. edit-existing-invoice on its own (`loadLiveLines` fires whenever a generated invoice exists for the cycle) — one deep link serves both `preview_invoice_draft` and `preview_invoice_edit`.
- **`app/late-filing/page.tsx`**: restructured (`LateFilingPageInner` + a `Suspense`-wrapped default export, required for `useSearchParams()`, same pattern as `app/billing/page.tsx`'s own `Page`/`CombinedPage` split) to fuzzy-match `openCompany` against `rows` and call `startEdit()` — the real inline edit dialog (remarks/dates, pre-filled), NOT the one-click `resolve()` PATCH itself, since that has no confirmation step on the real page and auto-triggering a real write from a mere navigation would be a different, unsafe mechanism duplicating the write path the chat's own modal already owns.
- **`lib/invoice-edit-lookup.ts`**: `InvoiceEditPreview` gained `fyeMonth`/`fyeCycle` (sourced from the same `CompanyBilling` row already being read) so its own deep link can load the right cycle, same as the invoice-draft preview's.
- **`app/my-tasks/page.tsx`**: all 3 cards (`InvoiceDraftCard`, `LateFilingResolveCard`, `InvoiceEditCard`) gained an "Open in ___ to review or adjust further →" link under their real action button.
- **`app/api/assistant/route.ts`**: the 3 tools' not-found responses now return each suggestion as `{name, link}` instead of a bare name, with an explicit instruction for Claude to render it as a real markdown link — the found:false path used to end in exactly the generic "just a link to the tab, stops there" experience Vincent was reacting to; now even a fuzzy near-miss becomes an actual working link to that specific company.

Verified: `billingDeepLink`/`lateFilingDeepLink`'s URL construction and the real `findUniqueBestMatch()` matching (exact name, and a realistic partial/fuzzy variant) both checked against real data via a throwaway diagnostic script — 16 real Late Filing rows and 792 real Billing rows, both exact (score 100) and partial (score 85, above the 70 threshold) matches succeeded. `npx tsc --noEmit` and `npm run build` both clean (the new `useSearchParams()` on Late Filing correctly stays a static-prerendered page thanks to the `Suspense` wrapper). **Not yet click-through tested in a real browser** — whether the Billing Drafts modal / Late Filing edit dialog actually pop open correctly on a real navigation is UI behavior this sandboxed environment can't exercise; Vincent needs to click the new links himself.

Previous update, same day: Assistant — phase 4, the last of the agentic-chat direction: Post Incorporate document generation via chat. Vincent explicitly rejected an earlier, more conservative framing — after I proposed downgrading this to a read-only "has this been generated" query tool (reasoning that director/shareholder identity data has no safe automatic source), he pushed back: "我比较极端 我希望是可以真正协助执行操作的，不只是停留在询问和回答阶段...你要思考用户真正要的是什么，你又可以帮助什么". The resolution: keep "never invent identity data" absolute (now `docs/INVARIANTS.md` INV-DATA-021), but make the tool a genuine multi-turn GUIDED INTAKE conversation instead of a one-shot Q&A — Claude collects company info, then each director, then each shareholder, a few real fields at a time, tracking progress itself across turns, and only calls the tool once it believes the picture is complete.

- **`lib/docx-post-incorporate.ts`**: new exported `PostIncorporatePreview` type (`{input, company, uen, directorsCount, shareholdersCount, needNdService}`) — carries the FULL assembled `PostIncorporateInput` back out so the frontend card can POST it to `/api/post-incorporate/generate` verbatim on confirm, same as the other 3 preview types each carry what their own submission needs.
- **`app/api/assistant/route.ts`**: new `preview_post_incorporate` tool with a large nested schema mirroring the real `PostIncorporateInput` shape (company + directors[] + shareholders[], including every nominee/nominator sub-field). Since Claude assembles this incrementally across turns, the payload arrives as loosely-typed JSON — a new `normalizePostIncorporateInput()` mechanically maps every field (verified field-by-field against the real type definitions, not re-derived from memory) to a safe default before validating, so a missing/mistyped field is reported by `validatePostIncorporateInput()` as "missing" instead of crashing the tool call on `.trim()` of `undefined`. `secretaryCompanyName` defaults to the same constant the real `/post-incorporate` page's own `emptyCompany()` uses (`TASSURE ASIA BIZSERVICES PTE LTD`) so the guided intake doesn't waste a turn asking for it. `claudeAnswer()` now captures/returns a 4th preview type, `postIncorporatePreview`. The static system prompt gained explicit guidance on HOW to run the guided intake (company → directors one at a time → shareholders one at a time, never invent identity data, re-send the full picture every call since the tool has no memory between turns). `messages.slice(-8)` widened to `-24` in `POST()` so a multi-turn intake doesn't lose earlier-collected details.
- **`app/my-tasks/page.tsx`**: new `PostIncorporateCard` + `PostIncorporateConfirmModal` — structurally different from the other 3 cards since the real endpoint (`POST /api/post-incorporate/generate`) returns a binary ZIP (`Content-Type: application/zip`), not JSON. Submission mirrors `app/post-incorporate/page.tsx`'s own `handleSubmit()` exactly: `res.blob()` → `URL.createObjectURL()` → synthetic `<a download>` click → `URL.revokeObjectURL()`, reading the real filename off the `Content-Disposition` header rather than inventing one.

Verified: `validatePostIncorporateInput()` (the real function `postIncorporatePreview()`'s completeness check depends on) exercised against 5 real cases via a throwaway diagnostic script — empty input (all 4 required-field errors), minimal valid input (zero errors), chairman name not matching any director, a UEN (corporate) shareholder missing `corporateDirectorNames`, and two fully-paid-up shareholders sharing a (case-insensitively) duplicate share certificate number — all 5 matched the real validator's own logic exactly. `npx tsc --noEmit` and `npm run build` both clean. **Not yet end-to-end tested against the real endpoint** (would actually write a `post_incorporate_operations` audit row and trigger a real ZIP download) — Vincent needs to try the real button himself, same reasoning as the other three phases' own un-exercised write paths.

Also this session, a smaller unrelated fix requested mid-turn: **Activity Insights moved into the Admin nav group, Vincent-only** ("这个放在 Admin 内的 2级标题...并且只有Vincent 可以看到"). It used to be its own level-1 sidebar item shown to 4 accounts (`ApprovedAccount.canViewActivityInsights`: Vincent/Cindy/Samuell/Yee Soon). Now nested as a level-2 item under `ADMIN_NODE` (alongside Appearance Settings / AI Learning), which only ever renders for `isAdmin` — so the nav link is Vincent-only automatically. The actual page guard (`app/activity-insights/page.tsx`) and API guard (`app/api/activity/insights/route.ts`) were both moved from `canViewActivityInsights` to `admin` to match — real server-side enforcement, not just a hidden nav link. `canViewActivityInsights` itself is left untouched on Cindy/Samuell/Yee Soon's accounts since it still gates a genuinely different, unrelated feature (the AI Learning candidates cross-staff view) that Vincent didn't ask to change. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: Assistant — phase 3 of the agentic-chat direction: editing an ALREADY-GENERATED QuickBooks invoice by natural-language request ("change the Secretary line to $700"). The hardest of the four phases — unlike a new draft, there's no algorithmic way to compute "what the edit should be"; the human states it, Claude turns that into a structured `changes` argument, and the tool applies it to a copy of the REAL current invoice for a before/after diff.

- **`lib/quickbooks-invoice-lines.ts`** (new): `getLiveInvoice(company, id)` — copied verbatim (not retyped) from `app/api/quickbooks/invoice-lines/route.ts`'s own GET handler. That route now just calls it, same behavior/error shapes preserved exactly.
- **`lib/invoice-edit-lookup.ts`** (new): `previewInvoiceEdit(companyQuery, qbCompanyHint?, changes)` — finds the company (same `computeAllCompanyBilling()` + matcher as invoicing), finds its CURRENT cycle's already-generated invoice via `CompanyBilling.generatedInvoices` (no new lookup needed, already computed), fetches its real live lines via `getLiveInvoice()`, then applies each requested change to a copy — matched by `service` first, `matchDescription` (a substring of the real description) as fallback, reporting anything that didn't match as `unmatchedChanges` rather than silently dropping it.
- **`app/api/assistant/route.ts`**: new `preview_invoice_edit` tool — Claude's job is only to turn the user's stated request into the structured `changes` array (never invent a numeric value the user didn't give); the actual matching/diffing happens server-side, never trusting Claude's own arithmetic. Same `isWithinRestriction` gate as invoicing. `claudeAnswer()` now captures/returns a 3rd preview type, `invoiceEditPreview`.
- **`app/my-tasks/page.tsx`**: new `InvoiceEditCard` (each changed line, unmatched requests flagged separately, total before→after) + `InvoiceEditConfirmModal`. Submission is the exact same `PATCH /api/quickbooks/update-invoice` `saveInvoiceEdit()` itself calls — same structural/sent/payment-or-void/SyncToken gates, all still enforced server-side, unchanged.

Verified what could be verified without a live QuickBooks connection from this sandboxed environment: company lookup, current-cycle-invoice matching, and both early-exit paths (nonexistent company; real company with no current-cycle invoice) all checked against real data. The live QuickBooks read itself (`getLiveInvoice`) could not be exercised from a diagnostic script here — Intuit's OAuth refresh call failed from this sandbox's network egress (TAB's access token happened to be expired at test time; the refresh token itself is valid until December, so this is a sandbox-reachability question, not a code bug) — but that function is a byte-for-byte copy of the already-working, already-in-daily-use route, not new logic. The diff-application algorithm was hand-traced against 3 real cases (exact match, description-fallback match, no-match) since it has no external dependency to block a script run. `npx tsc --noEmit` and `npm run build` both clean. Not end-to-end tested against the real PATCH (same reasoning as the other two phases) — Vincent needs to try it live.

Previous update, same day: Assistant — phase 2 of the agentic-chat direction: Late Filing "Mark Resolved" preview + real confirm, same shape as invoicing but a single PATCH instead of the QuickBooks reservation machinery.

- **`app/api/late-filing/route.ts`**: mechanically extracted `GET()`'s 253-line detection computation into a new exported `getLateFilingList(): Promise<LateRow[]>` (Python line-slicing again, verified with `git diff` that the computation itself is byte-identical, only the function wrapper changed) — also hoisted the local `type LateRow` to module scope (`export type`) since it's now needed outside the route file. `GET()` now just calls it and applies its own `fyeFilter`.
- **`lib/late-filing-lookup.ts`** (new): `previewLateFilingResolve(companyQuery)` — calls `getLateFilingList()`, narrows to one company via the same `normalize()`/`findUniqueBestMatch()` matcher used everywhere else, and reproduces `app/late-filing/page.tsx`'s own `resolve()` remarks logic verbatim (`"Review:" → "Resolved:"` swap, else `"Resolved: " + existing-or-"Reviewed and completed"`) — never a second, differently-worded copy of that rule.
- **`app/api/assistant/route.ts`**: new `preview_late_filing_resolve` tool, `claudeAnswer()` now also captures/returns `lateFilingPreview` alongside `invoicePreview`. No `isWithinRestriction` gate needed here (unlike invoicing) — `/late-filing` carries no page-level restriction the 6 AR-Reminder-only accounts don't already structurally lose by being unable to reach `/my-tasks` chat at all.
- **`app/my-tasks/page.tsx`**: new `LateFilingResolveCard` (current remarks → what it would become, an "ALREADY RESOLVED" badge, disabled state) + `LateFilingConfirmModal` (real popup) — same "AI never touches the write, only a real button does" principle as `InvoiceDraftCard`. Submission is the exact same `PATCH /api/late-filing` call `resolve()` itself makes (`{uen, company_name, remarks}`), including the same `logActivity('late_filing_resolve', ...)` call for consistency with the real page's own tracking.

Verified: `npx tsc --noEmit`/`npm run build` clean; `getLateFilingList()` and `previewLateFilingResolve()` both run against real data (16 real currently-overdue companies, correct remarks-transform output, fuzzy match and not-found-with-suggestions all checked). Not end-to-end tested against the real PATCH (same reasoning as invoicing — would actually change a real record) — Vincent needs to try the real button.

Previous update, same day: Security fix — 2 real missing-auth-check gaps, found while surveying candidates for the next agentic-chat feature (Vincent: "还有没有类似的其他东西需要这些功能的可以帮忙补全吗？", then confirmed doing all 4 phased: "可以把上面的4项分阶段进行吗？我觉得都需要" — this is phase 1). `POST /api/master-list/move` had NO server-side auth check at all — the only gate was a client-side `window.confirm()` in `components/MasterListTable.tsx`, trivially bypassed by hitting the URL directly. `POST`/`PATCH /api/late-filing` called `getRequestAccount()` but never checked its result for `null`, silently proceeding with `updated_by_email: null` for an unauthenticated request; `DELETE /api/late-filing` didn't call it at all. All four fixed to require a real approved account (401 otherwise), matching every sibling mutating route in this app (AR Reminder, Trademark, QuickBooks). Documented as `docs/INVARIANTS.md` INV-DATA-020 — `getRequestAccount()`'s mere presence in a route proves nothing, its result must actually be checked. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: Assistant — step 2 of agentic invoicing: a real "Generate Invoice" button + confirmation modal, wired to the exact same `/api/quickbooks/create-invoice` Billing Drafts itself uses. Vincent, on step 1's markdown-table preview: "这些内容很简陋，不能直接和用户确认后弹出真正的弹窗吗？...要预览实际页面预览，不是文字大纲" — wanted a real styled card and a genuine popup, not a text outline.

- `app/api/assistant/route.ts`: `claudeAnswer()` now returns `{ text, invoicePreview? }` instead of a plain string — captures the LAST successful `preview_invoice_draft` tool result across the tool-use loop as structured data (not something the frontend parses back out of Claude's prose). `POST()` includes `invoicePreview` in the JSON response. `invoiceDraftPreview()` now passes the FULL `InvoicePreview` object through to Claude (previously stripped to a lean subset) so the frontend has `companyId`/`email`/`pic`/each line's `productService` — everything `/api/quickbooks/create-invoice` needs.
- `lib/billing-lookup.ts`: `InvoicePreview` gained `companyId`/`email`/`pic` (sourced from `CompanyBilling.resolvedCompanyId`/`.email`/`.pic`, already computed, just not previously surfaced).
- `app/my-tasks/page.tsx`: new `InvoiceDraftCard` (a real styled table — service/amount/reason, TAB/TAC totals, warnings, an "ALREADY INVOICED" badge, a disabled state when there's nothing due) + `GenerateConfirmModal` (a real `position: fixed` popup — company/FYE/line-by-line summary, Cancel/Confirm buttons) rendered inline under any assistant chat message that carries `invoicePreview`. Clicking "Generate Invoice" opens the modal; only clicking "Confirm & Generate" there calls the real endpoint — the AI itself never triggers it. Submission reuses app/billing/page.tsx's own exact patterns: only `include`d lines submitted, split into `tabLines`/`tacLines` by service (ND → TAC), a fresh `crypto.randomUUID()` idempotency key per confirmation attempt (kept stable across an overlap-confirm retry of the same request), `docNumbers`/`expectedNextNumbers` deliberately omitted so the server always computes fresh instead of risking stale client-cached numbers, `sendEmail: false` as a safe default. Handles the same 409 `overlapConfirmationRequired` response Billing Drafts does (a second, explicit "confirm anyway" step showing the real warnings) and surfaces any other error plainly. On success, appends a real confirmation message into the chat conversation.

Verified: full `npx tsc --noEmit`/`npm run build` clean; the enriched `InvoicePreview` fields (companyId/email/pic/fyeCycle/each line's productService) and the tab/tac split logic checked against a real company's real data. **Deliberately NOT end-to-end tested against the real `/api/quickbooks/create-invoice` endpoint** — that would create an actual QuickBooks invoice for a real client, not something to fire from an automated diagnostic; Vincent needs to try the real button himself to complete verification.

Previous update, same day: Assistant — real bug, live Claude engine: replying about "Shi Ming" (Ang Shi Ming, a real staff member — names are stored/used system-wide exactly as romanized text, `lib/staff-directory.ts`), it fabricated a Chinese-character rendering ("石明") with no real source for it. Vincent: "这个会无端端翻译名字，名字是很严格的，不需要特地翻译成中文的译名，英文名字就好". Added an explicit instruction to `staticSystemPrompt()`: never translate a person's or company's name into Chinese characters, keep names exactly as given in the data, even mid-Chinese-reply — this is a fabrication risk, not a real translation, so it belongs in the shared system prompt (same tier as the existing recipient-CC rules) rather than a per-account memory. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: AI Learning — controlled auto-approval + unattended daily analysis. A parallel session's AI Learning feature (`ai_learning_candidates`, human-review-only by design, migration comment: "This does NOT let an AI silently define a person...A candidate remains inert until Vincent...approves it") surfaced its real status via that page's own chat: 1 conversation, 20 messages saved, 0 `user_memories`, 0 AI Learning candidates yet. Vincent: "其实我觉得你可以审核，不一定要我审核对话...我希望AI可以自主学习，最好是在我没有在线的时候，它也能不断的在跑，在分析在研究，运行" — a direct request to remove the human-approval gate entirely.

Flagged the real conflict before implementing rather than silently complying: this collides with the exact principle `docs/INVARIANTS.md` INV-DATA-017 already established ("AI 不应因为一次对话就永久定义用户", from Vincent's own shared blueprint) AND with the AI Learning feature's own very recent, deliberate tightening (its latest commit title: "restrict AI learning review to Vincent"). Presented 3 concrete options via AskUserQuestion; Vincent picked the middle path — auto-approve only very high-confidence candidates, keep human review for everything else, but make that review faster:

- **`lib/ai-learning/candidates.ts`**: `analyzeUserActivity()` now auto-approves a candidate (calling the existing `review_ai_learning_candidate` RPC with actor `system:ai-learning-auto`, distinguishable in the audit trail from a real human approval) only when BOTH `confidence >= 0.9` AND `distinct_days >= 5` — short of either, it stays in the human queue exactly as before. Never touches a candidate already finalized by a human (approved/rejected/dismissed) — the existing upsert logic already preserves that.
- **`app/api/ai-learning/analyze-all/route.ts`** (new) + **`vercel.json`** + **`proxy.ts`**: a new daily cron (23:30 UTC) runs `analyzeUserActivity()` for every approved account unattended — directly answering "在我没有在线的时候，它也能不断的在跑". Wrapped in `withAutomationRun` (extended `lib/automation-sync.ts`'s `AutomationSource` union with `ai_learning` — no migration needed, confirmed `automation_sync_runs.source` has no DB-level CHECK constraint) for overlap protection and a real audit trail, matching every other automation in this codebase. One account's failure never stops the others.
- **`app/ai-learning/page.tsx`**: new "Approve all ready (N)" bulk button — approves every remaining `ready_for_review` candidate in one click (each still gets its own real `ai_learning_feedback` audit row under Vincent's own email, not a blurred bulk action) instead of requiring one-by-one clicks. Updated the page's own description to state the real auto-approval bar plainly.

Verified the riskiest part directly against real data: inserted a synthetic high-confidence candidate, called the auto-approval path for real, confirmed the resulting `user_memories` row and `ai_learning_feedback` audit row were both correct (right content, right actor, right linkage) — then fully cleaned up (verified 0 rows left). `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: Assistant — step 1 of agentic invoicing: a READ-ONLY invoice-draft preview tool in the My Tasks chat. Vincent, after confirming the live Claude engine correctly refused to fabricate invoicing ("我无法帮你直接开单...开单涉及财务和法律责任，必须由有权限的真实用户操作"): "假设我真的要你执行，你能不能做到一步一步的引导，当遇到敏感的情况，就跳出弹窗要用户确认继续" — then "可以你一步一步做，我先看效果". This ships the FIRST step only (preview, no write path at all) — the real confirm-and-execute step (a genuine UI confirmation card wired to the existing `/api/quickbooks/create-invoice`) is deliberately separate, later work, not started.

Investigated the real Billing Drafts pre-fill/create-invoice flow first (background Explore agent + direct file reads) rather than guessing — key findings that shaped the design: (1) the pre-fill logic (`initialLines`) lived ONLY as a client-side `useMemo` inside `app/billing/page.tsx`'s `ExpandedBillingRow`, never in `lib/`, so nothing server-side could call it; (2) `/api/billing/renewals`'s `GET()` computed the real `CompanyBilling[]` data inline, unscoped to any one company; (3) `invoice_creation_reservations` (INV-QB-006) means a stale `pending`/`uncertain` reservation row blocks a DocNumber FOREVER until manually cleared — so a preview must never call `create-invoice` speculatively or generate a real idempotency key, full stop.

Built as a chain of real, verified refactors — never re-derived the pre-fill/lookup rules from scratch (the exact anti-pattern this repo's own `lib/company-name.ts` header and `CLAUDE.md` warn against):
- **`app/api/billing/renewals/route.ts`**: mechanically extracted the 441-line computation body of `GET()` into a new exported `computeAllCompanyBilling(withinDays)` — via a Python script slicing the file at exact line boundaries (verified with `git diff`: only the function wrapper changed, the computation itself shows as unchanged context, zero transcription risk in business-critical billing logic). Fixed one real type error the extraction surfaced (an inline `NextResponse.json(...)` early-return doesn't belong in a function that returns data — now `throw`s, caught by `GET()`, reproducing the exact same 500 response).
- **`lib/billing-draft.ts`** (new): `computeDraftLines()`/`computeDraftTotals()`, ported verbatim from `initialLines` (verified line-by-line against the live component before porting, including the exact real warning strings — "⚠ Confirm XBRL required this FY", "Active nominee per TeamWork · confirm annual fee (excl. deposit)", etc.). Deliberately does NOT touch `app/billing/page.tsx` itself — that stays exactly as it is, a live revenue-critical page, isolated from any risk this step could introduce; retrofitting it to import from here too is explicitly left as later, separate cleanup.
- **`lib/billing-lookup.ts`** (new): `previewInvoiceDraft(companyQuery, fyeYear?)` — calls `computeAllCompanyBilling()`, narrows to one company via `lib/company-name.ts`'s existing `normalize()`/`findUniqueBestMatch()` (the same matcher `app/billing/page.tsx` itself uses), runs `computeDraftLines()` on it.
- **`app/api/assistant/route.ts`**: new `preview_invoice_draft` tool, gated by the EXACT same `isWithinRestriction()` check the Billing Drafts *page* itself uses — the 6 AR-Reminder-restricted accounts can't open `/billing?tab=billing` directly (`proxy.ts`), and this tool must not become a silent chat-based bypass of that (verified: `isWithinRestriction('/billing?tab=ar', '/billing', {tab:'billing'})` → `false`). System prompt instructs Claude to always present this as a preview and NEVER claim to have created a real invoice.

Verified against real data at every layer: `computeAllCompanyBilling()` ran standalone (792 real companies, sane output) before anything was built on top of it; `previewInvoiceDraft()` against a real company (TRUSTLINER PTE. LTD. — correctly showed Secretary NOT due yet, AR $60 due, real descriptions/dates) plus a nonexistent name (empty suggestions, no crash) plus a fuzzy/lowercase name match. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: My Tasks — fixed a real misleading empty state. Vincent, testing his own account's Tasks tab after the chat engine was confirmed working: "但是Tasks 那边还是很像摆设，不知道是不是没有数据支撑". Checked against real data before assuming anything: across all 911 `ar_reminder` rows ever, Vincent's account has NEVER been PIC/ACC PIC/TAX PIC on a single one — not "currently 0", structurally zero, because he's the owner, not a caseworker. The page was showing "Nothing outstanding — you're all caught up", which falsely implies work existed and got finished. Added `everAssigned` (`lib/my-tasks-data.ts`'s `computeMyTasks()` — true if the account has EVER matched as PIC on any ar_reminder/Late Filing row, independent of the current overdue/dueSoon counts) and threaded it through: `/api/my-tasks`'s response, the empty-state copy in `app/my-tasks/page.tsx` (now explains plainly that this account isn't assigned as a caseworker on anything, with a link to the Activity tab / View As for a management account that wants to see what's actually happening), `generateMyTasksBrief()`'s zero-count branch (the "Today's Priority" banner — same fix), and `buildTaskDigest()` + the assistant's static system prompt (so the now-live Claude engine gives the same honest answer via chat, not just the on-screen page). Verified against real data: Vincent → `everAssigned: false, total: 0`; Lim Hoe Chyi → `everAssigned: true, total: 1` (a real, correctly-classified case). `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: Infra — `ANTHROPIC_API_KEY` set in Vercel production. After a real, grounded cost estimate (measured the actual `staticSystemPrompt()`/`CLAUDE_TOOLS` size from the real code — ~8100 chars / ~2000-2500 tokens — against real current Haiku 4.5 pricing looked up live, $1/$5 per million in/out, cached reads $0.10/million — landing at roughly $0.003/exchange, well under $15/month even under a generous "heavy usage" assumption) and a walkthrough of the automatic-degrade-on-failure behavior (confirmed by re-reading `POST /api/assistant`'s own try/catch: a failed Claude call — out of credit, network error, anything — falls back to `intentAnswer()` automatically, no user-visible error, no cost for the failed call, fully automatic recovery once credit is restored, zero blast radius outside this one feature), Vincent provided a real key: "ok 那接下来先实验一下免费的API效果". Added via the Vercel API as a `sensitive`-type env var (write-only, value never readable back even by me), Production target, then redeployed (env var changes need a fresh deployment on Vercel to take effect). **The key itself was never written to any file in this repo, any commit, or any doc — only ever passed directly in the Vercel API call itself.** This turns on real Claude reasoning for `app/api/assistant`'s tool-use engine and `lib/my-tasks-brief.ts`'s daily briefing for the first time since either shipped — not yet confirmed by an actual live chat exchange, see `docs/CURRENT_STATE.md`.

Also discovered, while trying to pull real historical chat-message volume for the cost estimate: `ai_messages`/`ai_conversations` currently have ZERO real rows, despite Vincent's own screenshots showing successful real chat exchanges earlier today. Isolated the DB write path directly (`createConversation`/`appendMessage` via `lib/ai-conversations.ts`, bypassing the HTTP layer) — confirmed it works correctly (a real row was created, read back, cleaned up). So the persistence gap is somewhere between the browser and that DB layer (`/api/ai/conversations` POST, or `getRequestAccount` resolution in a real deployed request, or something else not yet isolated) — NOT a broken table or a bad admin-client key. `sendChatMessage()`'s own try/catch around conversation creation silently swallows a failure there with zero visibility (by design, so a save failure never blocks getting an answer — but that same silence means this bug produced no error anywhere to notice). Flagged to Vincent, not yet investigated further or fixed — see Pending Improvements below.

Previous update, same day: Assistant — prompt-caching restructure. Vincent tried "根据chelsea 最近做的东西，你判断接下来应该会做什么" in the My Tasks chat and got the generic capability-list fallback — a genuinely different phrasing from "chelsea 最近做了什么" (which worked, via the newly-shipped recent_activity_summary path), and no amount of adding more regex patterns to `intentAnswer()` can ever generalize to arbitrary phrasing like this — that limitation is structural to a keyword-matching engine, not a bug. Vincent then shared his own research doc (`.claude/uploads/.../anthropicapibusinesscontext.md`) independently reaching the exact same conclusion already documented in this repo: "不是模型不够聪明，是业务知识没被结构化地喂进去" — his 3-layer architecture (system prompt = SOP/org knowledge, tool use = real-time data, RAG = history) is, point for point, what `claudeAnswer()` already is (system prompt with business context, `CLAUDE_TOOLS`, `listMemories()` as the RAG-lite layer) — it's just never running, because `ANTHROPIC_API_KEY` still isn't set. Restated this plainly again, and used the doc's own prompt-caching note as a real, actionable improvement: `systemPrompt()` split into `staticSystemPrompt()` (identical for every user — persona, system map, key workflows, tool-usage instructions) and `dynamicSystemPrompt()` (current user/location/memories, genuinely different per call) — the original single interpolated string interleaved dynamic content in the MIDDLE, which would have broken any cache-prefix match almost every call. `claudeAnswer()` now sends `system` as two content blocks, `cache_control: { type: 'ephemeral' }` on the static one only (which the Anthropic API caches together with the preceding `CLAUDE_TOOLS`, since tools+static-system form one stable prefix now reusable across different users/turns, not just repeated calls from the exact same person with the exact same memories). Also added an explicit line to the static prompt about answering an OPEN-ENDED "predict what she'll do next" style question by calling `recent_activity_summary` and reasoning over the result, not just reciting counts. This has NO effect until the key is set — still `intentAnswer()` running in production — but is real, correct, ready groundwork rather than a second thing to redo later. `npx tsc --noEmit` and `npm run build` both clean.

Production update, same day (`codex/ai-learning-foundation`, pushed to `main`; Vincent confirmed the SQL migration was run): added the first controlled AI-learning foundation, deliberately isolated from Claude Code's concurrent Assistant/My Tasks work. New `scripts/add-ai-learning-foundation.sql` defines server-only `ai_learning_candidates` and append-only `ai_learning_feedback`, with an atomic review function; no client-side database policy. New `lib/ai-learning/patterns.ts` deterministically detects only two evidence-backed observations from `user_activity_events` — repeated page sessions (30-minute duplicate collapse) and repeated successful named actions — with minimum observation/day thresholds, confidence, source counts, first/last timestamps and sampled source event ids. It deliberately says "observed repeatedly", never claims a page visit is a personal preference or company rule. New candidate API and `/ai-learning` review page expose governance only to Vincent's explicit `admin` account; non-admin API requests receive 403 and direct page navigation redirects home. The Vincent-only sidebar navigation now uses one final `Admin` level-1 group containing `Appearance Settings` and `AI Learning` as level-2 entries, with the group's open/closed state persisted like the existing operational groups. Only **explicitly approved** candidates connect to the existing `user_memories` context: one candidate maps idempotently to one `source='inferred'` memory, approval/rejection/reopen transitions atomically add/update/remove only that linked inferred memory, later evidence refreshes confidence/count without rewriting the human-approved wording, and an idempotent backfill closes any already-approved shadow-only rows during upgrade. Explicit memories remain unreachable by this path. Pattern regression script, targeted ESLint, `npx tsc --noEmit`, and `npm run build` passed for the original integration; production GET was verified after SQL with no browser-console errors.

Previous update, same day: My Tasks / Assistant — real "what has this person actually done" activity, closing a gap Vincent caught live: on the View-As-Chelsea screen shipped earlier the same day, showing 0 tasks, he pushed back hard: "Chelsea 已经使用这个系统蛮长时间了的，结果你还是只判断到 AR 和 LATE，而没有真正了解到这个系统在干嘛，我们的员工在做什么". Before building anything, queried real Supabase data directly (not guessed) and found the real gap: Chelsea has 89 generated invoices, 41 real AR Reminder edits, 95 email campaigns — none of it visible to My Tasks because none of it is AR-Reminder-PIC-shaped (Billing Drafts and Email Campaigns have no "assigned to" concept, only an audit trail of who did it). Cross-checked 4 more real staff (HC/ASM/KY/SX named directly by Vincent) and confirmed a genuinely different shape per role — HC/ASM/KY are mostly AR Reminder + **Master List** edits (a source not initially considered), not invoicing at all — confirming the fix had to be generic across every real "who did this" column in the system, not tailored to Chelsea's own pattern.

Also answered Vincent's direct question — "这些都要依靠大数据分析，或者你觉得需要ANTHROPIC api，你才能调用大数据分析？" — precisely: pulling and aggregating this real structured data needs NO API key at all (plain SQL, same as Activity Insights' own top-pages/actions); only the IN-APP chat assistant's ability to turn it into interpreted narrative benefits from a real key (Claude Code itself, in a conversation like this one, is not affected by that gap either way).

Asked which shape to build first via AskUserQuestion (recent-activity display vs. a real "still-outstanding" queue needing new business rules vs. both) — Vincent: "两个都要，先做展示版" (want both, display version first). Shipped the display version:

- **`lib/recent-activity.ts`** (new) — `getRecentActivity(email, limit)` merges real audit-trail columns across `generated_invoices.created_by_email` (invoicing), `ar_reminder.updated_by_email` (real edits, naturally excludes `system:*` sync markers since those aren't real emails), `email_campaigns.created_by_email`, `master_list.updated_by_email`, `email_drafts.sent_by_email` (only rows with a real `sent_at`), `post_incorporate_operations.created_by_email`, `trademark_records.updated_by_email`, `soa_owners.updated_by_email` — sorted, merged, capped. `summarizeByKind()` for a compact per-category count digest.
- **`/api/my-tasks`** — now also returns `recentActivity` (same account/`viewAs` resolution already in that route, same graceful-degrade-never-break-the-page pattern as `brief`).
- **`app/my-tasks/page.tsx`** — new "Activity" sidebar tab (3rd `ActiveView` alongside chat/tasks) showing the real timeline; the existing viewingAs banner now has a 3rd wording for this tab. Verified the merge/sort logic against real Chelsea/HC data before wiring it in (25-item cap correctly reflects the true most-recent mix across sources, e.g. Chelsea's top-25 skews toward campaigns/sent-emails simply because those are chronologically more recent than most of her invoices, not because invoicing stopped).
- **`/api/assistant`** — new `recent_activity_summary` tool + matching `intentAnswer()` regex branch ("最近做了什么" / "recent activity" / "what has X been doing"), reusing the SAME cross-person `person`/`findMentionedAccount` + `canViewAsOthers` pattern shipped earlier the same day for `my_tasks_summary` — explicit refusal for a non-privileged caller asking about someone else, explicit honest "no data" (not "broken") for someone whose real work is outside these specific features.

`npx tsc --noEmit` and `npm run build` both clean. Verified `getRecentActivity()` against live Supabase data (Chelsea, Lim Hoe Chyi, a nonexistent email) via a throwaway diagnostic script, deleted after use — not yet exercised by a real browser login click-through (same standing caveat as the View-As-for-chat work earlier today).

Previous update, same day: management can now ask the My Tasks chat about ANOTHER staff member, AND fully "View As" them for chat, not just Tasks. Two complementary asks in one session, both gated on the existing `canViewAsOthers` flag (not a new permission):

(1) **Natural-language cross-person query.** Vincent, from a screenshot of "如果我是HC，我要做什么今天？" returning the generic fallback: "我是最大的ADMIN , 和管理层就可以问这些问题，其他人一般只能问自己相关的东西" — then, re-confirming the exact shape wanted: "最好就是可以智能到可以通过简单的询问，就能获取到准确的回答：类似这样的：如果我是HC，我要做什么今天？". Added `findMentionedAccount()` (`lib/approved-accounts.ts`) — scans free text for a staff name/alias/initials, matched against `lib/staff-directory.ts`'s existing aliases (case-sensitive whole-word for short codes like "HC"/"JF" so they don't fire on ordinary lowercase text, case-insensitive for full names), restricted to people who actually have a login (`APPROVED_ACCOUNTS`, the same universe the View As picker already offers). `myTasksSummary()`'s tool (Claude engine) gained an optional `person` argument; `intentAnswer()`'s my-tasks regex branch (the fallback engine — still the ONLY one actually running in production, `ANTHROPIC_API_KEY` remains unset) gained the same resolution directly. Both: caller asking about themselves → unchanged; caller without `canViewAsOthers` asking about someone else → explicit refusal, never silent self-fallback and never someone else's real data; privileged caller → target's real task digest, tagged whose it is. Also widened the my-tasks regex again — "如果我是HC，我要做什么今天？" itself didn't match ANY existing branch (only "我该做什么" was recognized, not "我要做什么" — a second real gap on top of the missing cross-person support), and added an explicit "如果我是X / if I were X" hypothesis-but-unresolved path so an unrecognized name gets an honest "don't know who that is" instead of silently answering about the caller. A standalone diagnostic (`scripts/diag-cross-person.ts`, deleted after use) against 12 real cases — including Vincent's exact screenshotted phrase — caught a genuine gap this same fix introduced the tooling to find: neither "Vincent" nor "Cindy" (bare first name — how people actually refer to them) resolved at all, only their full names, unlike Samuell/Esther/Chelsea who already had first-name aliases; added both to `lib/staff-directory.ts`.

(2) **Full "View As" identity substitution for chat**, extending what was previously Tasks-tab-only. Mid-build, Vincent corrected the scope twice: first "不只是还原，而且我作为最大的ADMIN 甚至是要可以带入到那个员工的身份，去开一个NEW CHAT 在她的记录...通过View as" (not just read-only reproduction — able to fully step into that employee's identity and start a new chat that becomes part of HER real record) — overriding an initial read-only design already in progress before it shipped. New shared `resolveViewAsAccount()` (`lib/approved-accounts.ts`): given the real caller + an optional `viewAs` email, returns either the caller unchanged or — if `canViewAsOthers` and the target is a real approved account — the TARGET, for the caller to operate genuinely AS. Wired into `/api/ai/conversations` (GET list / POST create — now `?viewAs=`-aware, a POST under `?viewAs=` creates the conversation OWNED by the target), the `[id]` PATCH/DELETE and `[id]/messages` GET routes (ownership check relaxed to "literal owner OR `canViewAsOthers`" — no `viewAs` param needed there since the target conversation is already identified by id), and `/api/assistant`'s POST (new `viewAs` body field — when set, EVERYTHING in that request, tool calls included, resolves as the target, so asking "what should I prioritize today" while viewing-as HC correctly answers about her own tasks with no special phrasing needed). `app/my-tasks/page.tsx`: the existing View As `<select>` now drives both Tasks AND the chat sidebar/thread/composer identically — switching identity clears whatever thread was open (never leaves a stale thread visible under the wrong identity), and a persistent amber banner states plainly whose data is showing / whose account a new chat or message will be saved to. Mutations (pin/rename/delete) intentionally use no separate `viewAs` wiring on the frontend — the relaxed server-side ownership check already covers a privileged caller regardless.

Vincent also named the actual long-term direction this is building toward — real agentic action-taking, not just Q&A: "我后续要做的是除了回答问题，甚至是可以协助操作，比如我说要开A 公司的TAB INVOICE ，你就要问是不是 2026年 这个FYE month 的，然后问我是不是要开单，然后你和我确定好信息和内容后，就可以自动开单到QB...这个是我要做的大方向，目前你先把内容都完善" — explicitly a FUTURE direction, not built now (matches the shared blueprint's own v1 scoping: Read+Recommend+Draft only, no auto-actions yet — an agentic QB-invoice-creation flow is real money/billing automation and needs its own careful design pass when Vincent is ready to actually start it, not a side effect of this change). Noted in Pending Improvements below and in cross-session memory so it isn't lost.

`npx tsc --noEmit` clean. Verified via `scripts/diag-cross-person.ts` (12/12 pass after the alias fix) and a standalone regex check against the real screenshotted phrases — both deleted after use. **Not yet verified**: an actual live login click-through of the View-As-for-chat flow (select a target → New Chat → send → confirm the row really lands under the target's email in `ai_conversations`/`ai_messages`) — same category of gap as Company 360/My Tasks' own original ship (code-complete, `tsc`/logic-verified, not yet exercised by a real browser session).

Previous update, same day: 3 real My Tasks empty-state suggestion buttons fixed, PLUS a much bigger finding: **`ANTHROPIC_API_KEY` has never been set in Vercel production** (confirmed directly via the Vercel API's env list — the key is simply absent), meaning every "AI assistant" reply in production — the floating widget AND the new My Tasks chat — has always run `intentAnswer()`'s plain keyword-matching fallback, never real Claude reasoning/tool-use. Vincent, from a screenshot of "What should I prioritize today?" returning the generic capability list: "现在的回答还是很基础的AI模型，都是固定嵌套式的回答，没有真正回答到问题" — accurate: that IS what the no-key fallback is, by design.

Fixed the concrete, immediately-actionable bug: none of the 3 real suggestion buttons ("What should I prioritize today?", "Any overdue AR?", "How does this work?") matched `intentAnswer()`'s own regexes correctly — 2 fell through to the generic fallback (missing "prioritize" as a verb; the English "how does this work" phrasing wasn't recognized at all, that branch was Chinese-only), 1 was silently stolen by the unrelated company-wide due-soon branch (its `/(到期|due|...)/ ` pattern matches the "due" substring inside "over**due**"). Confirmed the exact failure AND the fix with a standalone regex-only diagnostic (no DB needed) before and after, plus a regression check against 3 of the router's own pre-existing example phrases to confirm the widened patterns didn't start swallowing something else's intent.

**The deeper fix — actually configuring `ANTHROPIC_API_KEY` in Vercel — is not something I can do myself** (needs a real Anthropic API key, which only Vincent can provide/create). Until that's set, every AI feature shipped today (`my_tasks_summary`, `my_activity_pattern`, `remember_this`, the daily briefing's natural-language phrasing, real free-form conversation) keeps degrading to its own rule-based/keyword fallback — which all work, by design, but are exactly the "固定嵌套式" experience he's reacting to. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: My Tasks chat — corrected a misread: "下方长度加7%" was applied as a 7% width increase on the input/suggestions row; Vincent: "看起来还是一样也，我说的是上下高度" (I meant the vertical height). Reverted the width change (`maxWidth` back to 560 on both rows) and instead scaled the actual panel height calc up 7% — `calc(85vh - 145px)` → `calc(90.95vh - 155px)`, `minHeight` 408 → 437 — same "scale both terms proportionally" approach as the original 15% reduction earlier the same day. Net effect vs. the very first version is ~91% (0.85×1.07), i.e. a ~9% reduction overall, not the original full 15%. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: My Tasks chat empty state — widened the input row and suggestion-buttons row by 7%. Vincent: "下方长度加7%" — `maxWidth` 560 → 599 (560 × 1.07 = 599.2, rounded), applied to both rows since they've shared the same max-width since the empty state was built. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: My Tasks chat — empty-state copy simplified to match ChatGPT's own minimal style. Vincent shared his own copy-editing pass (comparing against a real ChatGPT "准备好了，随时开始" screenshot): "这个内容...可以简单一点，放成英文的（准备好了，随时开始）就可以了...这个不需要：可以按你自己的登录账号，判断今天应该优先处理什么，回答关于你自己名下 AR / Late Filing 任务的问题" — then compared two versions and preferred the more ChatGPT-like one ("如果你希望整体更像 ChatGPT 的 AI 助手界面...第二版会更简洁、自然，也没有那么强的'系统说明感'"). Dropped the descriptive Chinese paragraph entirely; title/subtitle now just "My Tasks" / "Ready when you are."; the 3 suggestion buttons shortened to "What should I prioritize today?" / "Any overdue AR?" / "How does this work?". This empty state no longer pulls from `AssistantWidget.tsx`'s shared `getPageGuide()` — that guide's fuller Chinese explanatory copy is still correct for the floating widget (matches every other page's own guide there), just not for this page's own new, deliberately minimal empty state — so the two were deliberately decoupled rather than shortening the shared copy and affecting the widget everywhere else too. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: My Tasks chat — 2 real UI fixes from screenshots, plus both new migrations (`user_activity_events`, `user_memories`) confirmed live via a real write+read+cleanup round trip on each (not just "table exists"). (1) Vincent, from a screenshot of the chat panel running past the viewport: "上下尺寸稍微短一点，现在有点超过了，可以减少15%的高度" — scaled BOTH terms of the `calc(100vh - 170px)` height down by 15% (`calc(85vh - 145px)`, `minHeight` 480→408) rather than just bumping the fixed offset, so the reduction holds proportionally across different screen heights, not just tuned to his one screenshot. (2) Vincent, from a real ChatGPT screenshot: "你要稍微学习调整一下，在还没有开始问问题前，输入框是在中间的" — the empty/new-chat state now matches ChatGPT's own layout exactly: headline, a vertically-centered input, suggestions below it; only once a real conversation exists does the input move to a bottom-pinned bar with messages scrolling above (previously the input was always bottom-pinned, even on a blank new chat). `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: My Tasks — redesigned around a persistent ChatGPT/Claude-style chat interface, plus a `user_memories` schema from Vincent's shared AI-assistant blueprint. Vincent, on the static-table version shipped earlier the same day: "My Tasks 不要这样的，看起来好像有用，实际没有用处，只是摆设" (looks useful but is actually useless, mere decoration) — then, resuming an ask from before that: "My Tasks 这个页面是好像AI聊天这样的界面，好像 Chatgpt/ Claude 这样的，可以New Chat, 记录Chats and tasks, 可以 Pin/ Pinned", sharing 2 full documents (an AI-native personal work assistant blueprint, Chinese + English versions) describing a much larger architecture. Read both completely before building (his own explicit ask: "你慢慢读，认真读，认真分析"). The blueprint's own §16/§20 (MVP scope, "下一步建议") directly answered how much of its 10-table architecture to build now: `ai_conversations`/`ai_messages` (needed for the chat), `user_memories` (its own text says build the memory schema early "避免未来重构" even before the learning engine exists) — deferred the other 7 tables (`entity_memories`/`user_patterns`/`task_priority_scores`/`ai_recommendations`/`ai_feedback`/`ai_action_audit`), all genuinely Phase 2-4 in the blueprint's own roadmap with no consumer today; `user_events` was already covered by `user_activity_events` (shipped earlier the same day).

Asked 2 clarifying questions before building (matching the pattern that worked well earlier today): (1) does chat replace the task tables or do both coexist — answer: coexist, chat primary, tasks in a secondary sidebar tab; (2) build just the chat UI shell today, or also start the blueprint's foundational tables — answer: both.

Shipped:
- New tables `ai_conversations`/`ai_messages` (`scripts/add-ai-conversations.sql`, **not yet run**) and `user_memories` (`scripts/add-user-memories.sql`, **not yet run**) — table names for the first two match the blueprint's own "Suggested Database" section rather than inventing different names for the same concept.
- `lib/ai-conversations.ts` (thread/message CRUD, ownership-scoped) + `app/api/ai/conversations/*` (list/create/rename/pin/delete a thread, list its messages) — every route checks the real session owns the thread before touching it.
- `/api/assistant` extended: accepts an optional `conversationId` and persists both sides of the exchange after a successful reply (auto-titles from the first message, ChatGPT-style, never overwriting a later rename); gained a `remember_this` tool that ONLY fires on an explicit user ask ("记住...") — deliberately not auto-inferred from behavior data or conversation tone, per the blueprint's own explicit warning against exactly that shortcut (now `docs/INVARIANTS.md` INV-DATA-017); the system prompt now injects the user's own recent memories as context (the blueprint's "Context Package" concept) and is `async` to fetch them. Hardened the Claude tool-use loop so one tool's own failure (e.g. `remember_this` before its table exists) can't silently swap the whole reply to the plain fallback engine mid-conversation.
- `components/assistant/ChatRichText.tsx` (new) — `RichText`/`Inline` extracted out of `AssistantWidget.tsx` so the new full-page chat and the floating widget render assistant replies identically, not two copies that could drift; `AssistantWidget.tsx` also now exports `getPageGuide()` so the new chat's welcome/suggestions for `/my-tasks` reuse the exact same copy already written for the widget.
- `app/my-tasks/page.tsx` fully redesigned: a left sidebar (New Chat — lazy, matching ChatGPT's own behavior of not saving an empty thread until the first real message — then Pinned/Recent sections with hover-revealed pin/delete, then a "Tasks" entry with a live badge count) and a main panel switching between the chat (now default) and the original task tables/banner (unchanged content, just relocated). "View As" (task-data preview for admins) is explicitly scoped to the Tasks tab only — chat is always the real logged-in account's own conversations, never viewable as someone else (a real privacy line, not just a missing feature).

Verified before shipping: confirmed directly (not assumed) that every new read degrades to an honest empty result and every new write throws a clear, already-caught error before either migration has been run — chatting itself works immediately (just unsaved) even with zero DB setup done yet. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: Activity Insights — real behavioral-analytics tracking, a genuinely new layer, not an extension of the My Tasks v1 shipped earlier the same day. Vincent, correcting that earlier ship as too narrow: "你目前涉及到的东西还是太小了，我关注的不只是 AR，而是更深层更自定义/习惯/任务路径/操作顺序/频率/复杂度的 的东西，比如你是否可以查找到：现在每个用户进入系统后的点击操作路径...并且不只是AR / Late Filing 的情况，还有其他的行动分析，比如为什么这个用户每天会打开这个页面，为什么会时常在这个页面操作，为什么每次关注某些特定的更新". Investigated honestly first: this app had ZERO per-user navigation/click history anywhere — every table logs business state or single-action audit trails, never "who visited what page when." Asked 2 clarifying questions before building (who sees the results; how much detail to record) — answers: (1) staff see their own patterns, Vincent/management can pull any individual's or the whole company's data; (2) page visits + key actions.

Shipped: new `user_activity_events` table (`scripts/add-user-activity-events.sql`, **not yet run — see Active Issues in `docs/CURRENT_STATE.md`**) — starts empty at deploy, never backfillable (no prior system ever recorded this, unlike every other table in this app; see `docs/INVARIANTS.md` INV-DATA-016). `lib/activity-data.ts` is the one read/write layer: `logActivityEvent()` (write), `getPersonActivitySummary()`/`getCompanyActivitySummary()` (read, both from the exact same rows so individual and company-wide views can't drift). `POST /api/activity/log` (fire-and-forget sink, swallows every failure — a tracking call must never break the real action it's attached to) is called from `components/AppShell.tsx` on every route change (page_view, covers the whole app automatically, no per-page instrumentation needed) plus 4 hand-picked key actions via new `lib/activity-client.ts`'s `logActivity()`: Billing Drafts' Generate Invoice, AR Reminder's Delete, Late Filing's Resolve, Client Communications' Create Outlook Drafts (more can be added incrementally — each is a 1-line call once this exists). New `GET /api/activity/insights` + `/activity-insights` page (gated on a new `ApprovedAccount.canViewActivityInsights` flag — same 4 people as `canViewReports`/`canViewAsOthers` today, kept as its own flag per this file's own established "don't couple unrelated permissions" principle) shows top pages/actions/hour-of-day distribution per person or company-wide, with an honest "no data yet" state rather than fabricating a pattern. The Assistant gained a `my_activity_pattern` tool + intent-router branch (self-only, no permission gate — same reasoning as `my_tasks_summary`) so anyone can ask about their OWN habits conversationally.

Verified before shipping: confirmed directly (not assumed) that the write path doesn't throw and the read path returns an honest empty summary even before the SQL migration exists — the exact "no data yet" UI path was exercised for real, not just written and hoped to work. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: My Tasks — v1 of the AI-assisted task feature discussed earlier this session (see 2026-09-07's shelved "AI 员工习惯记忆" entry). Vincent: "继续之前提到的AI...最好可以应用在 MY TASKS内，更智能的分析和判断用户要做什么，可以沟通，可以对话，或者是每天打开My Tasks 的时候 AI助手会提醒今天可能会需要完成的任务，优先级是现在，具体的UI，我们会在后面不断的优化形式" — priority now on the logic, UI to keep iterating later.

Two pieces, both reusing existing infrastructure rather than building parallel systems:
1. **Daily briefing banner** — new `lib/my-tasks-data.ts`'s `computeMyTasks()` (the per-account AR/Late-Filing computation, extracted out of `app/api/my-tasks/route.ts` itself so nothing about "what's mine" is computed twice) feeds new `lib/my-tasks-brief.ts`'s `generateMyTasksBrief()`, which writes a short "Today's Priority" sentence — Claude-phrased (haiku, a compact digest of the worst-overdue/due-soonest items, never the full row set) when `ANTHROPIC_API_KEY` is set, otherwise a rule-based sentence built directly from the same counts (matching `app/api/assistant/route.ts`'s own established dual-engine philosophy) — and never throws, so a briefing failure can't break My Tasks loading. `/api/my-tasks` now returns this as `brief`; `app/my-tasks/page.tsx` renders it as a new banner, first thing under the header, before the metric cards.
2. **Conversational** — the existing general-purpose Assistant (`app/api/assistant`, previously fully anonymous) now resolves the real caller via `getRequestAccount(req)` and gained a `my_tasks_summary` tool (Claude engine) plus an intent-router branch (no-key engine) so asking "我今天要优先处理什么" anywhere in the app answers with THAT PERSON's own real overdue/due-soon items — reusing the exact same `computeMyTasks()`/digest the banner uses, so chat and the on-screen list can never disagree. A caller the route can't identify gets an explicit "can't tell who you are," never someone else's data or a guess. Added a `/my-tasks` page guide to `AssistantWidget.tsx` and a `/my-tasks` case to the assistant's own `currentPageHelp()` (both previously fell through to the generic default).

Verified against real data before shipping: Vincent's own account (0 tasks) correctly returns "all caught up"; a real AR-only restricted account (Jay Tay) correctly surfaces their one genuine overdue item by real company name and real day count via the rule-based path (no `ANTHROPIC_API_KEY` configured in this environment — confirms the feature works end-to-end with zero API dependency, exactly as designed). `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: Company 360 — collapsed sections now show a lighter gray title bar, back to dark navy once opened. Vincent: "收起的时候颜色放成灰色浅一点，打开才变回深蓝色". `DataCard.tsx` now computes `barBg`/`barColor` from `open` state, reusing this app's own existing "gray table header" pair (`--list-column-header-bg`/`-text`, already used everywhere by `.list-column-header-gray`) for the collapsed look rather than inventing a new gray, and the title bar's normal navy (`--list-header`) for the open look. The icon each section passes in (e.g. `<Receipt color="#fff" />`) is hardcoded white by all 8 call sites — rather than touching every one of them, `DataCard` re-colors it in place via `cloneElement` so it doesn't go invisible against the light collapsed background. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: Company 360 — every section (module 2 onward) is now collapsible, collapsed by default. Vincent: "这些部分设置成每个板块都是可以收起的，并且默认收起，用户需要的时候自己打开" then "除了第1板块不需要收起" (except module 1, the header card, doesn't need this). New `app/companies/[id]/DataCard.tsx` — `DataCard` (the shared shell every section — Communications/Outstanding/AR-AGM/Officials/Shareholders/Invoices/ND/Trademark — renders through) pulled out into its own `'use client'` file, same convention this route folder already used for its other small interactive pieces (`CopyUenButton.tsx`, `CustomerSourceField.tsx`) — keeps every actual section component (`ArAgmSection`, `InvoicesSection`, etc.) a plain server component, matching `_components.tsx`'s own original "every one of these is read-only, so all stay server components" reasoning; only the shell that now needs open/closed state crosses that boundary, and a server component's already-rendered JSX still works fine as its `children`. Title bar is now a click target (`role="button"`, `aria-expanded`) toggling a chevron (▸ closed / ▾ open, matching the same chevron convention the on-screen Outstanding list's own row-expand already uses) — the count badge stays visible either way, so someone can judge "is this worth opening" without opening it. `_components.tsx` re-exports `DataCard` from the new file so nothing importing it needs to change. The header card (`page.tsx`, module 1) is untouched — it was never rendered through `DataCard` to begin with, so it's naturally excluded. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: Company 360 — spacing between every section (module) increased 35%. Vincent: "每个板块之间的间距拉大35%". `app/companies/[id]/_components.tsx`'s `DataCard` (every module after the header card — Communications/Outstanding/AR-AGM/Officials/Shareholders/Invoices/ND/Trademark) `marginBottom` 16px → 21.6px; `app/companies/[id]/page.tsx`'s header card `marginBottom` 20px → 27px (both exactly ×1.35, kept as the precise value rather than rounded, since he gave an exact percentage). The unrelated `MatchQualityNote` warning banner's own `marginBottom: 16` left untouched — that's spacing around a conditional warning, not between two "板块"/section modules. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: Company 360's Outstanding section — "Invoice" renamed to "Invoice No." and de-styled to match the Invoices section's own Invoice No. cell exactly. Vincent: "Invoice 换成 Invoice No. , 格式要参考 Invoice No. 列的字体格式". Same page's own Invoices section again — its Invoice No. cell is bare inherited text with no "#" prefix, so this drops both the custom `fontSize`/`lineHeight` override and the "#" this column used to add on its own initiative. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: Company 360's Outstanding section — 3 columns re-styled to match sibling sections already on the SAME page, per Vincent's exact 3-part ask: "Source 换成 Company （和截图那边一样，包括字体和格式大小）/ Total Balance 列的格式要参考 Amount 列的字体格式 / Due Date 列的格式要参考 Appointed 列的字体格式". His "Company"/"Amount"/"Appointed" screenshots turned out to be this exact Company 360 page's own Invoices and Officials/ND sections, scrolled to elsewhere — not the on-screen Outstanding pages. Matched by literally copying each referenced cell's real styling rather than guessing a color from the screenshot: (1) the "Source" badge (pill background, bold, 3-letter code) renamed to "Company" and de-styled to bare inherited text, exactly matching Invoices' own `<div>{r.qb_company}</div>` — that section already calls this same TAB/TAC/TAO value "Company", so this is also a terminology fix, not just a style one; (2) Total Balance switched from a plain unprefixed number (`Arial`, no `$`) to Invoices' own Amount format (`$`-prefixed, `.toFixed(2)`, `tabular-nums`, no font-family override); (3) Due Date's per-invoice stacked dates dropped their custom `fontSize`/`lineHeight` override, now bare text matching Officials/ND's own Appointed cell (still stacks one line per unpaid invoice, since Appointed itself never needs to — that's a structural necessity, not a styling deviation). `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: Company 360's Outstanding section — Aging badge recolored yellow. Vincent, from a screenshot of the 31-60/61-90 badges: "放成黄色显示". Changed from the flat gray/navy shared with the Source badge to a yellow pairing (`#fef9c3`/`#854d0e`) — Source stays gray, since Aging is the one column that's actually a severity signal and should read as one at a glance. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: Company 360's Outstanding section — iterated from 5 to its final 6 columns in one rapid round, all confirmed scoped to Company 360 only ("我刚才说的全部是针对 Company 360" — the on-screen Outstanding pages themselves untouched). Vincent's sequence: "多一列，变成6列...Aging (帮我想一下英文的表头，主要显示是最久的是欠了多久时间，比如最久的是 91+，就放91+)" → "Company Name列 换成Invoice 列" → "Total 换成 Total Balance" → "把PIC 换成这个 Due Date". Final columns: **Invoice** (this company's own real unpaid invoice numbers, e.g. "#02610894" — was Company Name, redundant on a page that's already scoped to one company) / **Source** / **Aging** (the single most-overdue bucket this balance still touches — new `oldestAgingBucket()` in `lib/soa.ts`, scans `AGING_BUCKETS` from 91+ down to Current and returns the first non-zero one, so a company with some Current and some 91+ invoices shows 91+, matching his framing that the oldest one wins) / **Total Balance** (renamed from "Total") / **Due Date** (was PIC) / **Owner**. `lib/soa-data.ts`'s `SoaCompanyRow` gained `unpaidInvoices: {invoiceNo, dueDate}[]` (sorted oldest-due-first, computed via `lib/soa.ts`'s existing `dueDate()`) — collected during the SAME aggregation pass every other field on this row already goes through, not a second query, so Invoice and Due Date can never drift from the row's own real total/aging; every existing consumer of `SoaCompanyRow` (the on-screen Outstanding/All pages, both Excel exports) ignores the new field, unaffected. Verified against real data before shipping: "1V CAPITAL PTE. LTD." TAB row's real invoice number (`#02610894`, due 2026-07-31) matches exactly what Vincent's own screenshot showed. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: Company 360 — new "Outstanding" section, module 3 (right after Communications, pushing AR/AGM Cycles to module 4). Vincent: "在第3模块加上 Outstanding 板块，目的是为了让用户可以点击进来这个company 360后，立刻可以看到这家公司到底目前在欠着哪家公司的欠款（TAB/TAO/TAC）主要的负责人是谁，谁要去催款，这个单到底欠了多少天 / Outstanding 板块 5列 (这些都同步从 Outstanding 来的): Company Name / Source / Total / PIC / Owner". Reused `lib/soa-data.ts`'s `computeSoaRows()` — the exact same computation the on-screen Outstanding pages use — rather than a second copy, so this section can never silently show a different number/owner than those pages do for the same company. `computeSoaRows()` gained an optional `customerNamePrefilter` param (ilike-narrows the initial unpaid-invoices query) so Company 360 isn't scanning every unpaid invoice across every customer just to show one company's own balance; omitted (every existing caller — the on-screen Outstanding/All pages, both exports), it behaves byte-for-byte as before. `lib/company-360.ts`: added the 3 (TAB/TAC/TAO) `computeSoaRows()` calls to the existing 14-query parallel batch, each `.catch(() => [])`-guarded so a transient failure there degrades gracefully like every other section on this page rather than crashing the whole page (computeSoaRows is the only fetch in this file that can actually throw); rows matched to this company by `companyId` (computeSoaRows' own fuzzy company-name match) with a normalize()'d-name fallback for the rarer unmatched row. New `OutstandingSection` (`app/companies/[id]/_components.tsx`) styled to match the on-screen Outstanding "All" list's own final look (flat gray Source badge, plain regular-weight numbers) since "同步" means matching its display too, not just its numbers — a company owing on 2+ systems shows as 2+ rows here too, same as Outstanding's own "All" view, never merged. Verified against real data before shipping: "1V CAPITAL PTE. LTD." (Vincent's own recurring example) resolves to exactly 2 rows (TAB $1,000, TAO $2,650) matching the live Outstanding page exactly, and a company known to have zero unpaid invoices correctly resolves to 0. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, 2026-09-07: SOA list — reverted the Company Name/Owner column-ratio redistribution from the previous update. Vincent: "我没有叫你改啊，我只是问而已，上一版的比例好" (I wasn't asking you to change it, I was just asking — the previous ratio was fine). His "比例是多少?"/"列宽比例" question had been read as an implicit request to fix the still-visible Company Name/Source gap; it was purely informational. `soaListColumns` back to a single flexible share on Company Name alone (`1.2fr`/`1.4fr`), Owner a fixed `150px` again — exactly what shipped 2 updates ago, before the redistribution. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA list — fixed the Company Name/Source gap for real this time, by splitting the flexible width across 2 columns instead of 1. Vincent, from another screenshot showing the same gap even after the previous revert: "比例是多少?" / "列宽比例" (what's the ratio — asking about the CSS grid fr split). Root cause of the gap coming right back: reverting to a flexible `fr` share on Company Name ALONE meant it was the ONLY flexible column in `soaListColumns` — it necessarily absorbed 100% of whatever leftover viewport width existed, no matter what else changed around it, which is exactly what kept recreating the gap right before Source/Current regardless of the exact fr number used. Fixed by splitting the flexible share across TWO columns: Company Name now gets a small `0.35fr`, and Owner (the row's actual trailing column — already a `<select>` with `width:'100%'`, reads fine wider) gets `0.85fr` (via `minmax(150px,0.85fr)`, was a fixed `150px`). The page still fills edge-to-edge either way; the difference is WHERE the leftover space goes — mostly to the end of the row now (a wider Owner dropdown, a normal/expected look) instead of pooling right after Company Name and splitting it from Source. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA list — reverted the Company Name width cap and simplified the numbers/Source badge, from 2 screenshots. Vincent: "好像之前的比较好，就是尽量填满，然后我加多颜色太多了，全部变成灰色会在深蓝色就好，并且数字不需要加粗普通的 Arial". (1) The width cap from the previous update actually made it worse, not better — reverted `soaListColumns` back to the flexible `1.2fr`/`1.4fr` Company Name track so the list fills the available width like before; the real fix for the "messy" look turned out to be (2) below, not the column width. (2) The Source badge's per-system colors (added earlier today at his own request) turned out to be "太多颜色" once seen against the aging columns' own existing severity rainbow (BUCKET_COLOR: gray/teal/amber/orange/red by how overdue) — both simplified to one flat scheme: Source badge is now a single gray-bg/navy-text style for all 3 systems (no more per-company `SOURCE_COLOR` lookup, removed); aging-bucket numbers are now plain gray for any non-zero value (Total keeps navy, the one deliberate "dark blue" he asked to keep). (3) Aging/Total numbers no longer bold (`fontWeight: 400`, was 700/800) and now explicit `Arial, Helvetica, sans-serif` instead of the app's default UI font (`-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto` — resolves to Segoe UI on his own Windows machine, a different face despite looking similar, so this needed an explicit override to be literally Arial). `BUCKET_COLOR` itself untouched — still used by the detail modal's own per-invoice bucket badge, which he hasn't asked to change. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA — fixed a real layout bug on all 4 list pages: the Company Name column's dead space was opening up BETWEEN Company Name and the next column, not trailing after the row. Vincent, from a screenshot of the All list: "这部分的排版可以怎么样调整一下，现在看起来稍微有点不协调有点乱" (the Source badges read as disconnected from Company Name, with a big empty gap between them). Root cause: Company Name used an unbounded `minmax(200px,1.2fr)`/`minmax(220px,1.4fr)` CSS Grid track — on a wide screen this grows the COLUMN ITSELF far past what any real name needs, and the extra width shows as blank space inside that column (right after the text, before the next column starts), not as trailing space at the row's end. That's exactly what broke the "Source sits right next to Company Name" adjacency he asked for when All shipped earlier today. Fixed in `app/billing/soa/_components.tsx`'s `soaListColumns` by capping the column at a fixed max (440px in All mode, 460px in TAB/TAC/TAO mode — still comfortably wide enough for a genuinely long real name like "Anda Technology Pte. Ltd. (F.K.A. Anda Microelectronics Technology Pte Ltd)", confirmed against the real company by that name in the screenshot) instead of an unbounded `fr` share — any leftover viewport width now pools harmlessly after the very last column instead of splitting two related columns apart. Applied to both the All and TAB/TAC/TAO column definitions since it's the identical underlying mechanism, not something specific to All. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA — "Export Full Workbook" now includes a 19th "All" sheet, matching the new on-screen All view. Vincent: "因为现在多了一个All , 所有等于在 EXPORT FULL WORKBOOK那边要加多一个 ALL 的 SHEET". `lib/soa-data.ts`: extracted `tagAndMergeSoaRows(tab, tac, tao)` out of `computeAllSoaRows()` — the tag+concat+sort step alone, so `app/api/billing/soa/export-all/route.ts` can build the exact same "All" row set from the tab/tac/tao arrays it already fetched for its own 3 sheets, with zero extra Supabase round-trips and zero risk of drifting from what the on-screen All page shows. `lib/soa-export.ts`: `renderAgingTable()` and `setColumnWidths()` generalized with an explicit `includeSource` flag (not inferred from row data — an empty rows array must never silently pick the wrong header) that inserts a Source column right after Company Name, shifting the aging buckets/Total/PIC columns over by one; new `buildAllSheet()` (title block "All Systems Combined (TAB + TAC + TAO)" + the aging table with Source, labeled TOTAL) reuses this. New sheet placed FIRST in the workbook — no real tab on Vincent's own Google Sheet to match (it has no "All" tab), so this follows the app's own new All-before-TAB/TAC/TAO ordering instead. Verified against real data before shipping: 434 rows (exact match to the on-screen All page), header/title block render correctly, grand-total SUM formula result (669346.97) matches an independent JS sum exactly, and "1V CAPITAL PTE. LTD." appears exactly twice — TAB and TAO — same as the on-screen view. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA "All" view — Source badge now color-coded per system. Vincent, from a screenshot of the Source column all rendering in the same gray pill: "这边稍微用不同的颜色区分 TAB/TAC/TAO". New `SOURCE_COLOR` map in `app/billing/soa/_components.tsx` — TAB blue (`#eff6ff`/`#1d4ed8`), TAC violet (`#f5f3ff`/`#7c3aed`), TAO green (`#f0fdf4`/`#15803d`) — same pastel-bg/saturated-text pairing convention this app already uses for status badges, deliberately distinct hues from the aging-bucket colors sitting in the very next columns of the same row so Source never reads as another severity signal. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA — new "All" combined view, a 4th sidebar entry above TAB/TAC/TAO showing every system's rows together. Vincent: "接下来我要在 Outstanding -TAB的上面加多一个3级标题（All）, 这个All, 就是把 TAB/TAC/TAO的所有总和放进去 举例：TAB/TAO 都有 1V CAPITAL PTE. LTD.，所有就要在ALL 出现2行 1V CAPITAL PTE. LTD.，而这两行是同步 TAB/TAO内的记录的，当然在 ALL这边改OWNER，TAO/TAB也会有变化， 但是ALL要多一列，company name 右边第2列 要放Source : TAB or TAC or TAO" — deliberately NOT a dedup/merge view: a company owing on 2+ systems renders as 2+ separate rows, one per system, each carrying its own real numbers/Owner. New `computeAllSoaRows()` (`lib/soa-data.ts`) concatenates `computeSoaRows('TAB'|'TAC'|'TAO')` in that fixed order, tags each row with its real `qbCompany`, then stable-sorts by company name only — same-named rows across systems land adjacent (TAB before TAC before TAO) without ever merging their numbers. New `GET /api/billing/soa/all` serves it; new `/billing/soa/all` page (thin wrapper, same pattern as tab/tac/tao); new "All" sidebar entry added ABOVE "TAB" in `components/Sidebar.tsx` (order: All, TAB, TAC, TAO). Frontend (`app/billing/soa/_components.tsx`): `qbCompany` prop widened to `QbCompany | 'ALL'`; new `Row` type (`SoaCompanyRow & { qbCompany?: QbCompany }`) plus `rowCompany(c)` (resolves a row's real system — its own tag in All mode, else the page's fixed one) and `rowKey(c)` (`` `${system}:${companyName}` ``, since bare company name is no longer a unique row identity once the same company can legitimately appear twice — used for the React list key, the expanded-detail lookup, and the optimistic PATCH match, replacing bare `companyName` everywhere those used to collide risk existed). `updateSoaPic()` now takes the whole row (not just a name) so an Owner edit PATCHes `soa_owners` scoped to THAT row's own real system, never the literal `'ALL'` — this is genuinely the same write the single-system pages make, not a synced copy, so it's automatically consistent both directions with zero extra sync logic. New "Source" column (TAB/TAC/TAO badge) inserted right after Company Name, only in All mode. Single "Export Excel" button hidden in All mode (no one real sheet to export); "Export Full Workbook" and Refresh stay. Verified against real production data before shipping: 434 total rows (TAB 242 + TAC 60 + TAO 132, an exact sum — no loss/dup from the concatenation), 83 companies genuinely owing on 2+ systems, and Vincent's own example confirmed exactly as he described it — "1V CAPITAL PTE. LTD." really does appear as 2 separate rows (TAB: soaPic=Chin Kah Ye; TAO: soaPic=Chin Kah Ye but suggestedOwner=Lee Jing Fei, a real prior manual edit from earlier this session) — plus an adjacency check confirming every multi-system company's rows sort next to each other with no gaps. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: Late Filing — same title-bar/toolbar restructuring just done for SOA, applied here too. Vincent, from a real screenshot: "Late Filing Companies / Auto-detected from AR records / 放在company name 上面 蓝底白字" (put above Company Name, dark-blue background white text) and "Late FY: / 放到卡片上方左边" (move above the cards, on the left). `app/late-filing/page.tsx`'s top header row (icon+title+badge on the left, Refresh/Add Manual on the right) → title+badge removed from there, "Late FY:" year-filter buttons moved into that same left slot instead (Refresh/Add Manual stay on the right, unchanged); the now-redundant standalone "Year Filter" block below the stat cards removed. Title+hint ("Late Filing Companies" / "Auto-detected from AR records") now render inside a new `system-list-title-bar` directly above the table's Company Name column header — same dark-navy `.system-list-title-bar`/`.system-list-title`/`.system-list-title-hint` CSS classes SOA's own title bar already uses, so both pages now share the identical convention rather than each inventing its own. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA header — restyled again + title moved back into the list's own dark bar, matching AR Reminder/Billing Drafts' real layout. Vincent, from real screenshots of both: Export Excel/Export Full Workbook → teal-green `#397f78` background, white text, on the LEFT (matching a real "Export Excel" button he showed from another page); Refresh → dark navy `#1e3a5f`, white text, on the RIGHT — mirrors AR Reminder's own toolbar exactly. Separately: "把SOA — TAB Statement of Account...放回去Company Name 上面的深蓝色行" — the title+hint (which the previous "match Late Filing" pass had moved into a plain page-level header) goes back into the list's own dark `system-list-title-bar` directly above the column headers, matching Billing Drafts' own real dark-navy title-bar convention (`.system-list-title-bar`'s existing CSS was never removed, just its usage in this file) — the page-level header row now holds only the 3 action buttons, no title. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA header buttons — reordered + Export buttons restyled. Vincent, from a screenshot: reorder to Refresh / Export Excel / Export Full Workbook (was Export Excel / Export Full Workbook / Refresh), and the two Export buttons should be dark-blue background with white text — Refresh stays the neutral white/gray style. Reused the same `#1e3a5f` dark navy already established as this app's "primary action" button color (e.g. Late Filing's own "+ Add Manual" button). `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: Address Service — real bug fix confirmed end-to-end against real data. Vincent ran the SQL migration ("SQL好了") and a TeamWork sync was triggered directly (same CRON_SECRET + canonical-domain pattern as the earlier QuickBooks sync trigger this session). Real, verified impact: 376 active companies now correctly flagged `uses_address=true` (356 Anson Road, 3 High Street, 12 Paya Lebar, 2 Kallang, 2 Cecil Street) — **19 real companies newly discovered** that the old single-address rule had been silently missing this whole time (e.g. "GLORY TRADE PTE. LTD.", "CITYNEST PTE. LTD." at Paya Lebar; "NOVA GOLDEN ALPHA PTE. LTD." at Kallang). One remaining "Unrecognised" row ("GOLDEN BRIDGE MARTEC PTE. LTD.") is a pre-existing, already-documented TeamWork-side duplicate-entity quirk (see `components/AddressServiceTable.tsx`'s own comment), not a new gap — left as-is rather than chased further.

Previous update, same day: Address Service — real bug fix: only 1 of Tassure's 5 real address-service locations was ever recognized. Vincent relayed his boss directly: "Besides our own current office address as client registered office address, we also use" 4 more real addresses (77 High Street #08-04, 60 Paya Lebar Road #04-32, 47 Kallang Pudding Road #09-10, 105 Cecil Street #18-26). Investigated before touching anything: `app/api/teamwork/sync/route.ts`'s `usesOurAddress()` was a single regex matching ONLY "10 Anson Road #12-08" — its own code comment even documented that it was validated exclusively against companies ALREADY flagged `uses_address=true`, never checking whether any company NOT flagged actually has one of Tassure's other 4 real addresses. This is a genuine, silent undercount that's been live in every consumer of `companies.uses_address` (the Address Service page itself, AR Reminder, Reports, Company 360, dashboard/stats) — the raw registered-address text was never persisted anywhere either (used transiently within one sync run, then discarded), so the real scope can't be queried after the fact; only a fresh TeamWork sync run can surface it going forward. New `lib/address-service.ts` (`ADDRESS_SERVICE_LOCATIONS`, `matchOurAddress()`, `usesOurAddress()`) recognizes all 5 real addresses, validated by regex-testing all 5 exact strings Vincent's boss gave plus a few plausible TeamWork formatting variations (different casing/spacing) before shipping — all matched correctly, an unrelated address correctly didn't. New `companies.address_service_location` column (`scripts/add-companies-address-service-location.sql`, not yet run) records WHICH of the 5 a company is actually at, written by the same sync pass that sets `uses_address`. `app/address-service/page.tsx` + `components/AddressServiceTable.tsx`: replaced the old single hardcoded "10 Anson Road" metric card with a real per-location breakdown (5 MetricCards + an "Unrecognised" bucket for any company where `uses_address` is true but the address text didn't match — surfaces a data gap instead of silently dropping it from the count) and a Location column + filter on the table. Also fixed a real pre-existing silent-failure bug found while touching this exact function: `getData()` destructured only `{ data }` from the Supabase query, never checking `error` — a query failure (which adding a not-yet-migrated column to the select would have caused) would have silently rendered "0 companies" with no error at all; now throws with the real error message. Added **INV-TW-018**, and an Active Issues note in `docs/CURRENT_STATE.md` (the undercount doesn't self-correct until BOTH the SQL migration runs AND a TeamWork sync actually reprocesses affected companies). `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA — page layout matched to Late Filing's own pattern. Vincent, from a screenshot comparing the two: "这些按钮设计在卡片上方,好像late filing 页面那样的排版" (these buttons should be designed above the cards, like the Late Filing page's layout). `app/late-filing/page.tsx`'s real structure: a header row (icon+title+badge on the left, action buttons on the right) sits above its 5 metric cards, and its list further down goes straight into the table with no separate title bar inside it. `app/billing/soa/_components.tsx` restructured to match exactly: new page-level header (Receipt icon, "SOA — {qbCompany} Statement of Account", a badge) with Export Excel/Export Full Workbook/Refresh now living there — same button padding (7px 14px)/font size (13px)/radius (8px) as Late Filing's own buttons, not just moved — above the 3 metric cards; the list's own `system-list-title-bar` (which used to hold a duplicate title + these same 3 buttons) removed entirely, list now goes straight from `system-list-shell` into `system-list-scroll`, matching Late Filing's list precisely. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA Excel export — TOTAL row's numbers weren't visible, and per-person sheets got the "TOTAL" label too. Vincent, from a real exported file's screenshot: "TOTAL,没有整合每一列的数值总额，然后个人的也是要有TOTAL，也是要有整合" (the TOTAL row isn't showing each column's summed value, and the per-person sheets should have a TOTAL too, also summed). Root cause of the blank/missing sums: ExcelJS writes a `{formula: 'SUM(...)'}` cell but never evaluates it, and ships no cached value alongside it — whatever app opens the file (a Google Sheets import, in his case) only shows a number once it independently recalculates, which isn't guaranteed to happen automatically. Fixed in `lib/soa-export.ts`'s `renderAgingTable()` by precomputing the real sum in JS (already had every row in memory) and passing it as `result` alongside the formula — the correct number now shows immediately regardless of the viewer's own recalc behavior, while the formula itself stays live/authoritative if a row is later edited in Excel. Verified against real TAC data: `SUM(B6:B65)` now carries `result: 4840.5` etc., matching an independently-computed JS sum exactly (152217.06 grand total). Also changed `buildPersonSheet()` to pass `'TOTAL'` as the label (previously unlabeled, deliberately matching his real sheet's own per-person tabs — he's now asked for the label directly, a deliberate stated departure from that exact match). `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA — company names shown in ALL CAPS everywhere, for visual consistency. Vincent: "公司名要统一，参考Billing Drafts" — investigated first rather than guessing: checked for real cases where the same company shows a different name across TAB/TAC/TAO (only 1: "RTG Projects", and only because it has no `companies` row at all) and confirmed Billing Drafts never hits this because its company list is always sourced directly from `companies` (never a raw QuickBooks name) — asked Vincent directly which of a few concrete fixes he meant; his answer: "没事，我说的统一是大字母先罢了" then "都大字母" (never mind, I just meant — make everything uppercase). Root cause: `companies.company_name` is mostly already ALL CAPS (real Singapore entity names, as recorded), but the ~52/336 companies with no `companies` match fall back to the raw QuickBooks `customer_name`, which can be mixed-case — reading as inconsistent side by side. Fixed with `.toUpperCase()` at DISPLAY time only (list row, detail modal header in `app/billing/soa/_components.tsx`; the Excel cell value in `lib/soa-export.ts`'s `renderAgingTable()`) — deliberately NOT on the underlying `companyName` field `computeSoaRows()` returns, since that value is also used as an exact lookup key (`/detail`, `/pdf`, and `campaigns/preview`'s fast-path `.eq('company_name', lookup)` — confirmed that specific query gracefully falls through to its own full fuzzy-match path on a miss, so even a company whose real name isn't already uppercase would just cost an extra ~850ms on "Draft Email", never break the recipient resolution — but keeping the underlying value pristine avoids that regression case entirely and any future lookup risk). `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA — full 18-sheet Excel workbook export, mirroring Vincent's entire real Google Sheet. Vincent, from a screenshot of his sheet's tab bar: "另外要生成一个完整版的EXCEL（和GOOGLE SHEET 那边的一样的），要有 TAB/TAC/TAO/每个人员的/internal的". Investigated the real sheet's full tab list first (parsed its own htmlview page's embedded tab-list JS, not guessed) — found the exact same structure his screenshot showed: TAB/TAO/TAC (his real tab order, not alphabetical) + 14 staff-code tabs (JF/YH/VC/JT/WE/VY/CS/QT/TSM/LHC/JL/ASM/HSX/CKY) + "Internal" + 2 tabs he didn't ask for (Bank Account, Template - PIC — deliberately excluded). Inspected a real staff tab (CKY) and the real Internal tab's content before building anything: staff tabs have NO title block (unlike TAB/TAC/TAO) and their own bottom sum row has no "TOTAL" label; a company owing on 2 systems under the same owner appears as 2 SEPARATE rows, never deduplicated across systems; Internal is a stack of per-person mini-tables (own header, own rows, blank separator) for everyone without a dedicated code tab. New `GET /api/billing/soa/export-all` (no params — always the full workbook) builds all 18 sheets: TAB/TAO/TAC via the existing `buildCompanySheet`, one `buildPersonSheet` per resolved staff code (via `resolveStaffName()` — JF→Lee Jing Fei, CKY→Chin Kah Ye, etc.), one `buildInternalSheet` grouping everyone else by their real effective owner. Refactored `lib/soa-export.ts` (new) out of the single-company export route — `renderAgingTable()` is the one shared table-renderer both the per-company and full-workbook exports now call, so the two formats can't silently drift. New "Export Full Workbook" button added beside the existing "Export Excel" on all 3 SOA pages (no single shared page to put a combined-only button on). Verified end-to-end against real data before shipping: 434 total rows across TAB(242)/TAC(60)/TAO(132); all 14 codes resolved to real names with real row counts (TSM 73, ASM 67, CKY 47, LHC 47, ... down to WE 5); Internal correctly grouped into exactly BD(2)/Chelsea Ang(4)/Esther Loo(6)/Samuell Ng(4)/Tan Yee Soon(3) — matching what's actually in his real Internal tab; 4 rows genuinely have no owner at all and correctly appear in neither a person nor Internal sheet (only in their own TAB/TAC/TAO sheet, which lists every row regardless of owner); generated an 18-sheet, 57KB workbook and read it back successfully via ExcelJS to confirm it's genuinely valid. One deliberate deviation, stated rather than silently applied: his real Internal tab has 2 separate "BD" blocks (evidently pasted in at different times) — this groups each person into exactly ONE clean block instead of replicating that duplication. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA Excel export — added the missing bottom TOTAL row. Vincent: "你少了一行total" — his real sheet ends with a bold "TOTAL" row summing every column, values shown with an "S$" prefix (unlike the plain-number data rows above it). `app/api/billing/soa/export/route.ts` now appends this row using real Excel `SUM(range)` formulas over the exact data-row range (not a static precomputed number — stays correct if a row is ever edited/deleted directly in Excel), formatted with `"S$"#,##0.00`. Verified the formula ranges are exactly right against a real 60-row TAC export (`SUM(B6:B65)` etc., matching firstDataRow/lastDataRow computed from the real row count) and cross-checked the expected sums independently in JS. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA — Excel export, matching Vincent's real Google Sheet's own layout. Vincent: "我要可以导出EXCEL，要和GOOGLE SHEET的格式一样" — new `GET /api/billing/soa/export?company=TAB|TAC|TAO` builds an .xlsx with the exact same structure as his sheet's own tabs (confirmed against the real ones investigated earlier this session): row 1 the QuickBooks company's real legal entity name ("Tassure Asia Bizservices/Consultancy/Outsourcez Pte Ltd" for TAB/TAC/TAO), row 2 "A/R Ageing Summary Report", row 3 "As of {today}", then Company Name/Current/1-30/.../91+/Total/PIC columns — money cells are real numeric Excel cells (`#,##0.00` format, sortable/summable), not plain text, and blank buckets stay genuinely empty rather than "0.00" (matching both his sheet's own convention and the on-screen list). Refactored the shared row-computation out of `app/api/billing/soa/route.ts` into new `lib/soa-data.ts` (`computeSoaRows()`, `effectiveOwner()`) so the on-screen list and this export can never silently compute different numbers for the same company — same "one shared source" principle as `correctedCustomerName()` (INV-QB-012). Frontend: new "Export Excel" button next to Refresh in `app/billing/soa/_components.tsx`, same blob-download pattern `SoaDetail`'s own `downloadPdf()` already uses. Verified against real TAO data before shipping (132 real rows, generated workbook read back successfully via ExcelJS to confirm it's a genuinely valid file, not just "didn't throw") — this also incidentally surfaced a real, correct edge case: 1V Capital's TAO Owner now computes to "Lee Jing Fei" (not "Chin Kah Ye") because Vincent himself manually cleared that row's confirmed owner via the app since the last backfill (`updated_by_email='vincent@tassure.com'`) — `effectiveOwner()` correctly fell through to the QB-Class-derived suggestion, exactly as designed, not a bug. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA — "BD" made a universally-selectable Owner option, not just something already backfilled onto 3 companies. Vincent: "那个OWNER的部分，是每个人都可以放BD" / "每个公司都能放BD" — right after restoring the literal "BD" value onto the 3 companies that already had it, he clarified it should be pickable for ANY company, since it's a real, general-purpose "hold off for now" status, not tied to those specific 3. New `PLACEHOLDER_OWNER_CODES = ['BD']` (never added to `lib/staff-directory.ts` — it isn't a real person) renders as its own "Other" `<optgroup>` at the end of the Owner dropdown, present for every company except one where it's already the current value (already shown once, in "Associated with this company"). `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA — two small real-data/display fixes. (1) Vincent: "然后BD的，你就先放回去BD" — the 3 real companies using "BD" (a deliberate "放着先"/hold-off marker, not a person, per his earlier clarification) had been left with NO owner at all by the backfill's `KNOWN_NON_PERSON_CODES` skip; he wants the literal text "BD" written back as their Owner instead of blank. One-off script wrote `soa_pic = 'BD'` for Quantum Marine Services (TAB), RTG Projects (TAB+TAO), Monster Game (TAO) — the Owner `<select>`'s existing "always guarantee the current value is selectable" logic already renders an arbitrary string like this correctly with no code change needed there. (2) Vincent, from a real screenshot of the list's aging columns: "这些的所有数字前面都不需要 S$" — the Current/1-30/31-60/61-90/91+/Total columns drop the "S$" prefix (new `fmtNum()`, plain-number version of `fmtMoney()`); every OTHER money figure on this page (the detail modal's header summary, its invoice-breakdown table) keeps `fmtMoney()` since he pointed at the list specifically. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA Owner dropdown — can now assign anyone, not just people already associated with the company. Vincent: "假设某个人不在PIC，但是owner 我要加她怎么办" — the dropdown previously offered ONLY `picOptions` (companies.pic ∪ real QB Class names) once there was at least one, falling back to the full staff directory only when that list was completely empty — so Chelsea had no way to hand a balance to someone who genuinely hasn't touched this company yet (a coverage handoff, a new assignment). `app/billing/soa/_components.tsx`'s Owner `<select>` now always offers every real staff name, grouped via `<optgroup>` — "Associated with this company" (the likely picks: picOptions + the current displayed value) first, then "All staff" (everyone else, alphabetical — `allStaffNames()`'s own file-declaration order isn't scannable as a long flat list) below. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA Owner — soa_owners rearchitected to be per-QuickBooks-company, not global per customer. Vincent asked directly: "所以现在这边的逻辑是什么和我GOOGLE SHEET的显示，为什么又存在差异，真正产生差异的点和原因是什么？" — investigated by fetching his real Google Sheet's OTHER 2 tabs (TAB was the only one ever read before — the plain CSV export URL defaults to the sheet's first/initial tab, which happens to be TAB; discovered TAC's real gid=1889088740 and TAO's gid=1473071804 by parsing the sheet's own htmlview page for its embedded `items.push({name: "TAC"/"TAO", gid: ...})` tab list). Root cause, confirmed with real numbers, not assumed: the sheet has 3 SEPARATE tabs (51 TAC rows, 115 TAO rows, neither ever read before), 81 real companies owe on 2+ systems, and of those, 13 have a genuinely DIFFERENT primary PIC per tab (e.g. "Meishan Silk Road Trading": TAB tab=CKY, TAO tab=a different person) — `soa_owners` was keyed by customer name ONLY, so the TAB-tab-only 2026-09-05 backfill's answer was being applied uniformly across all 3 pages, silently wrong for those 13+ companies (and blank for any TAC/TAO-only company, relying entirely on the newer QB-Class `suggestedOwner` instead). Also discovered TAO's sheet rows very often carry a SECOND PIC name in an adjacent column (31/115 rows, e.g. "1V Capital": CKY + JF) that the original backfill's single-column reader never captured — separately already handled correctly by `collectInvolvedStaff()`'s real QB Class scan (confirmed: 1V Capital's real TAO invoice independently surfaces "Lee Jing Fei" from Class data, matching the sheet's "JF" annotation exactly, with no sheet-reading needed for that part). Fix: `scripts/add-soa-owners-per-company.sql` (new, not yet run) adds `soa_owners.qb_company` + changes the unique key to `(customer_name_norm, qb_company)`; `GET`/`PATCH /api/billing/soa` scoped accordingly; `app/billing/soa/_components.tsx`'s `updateSoaPic` now sends `company: qbCompany`. Added **INV-DATA-015**. Checked for real human edits before planning the corrected backfill (not just blind-overwriting): found exactly 1 row Vincent had manually cleared via the app himself (`updated_by_email='vincent@tassure.com'`, not `backfill@internal`) — the new backfill script (`scripts/backfill-soa-owners-per-company.ts`, written, NOT yet run — needs the SQL migration first) explicitly skips any row not stamped `backfill@internal`. Re-checked staff-name resolution across all 3 tabs: TAC 51/51 resolved; TAO 109/115 (unresolved: "VC" — 5 real companies incl. "Meishan Silk Road Trading", "Lebese International" — and "BD" — 3 companies, previously flagged, still unresolved); TAB re-checked too and shows the same "VC"/"BD" gaps (previously reported as "JT only", so "VC" is newly surfaced by this pass, not something that regressed). Neither guessed — flagged to Vincent, matching this session's established discipline for the earlier "BD" case. `npx tsc --noEmit` and `npm run build` both clean. **Completed end-to-end**: Vincent ran the SQL ("SQL也好了") and separately clarified "BD 就是放着先 的英文简写" (BD is shorthand for "leave it for now" — a deliberate non-person status marker, not someone to identify) — added `KNOWN_NON_PERSON_CODES` to the backfill script so it reports "BD" as an intentional, already-explained skip rather than re-flagging it as a mystery. Verified the migration applied (a real composite-key upsert round-trip succeeded) before running the backfill: **374 resolved, 373 upserted** (214 TAB + 51 TAC + 108 new TAO, the 1-row gap being the single human-edited "D3 Capital Investment" TAB row correctly preserved untouched), 4 "BD" skips, 5 genuinely unresolved ("VC" — still needs Vincent, not guessed). Spot-checked 3 real companies directly against the database to confirm the fix actually works, not just that the script ran without error: "Meishan Silk Road Trading" now correctly has ONLY a TAB row (its TAO PIC is the still-unresolved "VC", so no TAO row was written — falls back to the real QB-Class-derived `suggestedOwner` until "VC" is confirmed, exactly as designed); "1V Capital" now has its own independent TAB row AND TAO row (both "Chin Kah Ye", matching each tab's own primary PIC); "D3 Capital Investment" shows its preserved-null TAB row alongside a brand-new TAO row ("Lee Jing Fei") that simply never existed before this fix, since TAO was never backfilled at all previously. Deleted both one-off scripts after running. Final piece: Vincent confirmed "VC - Vernice Chai" — added `VC` as a `lib/staff-directory.ts` alias for Vernice Chai (she already existed there under alias `Vernice`) and ran a small targeted re-backfill for exactly the 5 previously-unresolved rows (all 5 resolved and written: "Lebese International" TAB+TAO, "Asgard Property", "Hong Ming Development", "Meishan Silk Road Trading" TAO). All genuinely-real PIC codes across all 3 tabs are now resolved — only the intentional "BD" (放着先) skips remain unassigned, by design. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: Sidebar — "SOA" renamed to "Outstanding". Vincent: "SOA 名字换成 Outstanding" — `components/Sidebar.tsx`'s level-2 group label only; the 3 level-3 entries (TAB/TAC/TAO) keep their own labels, and every internal "SOA" name (routes `/billing/soa/...`, the API, `soa_owners`, `lib/soa*.ts`, page titles/headings) is untouched — purely the nav's own display text. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA — list sorted alphabetically by company name, not by outstanding total. Vincent: "SOA 这边的排序也是要按照ABC 的顺序排序". `GET /api/billing/soa`'s final `.sort()` changed from `totalOutstanding` descending to `companyName.localeCompare()` — a single change at the API layer, so it applies consistently before any client-side filter/search/pagination touches the list. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA — confirmed real-time balance updates, closed a webhook data gap, added frontend auto-refresh. Vincent: "当QUICKBOOK那边更新了，这个客户已经付款了，这边的欠款数字会不会更新？而且最好是实时更新". Investigated against real production data before answering (not just re-reading code): a genuine QuickBooks webhook already exists and is actively firing on all 3 realms — queried `quickbooks_webhook_events` directly and found 478 real events ever processed, the most recent literally minutes old, with `changed_at → processed_at` typically 6-15 seconds end-to-end (Intuit's own delivery latency dominates). So a payment applied in QuickBooks already reaches `quickbooks_invoices.balance` within seconds — confirmed, not assumed. Two real gaps found and fixed while investigating: (1) `lib/quickbooks-invoice-incremental.ts` (the webhook path) never extracted `DepartmentRef`/`ClassRef` — only today's full-year sync did — so a brand-new invoice's Owner-suggestion signal would have silently lagged a full day behind its balance; now captures `location_name`/`class_name` identically to the full sync (documented as a shared obligation in **INV-QB-013**'s addendum, so a future change to either path doesn't quietly break the other). (2) The SOA page itself only ever fetched once per visit — the backend was already fresh, but nothing made the open page re-check it. Added a 30s silent background poll (`app/billing/soa/_components.tsx` — new `load({silent:true})` mode that skips the `setCompanies(null)` "Loading…" flash the initial/company-switch/manual-Refresh load still uses) so the numbers update themselves without a manual click, without disrupting an open detail row or in-progress search/filter typing. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA — confirmed working after a hard refresh (the earlier "Chin Kah Ye" vs "Lee Jing Fei" discrepancy was a stale page load, not a bug — Vincent: "可以现在好了"), plus two style tweaks he asked for from the real screenshot: (1) the PIC column's multiple names now render one per line (`c.picOptions.map(name => <div>{name}</div>)`) instead of a single comma-joined, awkwardly-wrapping line; (2) the Owner dropdown's teal "suggested, not yet confirmed" text ("那个青色的字不要斜体") drops the italic styling — stays teal-colored to distinguish it from a confirmed pick, just no longer slanted. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA PIC column — enriched with real QuickBooks Class names, not just companies.pic. Vincent, from a real example: "1V Capital Pte Ltd" — "确实最终PIC 是CKY，但是LJF也是负责人之一，真正在系统的显示应该是PIC：CKY,LJF，但是OWNER：CKY" (the final PIC really is CKY, but LJF is also one of the responsible people — the system should really show PIC: CKY,LJF, but OWNER: CKY). Investigated with real data: `companies.pic` for this company only lists "Chin Kah Ye" — incomplete, since its real TAO invoice's Accounts lines carry `Class="Lee Jing Fei"`, a genuine second person who's actually touched this account's billing that companies.pic (TeamWork-sourced, Secretary-department-only) never captured. New `lib/soa-owner.ts`'s `collectInvolvedStaff()` — unlike `computeSuggestedOwner()` (picks the ONE most-likely current owner), this collects EVERY distinct resolvable Class name across ALL of a customer's own unpaid invoices, deliberately excluding Location (a per-operator QB-login tag per INV-QB-013 — pooling it in would flood PIC with whoever happened to key invoices in, not who the service is for). `GET /api/billing/soa`'s `picOptions` is now the union of `companies.pic`'s decomposed names and this collected set; the PIC column (`app/billing/soa/_components.tsx`) now renders `picOptions.join(', ')` instead of `formatStaffName(companies.pic)` directly, so it can show names companies.pic never had. **Flagged directly to Vincent, not silently changed**: for THIS specific TAO example, `computeSuggestedOwner()` — Class-first, most-recent-invoice — actually computes Owner as **"Lee Jing Fei"**, not "Chin Kah Ye" as his screenshot showed, since TAO's own only currently-open invoice for this company (02660519) carries Class="Lee Jing Fei" on every resolvable line and no other unpaid TAO invoice exists to check; confirmed via direct DB replication of the exact route logic, not just re-reading code. Could not reproduce "Chin Kah Ye" as Owner from any real TAO-scoped data — asked Vincent to hard-refresh and confirm what the TAO page now shows, since the deployed code and logic are unchanged from the last verification pass (94.6/81.4/98.5% coverage) that this same screenshot session followed. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA Owner — auto-computed from real QuickBooks Class/Location data, no more manual Google Sheet backfill. Vincent asked Chelsea how PIC/ownership actually works in practice; her answer: "PIC不是说谁开单的 location那边，一般是服务的 class 那边的备注人员，如果class 没有才是看location的" (it's not about who's on the invoice's Location — normally it's whoever's noted on the service's Class; only fall back to Location when Class has nothing). Verified this against 30 real live invoices across TAB/TAC/TAO via a direct `qbQuery` call before writing any code — confirmed exactly the failure mode Chelsea described: e.g. a real TAB invoice with `Location="Chelsea Ang"` but every line's `Class="Chin Kah Ye"` — Chelsea processes invoices for several secretaries' clients under her own QB login, so Location alone would have wrongly attributed the client to her, not the real handler. Vincent's goal, stated directly: "不用再靠人工从 Google Sheet 回填" (so this no longer needs manual Google Sheet backfill). Implementation: (1) `scripts/add-quickbooks-class-location.sql` (new, not yet run in Supabase) adds `quickbooks_invoices.location_name` and `quickbooks_invoice_items.class_name`; (2) `app/api/quickbooks/sync/route.ts` now extracts `DepartmentRef.name`/`ClassRef.name` from the SAME already-fetched invoice JSON it was already parsing for service classification — zero extra QuickBooks API calls, and since the sync always re-pulls the full current+2-prior years (`SELECT *`), the very next sync run backfills every recent invoice automatically, no separate one-off script; (3) new `lib/soa-owner.ts`'s `computeSuggestedOwner()` — most-recent-invoice-first, Class before Location, via new `resolveStaffName()` export (`lib/staff-directory.ts`, returns null instead of guess-via-title-case when a Class/Location value like "To Be Assign" isn't a real staff member — confirmed this literal placeholder value exists on real TAO/TAC invoices); (4) `GET /api/billing/soa` now also queries `quickbooks_invoice_items` (scoped to that company's own unpaid invoice ids, not the whole history table) and returns a new `suggestedOwner` field per company; (5) `app/billing/soa/_components.tsx`'s Owner column and "My book" filter both now resolve an `effectiveOwner = soaPic ?? suggestedOwner`, shown in italic/teal when it's a suggestion rather than a human-confirmed pick, with the single-picOption fallback demoted to third priority. Added **INV-QB-013** documenting this Class-before-Location priority rule (companion to INV-QB-007, which governs WRITING a PIC/Class, not reading one back). `npx tsc --noEmit` and `npm run build` both clean. **Verified end-to-end against real data**: Vincent ran the SQL migration ("SQL好了"), then a full production sync was triggered directly (`GET /api/quickbooks/sync` with `CRON_SECRET`, via the canonical `tassure-corporate-services.vercel.app` domain — `tassure-invoice.vercel.app` 307-redirects there) — all 9 company×year combinations (TAB/TAC/TAO × 2024/2025/2026) synced with zero errors. Confirmed real coverage by replicating the route's exact logic in a temporary script against the live database (not just trusting the code compiles): **TAB 226/239 (94.6%)**, **TAC 48/59 (81.4%)**, **TAO 130/132 (98.5%)** of companies with an outstanding balance now get a real, non-guessed `suggestedOwner` with zero manual input — e.g. 天马微电子股份有限公司 (one of the mojibake companies fixed earlier this session) correctly resolves to "Chin Kah Ye". The one notable TAB gap, "TASSURE PAC" (S$34,596.57 outstanding, no suggestedOwner) — reads as an internal/non-real-client account rather than a data problem, left as-is rather than guessed at.

Previous update, same day: SOA split into 3 company-scoped pages + its own sidebar heading. Vincent: "我要你把 SOA 放成一个单独的2级标题，然后把 TAB/TAC/TAO分成3个不同的3级标题，数据分开" — SOA used to be nested inside "Billing Drafts" as one combined page pooling TAB+TAC+TAO invoices into a single row per customer (a company owing on two systems showed one merged total/aging). Restructured: `components/Sidebar.tsx` now has a standalone "SOA" level-2 group (sibling of "Billing Drafts", not a child of it) with 3 level-3 entries — TAB/TAC/TAO — each its own route (`/billing/soa/tab`, `/tac`, `/tao`). The old `app/billing/soa/page.tsx` (previously the combined view) is now a thin client redirect to `/billing/soa/tab` so an old bookmark/link doesn't 404; the real view moved to a new `app/billing/soa/_components.tsx` (`SoaBillingView`, taking a `qbCompany: QbCompany` prop), rendered by 3 thin `page.tsx` files under `tab/`, `tac/`, `tao/`. Data separation is real, not just UI labeling: `GET /api/billing/soa`, `GET /api/billing/soa/detail`, and `GET /api/billing/soa/pdf` all now REQUIRE a `company=TAB|TAC|TAO` param and filter the underlying `quickbooks_invoices` query with `.eq('qb_company', company)` at the query level (not filtered after the fact) — a TAB statement can never include a TAC or TAO invoice. Deliberately kept SHARED across the 3 books: the `soa_owners` collections-owner assignment (still keyed by customer name only, no qb_company column) — it's the same real person chasing the same real customer regardless of which system billed them, so picking an Owner on the TAB tab also shows up on that customer's TAC/TAO tab. Also added a `qbCompany` reset on every company switch (search/PIC-filter/expanded-row state all clear) since Next.js reuses the same component instance across these 3 sibling routes rather than remounting it — without this a stale TAB search/filter would silently carry into the TAC page. `npx tsc --noEmit` and `npm run build` both clean; all 3 routes build correctly (`/billing/soa/tab`, `/tac`, `/tao` alongside the redirect `/billing/soa`).

Previous update: 2026-09-06 (SOA email templates — missing PayNow/payment-options image. Vincent, looking at a real SOA draft preview: "这个内容把那个PAYNOW的图也要配上，就和那个TAB/TAC的drafts 的一样" (this needs the PayNow image too, same as the TAB/TAC drafts). Not a code bug — `draft-helper/app.py`'s embedding logic is entirely content-driven: it scans the outgoing body for the literal line "PAYMENT METHOD付款方式:" and inserts the bundled payment-options image right there (`OutlookStyleSendModal`'s own preview does the same check via `PAYMENT_MARKER`). Queried the real `email_templates` table: all 3 `type='ar'` templates (used by TAB/TAC) end with that exact line; both `type='soa'` templates (SOA1 - the default, SOA2 - Batch) never had it — a template-content gap from when they were first created, not an app bug. Fixed by appending the identical marker line to both SOA templates directly (one-off script, deleted after running, per this repo's convention for content-only DB fixes with no code change). Any SOA draft created from now on will show the payment image in preview and get it embedded on send/open; a draft already created before this fix keeps its old body (baked in at creation time) — re-running "Draft Email" for that company picks up the corrected template.

Previous update, same day: Real bug fix — `OutlookStyleSendModal` silently dropped a caller's pre-attached `additional_attachments`. Vincent: "能不能在draft email 的时候一起把这个SOA带上呢，就算我还没有下载...PDF的前提" (can the SOA PDF come along when I draft the email, even without pre-downloading it) — investigated why SOA's `draftEmail()` (which already fetches the merged statement PDF and sets `additional_attachments: [pdfFile]` on the draft it opens) wasn't actually delivering it. Found the modal (`components/client-communications/OutlookStyleSendModal.tsx`) manages its own separate `manualFiles` state for drag-and-dropped attachments, always initialized to `[]`, and `handleSend` builds the outgoing draft from THAT state — never from the `draft.additional_attachments` prop it was given. The attachment list UI (line ~290) only renders `manualFiles` too, so the pre-attached PDF was invisible in the review screen, not just missing from the send — no partial win, it never showed at all. Confirmed via grep this is isolated to this one modal: the two other real callers that pre-populate `additional_attachments` (`campaigns/page.tsx`, `history/page.tsx`) both go through `openDraftsInOutlook` directly, which already reads the prop correctly — only SOA's flow routes a pre-attached file through this modal. Fix: seed `manualFiles`'s initial state from `draft.additional_attachments ?? []` — Remove/drag-drop on top still work unchanged. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA — "My book" PIC filter on `/billing/soa`. Vincent, looking at the Owner column: "我选择某个PIC，她就能看到和自己相关的所有欠款公司" (let me pick a PIC and see all the outstanding companies related to her). Added a dropdown next to the search box, populated only with names that actually appear somewhere in the real data (soaPic or the decomposed picOptions — not the full staff directory, most of whom never touch collections). Selecting a name scopes BOTH the 3 KPI cards and the list to: everything where she's the confirmed Owner (soaPic), plus anything still listed on the raw PIC field that nobody has claimed an Owner for yet (so she can find her own unassigned companies too) — once a different person is picked as Owner, it drops out of her view, since that assignment is meant to be authoritative. The free-text search box stays a find-one-company tool layered on top of whichever scope is active, not a KPI input, matching this page's existing convention (`counts` was already independent of `search`). `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA owners — real backfill from Vincent's Google Sheet, plus 8 missing staff-directory aliases it surfaced. Vincent: "还是有一些是空的，这些能不能通过GOOGLE SHEET的记录去填补呢？" — fetched the sheet's real CSV export (its `/export?format=csv` URL, following the redirect it returns) rather than trusting WebFetch's usual markdown-summarized read for bulk data, and got back ~217 clean, verbatim CompanyName|PIC rows. Running them through the real `formatStaffNameList()` resolver (not a fresh string-matching script) surfaced 3 real gaps: `lib/staff-directory.ts` had never captured "LHC"/"HSX"/"ASM"/"CKY"/"JL"/"TSM" (the Corporate Secretarial team's own initials shorthand — the file's own comment already noted only Accounting/Tax's initials, YH/WE/JF/QT/VY/CS, had been added, "no single consistent formula fit all of them") or "JT" for Jay Tay (had "Jay"/"JAY" but not his actual initials), or bare first names for Chelsea Ang/Samuell Ng/Esther Loo (used bare in the sheet, e.g. "Christchurch Global LLC | Chelsea"). Added all 9 as aliases, verified the backfill went from 214/217 resolved to 215/217, then ran it for real: `soa_owners` now has 215 real entries upserted, keyed by normalized company name (including the two mojibake companies fixed earlier this session — 天马微电子股份有限公司/思店科技(杭州)有限公司). 2 companies ("Quantum Marine Services Pte. Ltd.", "RTG PROJECTS PTE. LTD.") use a PIC code "BD" not in the directory at all — left unresolved rather than guessed, flagged to Vincent. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA — owner assignment rekeyed to customer name, not companies.id. Vincent, looking at his real Google Sheet: some real customers with an outstanding balance (e.g. 天马微电子股份有限公司, one of the mojibake companies fixed earlier this session) DO have a manually-assigned collections owner there, but showed nothing in the new "Owner" column. Investigated with real data before proposing a fix: 60 of 332 real customers with a balance (18%) have no matching `companies` row at all. Broke that 60 down further rather than treating it as one bucket: 7 are individuals ("Logan", "Phoebe Chiang", "Yu Li", "Chen, Jindi" — TAO's real invoice items include Personal Tax/IR21 services, so billing individuals directly is genuinely normal, not a data error), 12 fuzzy-match an existing `companies` row closely enough to need human review before merging (e.g. "Grand Chen Holdings Pte. Ltd." scores 67% against the real, likely-DIFFERENT "GRAND CHEN RESOURCES PTE. LTD." — bulk-auto-merging by word-overlap score risked silently combining two separate real companies that just share a naming pattern), and 41 are genuinely new companies never onboarded via TeamWork. Recommended against bulk-inserting any of this 60 into `companies` — asked Vincent for the safest path given that split; he deferred to judgment with "只要相对准确和安全就可以做" (as long as it's reasonably accurate and safe).

Chose: don't touch `companies` at all. New `soa_owners` table, keyed by normalized customer name (not companies.id) — works identically for a real company, an individual, or a company with no `companies` row, with zero risk of polluting the shared companies table with non-company entities or merging two distinct real companies. Supersedes `companies.soa_pic` (added hours earlier the same day, never actually populated/used yet, so switching now cost nothing) — `scripts/add-soa-owners-table.sql` both creates the new table and drops that column. `GET/PATCH /api/billing/soa` reworked to read/write `soa_owners` by name instead. `lib/staff-directory.ts` gained `allStaffNames()` — a customer with no `companies.pic` at all still needs an owner to choose from, so the "Owner" dropdown falls back to the full staff list rather than showing nothing. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA — Total Outstanding metric card money formatting. Vincent: "货币单位和数字可以稍微分开吗？现在看起来像堆在一起" — the metric card's large-figure styling (`.metric-card-value` in `app/globals.css`: 28px, weight 800, `-0.035em` letter-spacing) squeezed "S$" straight into the digits with no visual separation at that size. New `MoneyValue` component in `app/billing/soa/page.tsx` renders the currency prefix as its own smaller, muted, right-margined span (`letterSpacing: normal` locally, so it isn't squeezed by the parent's tight tracking either) beside the number — the small-currency-tag-beside-a-big-number convention most finance dashboards use. Scoped to just this one large metric card; the plain `fmtMoney()` string stays for every smaller in-table money value on the page, where the tight spacing was never a legibility problem. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA — collections owner column, for when the PIC field lists more than one person. Vincent, looking at the just-shipped SOA list's PIC column: some real rows show "Chin Kah Ye, Ang Shi Ming" or "Lim Hoe Chyi, Hoo Seng Xin" — companies.pic can legitimately list 2+ co-assigned people, and for chasing one specific outstanding balance Chelsea needs to say which ONE of them actually owns it, without ever overwriting companies.pic itself (used elsewhere for its own separate meaning) just to resolve that ambiguity. New `companies.soa_pic` column (`scripts/add-companies-soa-pic.sql`, run in Supabase SQL editor — same ALTER TABLE convention as every other manual-override column in this repo). `GET /api/billing/soa` now also returns `picOptions` (each individual name decomposed out of `pic` via the `formatStaffNameList()` built earlier this session for the multi-name filter fix — direct reuse, no new splitting logic) and `soaPic`; new `PATCH /api/billing/soa` saves Chelsea's pick. `app/billing/soa/page.tsx` gained an "Owner" column next to PIC — a dropdown over `picOptions`, optimistically updated on change, showing the single option directly (no forced choice) when there's no real ambiguity. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: SOA — Statement of Account, shipped as a new third-level heading under Billing Drafts alongside TAB/TAC and TAO. Vincent introduced the concept mid-session: when a client has an outstanding balance, they get sent a consolidated statement covering this cycle's invoice plus every previously unpaid one, merged into ONE PDF, drafted as an email — "SOA 不同于普通的开单，它有自己复杂的一套程序和操作". Investigated before building anything: `email_campaigns.type='soa'` already existed as a real, live-selectable Client Communications campaign type with working candidate-detection (`lib/client-comms-resolve.ts`'s `loadInvoicesByCompany`/`loadAutoTargetNames` already find every customer with `quickbooks_invoices.balance > 0` and aggregate their unpaid invoices) — but only ONE campaign of that type had ever existed ("Historical Import — List_SOA1", a one-time backfill), and there was no PDF-merging capability anywhere in the codebase (confirmed: Vincent's own words, "每次都是要CHELSEA自己一张一张的合并成一个PDF内，其实也花费了大量的时间" — the merge itself is real, just entirely manual). Vincent then shared his team's real, live "Individual outstanding billing" Google Sheet — an A/R Ageing Summary (Current/1-30/31-60/61-90/91+ buckets) with per-QB-company (TAB/TAC/TAO) and per-PIC/staff tabs — confirming the real operational shape to design toward.

A real credential bug surfaced and got fixed along the way: local `.env.local`'s `QB_CLIENT_ID`/`QB_CLIENT_SECRET` had two single-character typos (an 's' read as 'a', an 'l' read as capital 'I' — both classic visually-ambiguous character swaps) that had silently sat there since some earlier transcription, invisible until this session's first-ever local token refresh attempt (every prior script call this session happened to hit the non-expired fast path). Fixed by copy-pasting the real values directly from Intuit's own Keys & Credentials page (Vincent confirmed which "Production" pair was current) — verified with a real OAuth refresh (200, real rotated tokens) and manually re-synced all three of TAB/TAC/TAO's `quickbooks_tokens` rows so nothing was left holding a now-invalidated refresh token. This is a LOCAL dev-environment-only fix — Vercel's production credentials were never wrong, confirmed by every real production operation succeeding all session. Along the way, confirmed via QuickBooks' own live `Invoice.DueDate` field and its native `AgedReceivables` report (same TAB/TAO connections) that the exact bucket boundaries Vincent's spreadsheet uses ARE QuickBooks' own standard report, computed off Net 7 terms (DueDate = TxnDate + 7, confirmed on every real sample) — so the aging math needed no invented business rule, just a documented, verified fact.

Implemented: `lib/soa.ts` (new) — `agingBucket()`/`dueDate()`/`AGING_BUCKETS`, computed locally from the already-synced `quickbooks_invoices` table (not a live QuickBooks report call per page load — same bucket definition, confirmed identical, without the external round-trip or a live-token dependency just to view the list). `GET /api/billing/soa` — every company with `balance > 0` across TAB/TAC/TAO combined, aged and summed, with `companies.pic` for the collections-owner column real Google Sheet organizes by. `GET /api/billing/soa/detail` — full per-invoice breakdown for one company. `GET /api/billing/soa/pdf` (the actual automation target) — fetches every real unpaid invoice PDF for a company (any of TAB/TAC/TAO) and merges them into one PDF via the newly added `pdf-lib` dependency; verified end-to-end against a real 3-invoice, 2-company case (Kindle Beacon Pte. Ltd., TAB+TAO, 3 separate 1-page PDFs → one real 3-page merged PDF). `app/billing/soa/page.tsx` — same list+modal visual language as TAO/TAB-TAC (`system-list` classes, chevron-expand row, the same fixed-overlay modal shell), with aging-bucket columns instead of service badges. "Draft Email" deliberately reuses the EXISTING recipient/template/campaign infrastructure end to end (`GET .../campaigns/preview?type=soa` for recipient resolution, `POST .../campaigns` for a real tracked draft row, the already-existing "SOA1"/"SOA2" templates) rather than inventing new recipient logic — the only new piece is swapping the merged PDF in as `additional_attachments` while leaving `invoice_refs: []` so `fetchSystemAttachments` doesn't also fetch the individual PDFs on top of it — then opens the existing `OutlookStyleSendModal` for review before send, same as Billing Drafts' own quick-draft flow. Client Communications' pre-existing "Statement of Account" campaign type is untouched and stays available (Vincent: "两个入口并存"). `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: Multi-name column filters — decomposed into individual people. Vincent, from a screenshot of Active Client's "Nominee Dir." filter dropdown showing raw combos like "Li Jianwei, Sherly Gunawan" as its own separate checkbox alongside "Li Jianwei" alone: "filter 那边只需要显示一个人的名字就好，就算这个公司的格子内写着两个人的名字，但是只要对应到我filter 的那个PIC 也要显示出来" — every column-filter dropdown over a field that can hold multiple comma-separated names (ND/Secretary/ACC-TAX PIC/Contact Window via the staff directory, plus raw `directors`/`shareholders`) grouped by the FULL formatted string, so a two-person combo was a wholly separate, unrelated option from either person alone — filtering for "Zhang Dan" could never surface every company he's actually on, only the one exact pairing that happened to match verbatim.

Used an agent to map the real scope first (not guessed): confirmed THREE independent, already-duplicated reimplementations of the same filter-popover pattern exist — `ColumnFilterMenu` (`components/MasterListTable.tsx`, module-private), `ARColumnFilterMenu` (`app/billing/page.tsx`), `DimensionFilterMenu` (`app/reports/page.tsx`'s Explore section) — each with its own code comment already stating the duplication was deliberate ("kept local... rather than promoted to a system-wide primitive for a single caller"), so the fix was applied independently to each rather than forcing a consolidation. Verified against real production data before shipping: `directors`/`nominee_director` genuinely reuse the same person across many companies paired with a different co-appointee each time (e.g. real rows show "Zhang Dan" alongside "Xu Min", "Sun Changjiang", "Feng Jiong", "Song Jianfeng" — all different companies) — confirming this wasn't a hypothetical edge case.

Implemented: new `formatStaffNameList()` in `lib/staff-directory.ts` (`formatStaffName()` now just `.join(', ')`s it — byte-identical output, zero behavior change for existing callers) returns each co-assigned person as a separate array entry instead of one joined string. `components/MasterListTable.tsx` gained `MULTI_NAME_RAW_FIELDS` (`directors`/`shareholders` — real people, never staff-directory-resolved, comma-split only) and a new `displayFieldValues()` (plural) used by `ColumnFilterMenu`'s option-counting and the `columnMatch` row filter — every other column still returns its existing single value as a one-element array, so non-multi-name columns are byte-for-byte unchanged. `app/billing/page.tsx` got the identical treatment (`arColumnValues()` alongside the existing `arColumnValue()`) for AR Reminder's PIC/ACC PIC/TAX PIC filters. `app/reports/page.tsx`'s `pic` dimension was deliberately NOT touched in this pass — its single `value: (r) => string` function also drives the pivot table/chart grouping, not just filtering, so decomposing it is a separate, larger change; documented as still-open in `docs/INVARIANTS.md`'s new INV-PIC-006. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: TAO description field — show the full text, not a scrolling box. Vincent, from a screenshot: "TOA 这边如果description 太长不要隐藏起来要完全的展示" — the Description cell was a fixed `rows={2}` `<textarea>`, so longer real descriptions (period text, disclaimers) scrolled internally instead of being visible. Swapped in a local copy of `app/billing/page.tsx`'s own `AutoTextarea` (not exported, same reuse pattern already used for `TaoInvoiceRef`) — grows its height to fit content via `scrollHeight`, `overflow:hidden`/`resize:none` since it's never clipped. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: Brand-new companies — QuickBooks customer creation + a TAO "Add new company" side door. Vincent: "假设我今天要在TAB/TAC/TAO 开单一个没有过往记录，又或者是一个完全全新的公司，我相信QB本来就有这个最基础的功能" — asked whether this system could invoice a genuinely new client end-to-end. Researched with an agent before answering (not guessed): (1) `findCustomer()` (`lib/qb-invoice-conventions.ts`) only ever looks a QuickBooks Customer up — a repo-wide grep found no code anywhere that creates one; a miss hard-fails invoice generation with "Customer not found in QB {company}". (2) TAB/TAC's Billing Drafts has a bypass for a company not yet in `companies` — AR Reminder's own "Add Manual" (free-text `entity_name`, no `company_id` FK) — but that only gets a template draft row; the QuickBooks Customer must still already exist for the final "Generate Invoice" step to succeed. (3) TAO's list (this session's own new work) has no such bypass at all — its eligibility is drawn entirely from `companies` + real QB history, and `companies` itself is populated only by TeamWork sync, with no manual "add a company" route anywhere in the codebase. Gave Vincent this precise picture (correcting his own restated summary: the real gate is "does this company exist as a QuickBooks Customer," not "is it in TeamWork" — TeamWork only gates one input into TAB/TAC/TAO's own candidate-list logic, not invoice generation itself), then he asked for the fix: bring real "create a QuickBooks customer" capability into TAB/TAC and TAO, plus a way to add a genuinely new company to TAO's list. Confirmed scope with him: minimal customer creation (DisplayName only — everything else filled in later directly in QuickBooks), and a dedicated "Add new company" entry point on the TAO page.

Implemented: `createCustomer()` (`lib/qb-invoice-conventions.ts`) — a minimal POST to QuickBooks' Customer endpoint, with QuickBooks' own "Duplicate Name Exists" rejection acting as a safety net against accidentally creating a second record for a company that already exists under the same name. New shared `POST /api/quickbooks/create-customer` (`company` + `companyName`), gated behind the same approved-account auth as `create-invoice`, with one more live `findCustomer()` check immediately before creating to narrow the race window. `app/billing/tao/page.tsx`'s `TaoInvoiceBuilder`: generation failures matching "Customer not found in QB" now surface a "Create ... in QuickBooks" button inline in the error banner that creates the customer and automatically retries generation. `app/billing/page.tsx`'s `ExpandedBillingRow` (TAB/TAC) gets the identical treatment, tracked per company since a combined TAB+TAC generation can fail on either or both independently. New `POST /api/billing/tao/route.ts` handler — TAO's own "Add new company" side door: minimal `companies` insert (`company_name` + `has_accounts: true`, every other column nullable — confirmed via a real dry-run insert/delete against production before writing this), with an exact-then-fuzzy duplicate guard against the existing `companies` roster so a mistyped search can't quietly create a shadow duplicate. `app/billing/tao/page.tsx` gained an inline "+ Add new company" form next to the search bar, opening the new company's builder modal directly on success. Not live-tested against real production QuickBooks (creating a real customer record isn't something to do just to verify code) — flagged to Vincent to try once on a real case before relying on it. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: TAO — merged the "previously billed services" checklist into the line-items table itself, with description periods rolled forward. Vincent, right after seeing the checklist-above-empty-table version: "为什么要分开 之前把previous 的变成下面那个services 的就好了啊，和TAB/TAC的那样...唯一不同的是TAB/TAC会把服务勾选好，description 的周期填写好这个周期的月份，TAO的也是判断出之前的所有服务，但是不主动勾选，周期...根据判断放最新的年份或者月份，但是上一次开单相关服务日期还是要显示" — three corrections in one message: (1) one merged table, not a checklist sitting above a separate empty one; (2) TAB/TAC's real table already has an include/exclude checkbox per row (re-checked `app/billing/page.tsx`'s `renderTable` — it does, `l.include`, greyed out when unticked) which the previous TAO pass had dropped entirely on the mistaken assumption every manually-added line was meant to be included; TAO's rows should have the same checkbox, just starting UNCHECKED (no due-date signal here to justify TAB/TAC's checked-by-default); (3) the description text should have its date/period rolled forward one cycle as a starting guess, while the actual last-billed date stays visible per row for reference ("目的是为了让用户知道上一次开单是什么时候，这次还要不要开单").

For (3), reused `rollRecurringDescriptionForward()` (`lib/invoice-period.ts`) directly rather than writing new date-rolling logic — it's generic text-pattern rolling (FYE/YE/YA year bumps, "Month YYYY to Month YYYY" range bumps, "year ended DD Month YYYY" bumps), not tied to Secretary/Address specifically. Verified against 5 real TAO description strings before shipping (via a throwaway tsx script) — all rolled correctly, e.g. "(YA2026)" → "(YA2027)", "Apr 2026 to Jun 2026" → "Apr 2027 to Jun 2027", "year ended 31 December 2025" → "...2026". `Line` gained `include: boolean` (defaults `false` for history rows, `true` for "Add line" rows) and `lastBilled: string | null`; `historyKey`-based tracking removed entirely since the checklist and the table are now the same rows. Row rendering restored the checkbox column and greyed-out/opacity treatment TAB/TAC's own table uses, with a small "Last: DD Mon YYYY" / "New service" caption under the service label taking Status's place. Total/validation/generate now filter by `included = lines.filter(l => l.include)`, matching TAB/TAC's exact pattern. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: TAO — "previously billed services" checklist. Vincent asked whether TAO could do what TAB/TAC does — look at past invoices to judge what should be billed this cycle. Answered directly first: no, not currently, and that was his own explicit decision earlier this session (manual builder, no due-date detection, since Accounts/Tax has no periodicity data model and real amounts are individually negotiated). Asked whether he wanted the full period-rolling treatment anyway — he dismissed that question, then reframed with a smaller, well-scoped ask: "虽然是没有像TAB/TAC那样分到那么细，但是好歹应该也可以判断上一个单", then refined further mid-turn to "不管周期，是判断之前开过的所有服务，然后用户才来自己打勾自己要开的单" — not a single "last invoice," and not any due-date logic at all: surface EVERY distinct service this company has ever been billed for via real TAO invoices, and let ACC tick which ones apply this time.

Implemented as a genuinely new, much smaller mechanism than TAB/TAC's renewal cycle — no period parsing, no "due" computation, purely presence-in-history: new `app/api/billing/tao/service-history/route.ts` (GET, `?companyName=`) pages through `quickbooks_invoice_items` for that company (same normalized-name-then-fuzzy-match pattern as the rest of this feature), dedupes by `product_service` keeping the most recent occurrence's description/rate/qty/date. `app/billing/tao/page.tsx`'s `TaoInvoiceBuilder` renders these as a checklist above the line-items table — ticking one appends a line prefilled from that history (tagged with a `historyKey` so unticking removes exactly that line, without disturbing lines added via the separate "Add line" catalogue dropdown); a history item whose `product_service` matches a curated `TAO_PRODUCTS` entry shows that entry's clean label, otherwise the raw QuickBooks item name. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: TAO "Add line" — matched to TAB/TAC's actual catalogue-dropdown behavior, not just its look. Vincent, pointing at a screenshot of the footer bar: "TAO 的这部分不能和 TAB/TAC的一样吗？" — the previous pass had matched the VISUAL chrome of the "Add line" bar but not its actual interaction: TAB/TAC's `QB_CATALOG` dropdown is picked ONCE (grouped by category via `<optgroup>`, value resets to "" after each pick) and appends a fully-formed line; the per-line Service cell is then a fixed label, not a re-editable dropdown. The TAO version had it backwards — a per-line dropdown letting the service be changed after the fact, and a plain "Add line" button that added a blank, unconfigured row. Reworked to match exactly: `TAO_PRODUCTS` gained a `category` field (Accounts/Tax/Disbursement/Other, mirroring `QB_CATALOG`'s categories), `Line` no longer carries a `productOption` index — the row's first cell is now a fixed label matching TAB/TAC's `svcLabel` styling, and the footer bar's `<select>` (grouped by `<optgroup>`, "Custom / Other…" left ungrouped same as before) appends a new line pre-filled with that item's description on pick, then resets to the placeholder. The builder now starts with zero lines (there's no suggested-draft data to seed from, unlike TAB/TAC) rather than one arbitrary default line, and any line — including down to zero — can be removed, with Generate requiring at least one. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: TAO invoice builder — matched to the REAL TAB/TAC builder's visual language, not just its outer modal shell. Vincent, right after the list+modal rework: "这边的UI设计可以再顺畅舒服一点吗?...没有顺序，没有层级" then "INVOICE 那边也没有参考 TAB/TAC的弹窗UI" — the modal's outer shell already matched, but the builder form inside it (email/date row, line items) was still a flat, ungrouped set of inputs I'd designed myself rather than reading what TAB/TAC's own builder actually looks like. He then sent a real screenshot of the live TAB/TAC builder ("之前的UI就很不错" — that one's good) confirming exactly what reading `ExpandedBillingRow`'s JSX in `app/billing/page.tsx` had already turned up: a header row (email icon+input, invoice date), a section-badge row (company pill + title + an "Estimated QB Number" box with its own label/input/refresh icon, right-aligned via `marginLeft:auto`), a bordered table with an uppercase grey header row (Service/Description/Status/Qty/Rate/Amount) and per-row computed Amount, an "Add line" bar attached directly under the table (no gap, continuing its border), then a bottom bar ("N lines · Total $X" + a green "Generate Invoice in QB" button) and a small grey disclaimer line about the invoice being created as an unsent QB draft. Rebuilt `TaoInvoiceBuilder` to this exact shape — same shared `inputStyle`, same number-box styling/copy, same table chrome and footer bar, same bottom-bar phrasing and disclaimer — dropping only what genuinely doesn't apply to TAO (no per-line include/exclude checkbox, since every manually-added TAO line is meant to be included; no Status column, since that tracks Secretary/Address/ND renewal-period due-dates which don't exist for Accounts/Tax; no PIC/Class note, since TAO invoices never carry one). `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: TAO billing page — rebuilt to match Billing Drafts' own list+modal UI. Vincent, after using the page: "TAO的UI不能做到像TAB/TAC 那样的类似UI吗？...点开弹窗，弹窗的UI也要和 TAB/TAC类似" — the original TAO page was a plain list + separate full-page builder view; he wanted the exact Billing Drafts interaction pattern instead. Read `app/billing/page.tsx`'s real structure closely rather than guessing: its list uses `.system-list-shell`/`.list-column-header-gray`/`.system-list-row` rows with a `ChevronRight`/`ChevronDown` toggle (`expanded === c.companyId`), and clicking a row doesn't expand inline — it opens a genuine modal (`expanded !== null` renders a fixed-position dark overlay with a centered white card, navy gradient header showing company name + context badges + an X close button, body is `<ExpandedBillingRow>`). Rebuilt `app/billing/tao/page.tsx` to the identical shape: same list-row/grid-column CSS classes, 3 `MetricCard`s (reused as-is) reinterpreted for TAO's reality (no FYE cycle, so "Never Billed via TAO" / "Billed Before" replace "Needs Billing this cycle" / "Invoiced this cycle"), `usePagination`/`PaginationBar` (not present in the first version — 806 rows needed it), and the same fixed-overlay modal with a navy gradient header, rendering the existing line-item builder (extracted into its own `TaoInvoiceBuilder` component) as the modal body instead of a separate page section. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: Mojibake customer names — real QuickBooks-side duplicate-customer records, fixed durably at sync time, not just patched once. Vincent spotted a garbled Chinese company name on the new TAO billing page ("¼ªÄ¾Ð¿¹ú¼ÊÃ³Ò×..."). First pass: confirmed the corruption lives in `quickbooks_invoices`/`quickbooks_invoice_items.customer_name` itself (not introduced by the new page's rendering), scoped it to exactly 4 distinct real Chinese company names (GBK bytes misread as Latin-1, then stored) across TAC (8 invoice rows) and TAO (1 row) — `companies` table itself is clean, 0 affected. Decoded all 4 with high confidence (one, 天马微电子股份有限公司/Tianma Microelectronics, is a real well-known listed company, confirming the GBK-reversal was correct) and, per Vincent's choice, corrected the stored rows directly.

Then went one step further before considering it done: checked whether tonight's automated daily sync (`quickbooks_invoices`/`quickbooks_invoice_items` are upserted keyed by `qb_company,qb_invoice_id`, always re-reading `customer.name` fresh from QuickBooks) would silently revert the fix. It would have — queried the live QuickBooks TAC and TAO customer lists directly (paginated, not just the first 1000) and found the real root cause: QuickBooks itself holds TWO customer records per affected company, one correctly named and one with the corrupted name (e.g. TAC customer 362 and TAO customer 1697 both duplicate the correctly-named customer "吉木锌国际贸易（上海）有限公司") — a QuickBooks-side duplicate-customer data-hygiene issue, not a bug in this app's sync code. Asked Vincent how to handle the QuickBooks-side duplicates directly (merge via QBO's own UI, merge via API rename, or leave QuickBooks alone and correct only our own copy) — he chose the last option, not wanting to touch QuickBooks' real customer/transaction records for this. Implemented durably: new `correctedCustomerName(company, qbCustomerId, rawName)` in `lib/quickbooks.ts`, keyed by the stable `qb_customer_id` (never the free-text name, which IS the corrupted value) via a small `CUSTOMER_NAME_CORRECTIONS` map — called from BOTH `customer_name` write sites that exist in this codebase: the full-year sync (`app/api/quickbooks/sync/route.ts`) and the incremental webhook sync (`lib/quickbooks-invoice-incremental.ts`), so neither path can silently re-introduce the garbled text on a future sync. `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: TAO billing page — fixed a real "list is basically empty" bug found immediately after shipping. Vincent tested `/billing/tao` live and saw exactly ONE company. Root cause: the eligibility query relied on `companies.has_accounts`/`services_manual.accounts` as if those were the same merged signal `app/api/ar-reminder/route.ts` computes for its own "has Accounts" flag — they are not. Verified directly: `has_accounts` is a rarely-set MANUAL OVERRIDE column, `true` for exactly 1 of 945 companies; the real signal ar-reminder actually uses is live-computed from real QuickBooks invoice history (`qbSvcs.has('Accounts') || compMatch?.has_accounts`), which this new route never replicated. Rebuilt `app/api/billing/tao/route.ts`'s eligibility set as the union of: (1) any company with an Accounts/Tax `quickbooks_invoice_items.service_type` in the last 3 years — same window/logic as ar-reminder — across any QB company, not just TAO; (2) any company with real TAO invoice history at all (a pure safety net so ACC's actual existing client base always shows up even if service_type tagging is imperfect); (3) the manual override, for a genuinely new client with no invoice history yet. Verified against real data before shipping: 806 eligible companies (vs. the broken version's 1), of which 489 match a `companies` row by exact normalized name and the rest resolve via the same fuzzy `wordMatch` fallback ar-reminder uses (`lib/company-name.ts`'s `findUniqueBestMatch`) or fall back to the real QuickBooks customer name with `companyId: null` (still billable — `create-invoice`'s parent-company lookup already tolerates a null companyId). `npx tsc --noEmit` and `npm run build` both clean.

Previous update, same day: TAO Phase 2 core — ACC gets its own real invoice-generation page. Vincent laid out the real scenario Phase 1 deliberately deferred: TAB/TAC are billed by Chelsea, TAO (Accounts/Tax) should be billed independently by ACC — except when a client is a payment-risk case, where ACC needs Chelsea's help, which turned out (after Vincent caught his own initial misreading mid-conversation) to mean bundling Accounts into Chelsea's existing TAB invoice rather than generating a separate TAO one — a mechanism that already exists today as `app/api/billing/renewals/route.ts`'s automatic TAB-Accounts carry-forward (verified real: 107 historical TAB line items, most recent 2026-06-15), just not yet conditional on any ACC-set flag. Before building anything, directly answered Vincent's explicit safety question — "那107笔的记录TAB和TAO实际是有冲突记录的吗？" — with a real data check, not an inference: classified all 62 companies with a TAB-Accounts line item by how close their nearest TAO invoice date falls (20 never touched TAO, 22 more than 90 days apart, 8 within 31-90 days, 12 within 30 days including a few same-day pairs), then pulled the actual line-item descriptions for the closest cases. Result: no genuine double-billing — TAB's Accounts line is the annual Compilation service bundled with Chelsea's Secretary renewal (or a strike-off closing package's tax-clearance step); TAO covers ACC's own ongoing, much broader Corporate/Personal Tax, GST, AIS, IR21, and monthly/quarterly bookkeeping work — different scopes, even on the two cases that shared an exact invoice date. This cleared the way to build the TAO billing page now and defer the "flag for Chelsea" conditional-carry-forward mechanism to a later phase, per Vincent's own sequencing ("先做核心部分，标记功能后续再补") — safe specifically because no live conflict exists to guard against yet. Also resolved two remaining Phase 1 business-rule deferrals with real evidence instead of guesses: the TAO billing page is a manual line-item builder, not an automated due-date list, because Accounts/Tax services have zero renewal-cycle tracking anywhere in this codebase and real TAO invoice amounts are individually negotiated per client (confirmed by inspecting real line items — quarterly/monthly bookkeeping fees vary $500-1500, nothing templated); and the DocNumber series digit is "6", not invented but read off ACC's own real pre-existing manual QuickBooks numbering (02660650, TAO02560838, etc. all decode to yy+6+seq).

Implemented (6 files + 1 migration): `lib/qb-invoice-conventions.ts`'s `nextDocNumber` binary ternary (`company === 'TAB' ? '1' : '8'`) replaced with an explicit `DOC_NUMBER_SERIES` lookup that throws on an unrecognized company rather than silently defaulting — same INV-QB-011 discipline as Phase 1's OAuth route fixes; PIC/Class needed no code change at all, since `requiresPicClass()` only ever flags Secretary/XBRL lines and TAO invoices only ever carry Accounts/Tax lines, so they end up classless (like TAC) automatically. `app/api/quickbooks/create-invoice/route.ts` extended with a third parallel `taoLines` path alongside the existing hardcoded `tabLines`/`tacLines` at every one of the previously-mapped touch points (request destructuring, validation, `activeCompanies`, the reservation-update tuple loop, `toRecord`, and all 5 hand-written response fields) — `createInvoiceInCompany()` itself needed no change, already generic over `QbCompany`. New migration `scripts/add-tao-invoice-reservations-support.sql` (run in the Supabase SQL editor before this goes live end-to-end) widens `invoice_creation_reservations`'s CHECK constraint from `('TAB','TAC')` to include `'TAO'`, using the same dynamic `pg_constraint` lookup pattern as `add-multi-company-qb-support.sql` since the constraint name is auto-generated. `app/api/quickbooks/next-invoice-numbers/route.ts` extended to return a TAO suggestion alongside TAB/TAC. New `app/api/billing/tao/route.ts` (GET) lists eligible companies from `companies.has_accounts`/`services_manual.accounts` (confirmed the single authoritative signal, NOT `master_list.acc_active`, which is separately unsynced) with each one's last real TAO invoice as on-page context only. New `app/billing/tao/page.tsx`: a standalone company-picker + manual line-item builder (curated real TAO product/service names for exact QuickBooks Item matching, plus a same generic fallback path as TAB/TAC) — deliberately built as its own simple component rather than wedged into `app/billing/page.tsx`'s existing FYE-cycle-driven tab state machine, which stays completely untouched. `components/Sidebar.tsx`'s "Billing Drafts" leaf converted into a two-item group (mirroring the existing "Client Communications" pattern) — "TAB / TAC" (unchanged href) + new "TAO" leaf, no `restrictedTo` (Vincent: "这个暂时先不限制，所有人都看得到"); confirmed `isActive()` needed no change since `/billing/tao` is a distinct pathname, not a `?tab=` variant of `/billing`. `npx tsc --noEmit` and `npm run build` both clean.

**Explicitly deferred, per Vincent's own sequencing**: the "flag for Chelsea" mechanism itself (a new `companies` boolean, naming TBD when this phase starts) and making `app/api/billing/renewals/route.ts`'s TAB-Accounts carry-forward conditional on it instead of unconditional — safe to defer only because this session's real-data check found no live conflict today; if that ever changes (e.g. a genuinely overlapping case shows up), this can't keep waiting.

Previous update: 2026-09-04 (TAO — third QuickBooks company, Phase 1 (connect + sync only). Vincent: "现在有新的公司 TAO" — a real third QB account he's never connected to this system, confirmed as being for Accounts/accounting invoicing specifically ("TAO 是来开会计的单的"), no historical data to import (unlike TAC's 2026-07 rollout, which backfilled real history — TAO is genuinely new, nothing to backfill). This wasn't the first mention: `PROJECT_STATUS.md`'s own 2026-07-19 entry already named TAO (found via the team's manual `BULK.xlsm` bulk-email workbook, which already referenced TAB/TAC/TAO) and left a known gap — Client Communications' AR/SOA totals, PDF downloads, and "add a company" lookup only ever saw TAB/TAC. Used 3 Explore agents to map every hardcoded-to-two-companies spot across (1) OAuth/token/sync/webhook, (2) invoice generation/display, (3) how Accounts billing specifically works within today's TAB catch-all — all three reports full and precise, file:line level. Asked Vincent 2 business-rule questions the code couldn't answer safely by inference (TAO's DocNumber series digit; whether TAO needs PIC/Class like TAB) — he redirected: "先不需要太复杂先，我现在要考虑的是接API的流程" (don't need it complex yet, just want to get the API connection working) — so scope was deliberately cut down to Phase 1 only: connect TAO via OAuth and let its data start syncing, with every invoice-*generation* routing decision (Billing Drafts' TAB/TAC split, DocNumber series, PIC/Class, per-staff QB Location, `invoice_creation_reservations`' CHECK-constraint migration) explicitly deferred to Phase 2, not touched. Plan approved via ExitPlanMode before implementation.

Implemented Phase 1 (15 files): `lib/quickbooks.ts`'s `QbCompany` type widened to `'TAB' | 'TAC' | 'TAO'` (all three share one Intuit OAuth app/credentials — confirmed via the explore report, no new Intuit Developer app needed, TAO is just a third company realm authorized against the same app). Three OAuth routes (`auth`/`callback`/`status`) fixed from a real, previously-shipped class of bug — `company === 'TAC' ? 'TAC' : 'TAB'`-style binary ternaries that would have silently resolved any future `?company=TAO` to TAB with no error — to explicit validation (missing param defaults to TAB for backward compatibility with old links; a present-but-invalid value is rejected outright, never silently folded into TAB). Same fix applied to `invoice-pdf/route.ts` (was silently serving TAB's PDF for any non-TAC company param — a real bug that would have returned the *wrong company's actual invoice PDF* with no error once TAO existed, now blocked with a 400 instead). `components/QBConnectButton.tsx` gets a third TAO badge, mirroring TAB/TAC exactly. Daily sync cron (`app/api/quickbooks/sync/route.ts`) and webhook realm-resolution (`lib/quickbooks-webhook-queue.ts`) both extended to include TAO — same "skipped silently, reported as a soft error, not fatal to the run" treatment TAC already got before it was connected. Client Communications' three known TAO gaps closed for real (not just typed as `| 'TAO'` in the surface types, which two of the three files already were, cosmetically, since 2026-07-19): `lib/client-comms-resolve.ts`'s AR/SOA invoice queries never actually filtered by company at the SQL level, only the TypeScript casts were mistyped — widened to the real `InvoiceRef['qbCompany']` type; `campaigns/page.tsx` and `history/page.tsx`'s explicit `ref.qbCompany === 'TAO'` branches that forced every TAO invoice into "attach manually" regardless of whether a real `qbInvoiceId` existed removed, same criterion (`!qbInvoiceId`) as TAB/TAC now; `draft-helper-client.ts`'s Outlook-attachment filter and `refresh-amounts/route.ts`'s live-amount re-check both had the identical TAB/TAC-only exclusion, both fixed the same way. `lib/invoice-filename.ts` (the shared PDF filename convention used by Client Communications' download flow — NOT `app/billing/page.tsx`'s own intentionally-separate duplicate copy, which stays TAB/TAC-only as genuinely Phase 2 territory) — the `company === 'TAB' ? 'INV' : 'TAC'` binary ternary would have labeled every downloaded TAO PDF "TAC...", fixed to `company === 'TAB' ? 'INV' : company` (TAO gets its own literal "TAO" prefix) — a real bug caught only because widening the Client Communications download path to include TAO would have actually exercised it; this is a filename label, not a billing-numbering decision, so it didn't need to wait for the DocNumber question Vincent deferred. `lib/approved-accounts.ts`'s `qbLocations` key type widened to include `'TAO'` purely to keep `create-invoice/route.ts` type-checking against the now-3-way `QbCompany` — a type-only change, no staff has a TAO Location value set, real Location assignment is genuinely Phase 2. `npx tsc --noEmit` and `npm run build` both clean — one real collateral type error surfaced and fixed along the way (the `qbLocations` one above), confirming the type-widening approach actually catches every call site that needs attention rather than silently compiling around gaps.

**Still needs (Phase 1 completion)**: Vincent to actually authorize TAO via `/api/quickbooks/auth?company=TAO` (Dashboard → QBConnectButton's new TAO badge → Intuit OAuth consent, logged in as whichever QuickBooks account is TAO's real Company Admin — same TAC-rollout lesson: Vincent's own Intuit account was not TAC's Company Admin, "You're not a listed admin", had to use the actual admin's login). Once connected, confirm the next daily sync (or a manual `/api/quickbooks/sync` POST) actually pulls TAO's real invoice history into `quickbooks_invoices`/`quickbooks_invoice_items`, and that Client Communications' Campaign Centre/History no longer show TAO invoices as amber "attach manually" once they have a real `qbInvoiceId`. **Phase 2 (deliberately not started)**: routing NEWLY GENERATED invoices to TAO from Billing Drafts — needs Vincent's decisions on the DocNumber series digit, PIC/Class policy, and per-staff TAO Location names, plus the `invoice_creation_reservations.qb_company` CHECK-constraint migration (currently hard-blocks `'TAO'` at the database layer) and `create-invoice`/`update-invoice`/`invoice-lines` routes' hardcoded tab/tac-named-field request/response shape.)

Previous update: 2026-09-04 (Company 360: every remaining table-shaped section converted to a real equal-width grid, plus Invoices sorted by date. Two separate requests, same session: (1) Vincent: "这边的排序要按照date排：最新的年费日期在最上面以此类推往下排序" — neither of Invoices' two tables was sorted at all before, just whatever order the query returned; added a shared `byDateDesc(field)` sort helper (`app/companies/[id]/_components.tsx`) applied before rendering — "This system's invoice records" sorts by its own Cycle (`fye_cycle`, the billed FYE period — "年费日期"), "QuickBooks history" by its own Date (`txn_date`, when the invoice was actually raised), since the two dates mean different things. `fye_cycle` is "dd.mm.yyyy" text, not lexicographically sortable, so it goes through the same `toIsoDateValue()` parser already used for this exact class of problem (Strike Off/Terminated's `update_date` sort). (2) Vincent, immediately after: "这边也是，5列就以5等分对齐，6列就按6等分对齐" — generalized the grid-alignment technique (already proven on AR/AGM Cycles/Communications/Officials earlier the same day) to every remaining `<table>`-based section: `ShareholdersSection` (6 cols), `InvoicesSection`'s both tables (6 cols each), `NdSection` (5 cols), `TrademarkSection` (4 real columns — Application No./Filed/Expires/Status — with its match-badge inlined into Status rather than becoming a 5th column, same treatment AR/AGM Cycles' Remarks cell already got). Renamed the two shared grid constants from their originally narrow, now-misleading names (`CARD_GRID_COLS`/`AR_AGM_GRID_COLS`, each implying a single owning section) to generic `GRID_5_COLS`/`GRID_6_COLS`, plus a new `GRID_4_COLS` for Trademark — all defined once, reused across every section that needs that column count. `DocsGeneratedSection` (unused/unrendered since Vincent removed it from the page 2026-09-03) deliberately left as its original `<table>` — not worth converting dead code. `npx tsc --noEmit` and `npm run build` both clean.)

Previous update: 2026-09-04 (Company 360: module order swapped again + Officials converted to a real grid too. Vincent, immediately after the SEC PIC restoration: "模块排序交换一下 Communications 变去 第2，AR / AGM Cycles变去第3" — swapped `app/companies/[id]/page.tsx`'s render order (Communications now directly after the header, AR/AGM Cycles third). Then: "AR / AGM Cycles 6等分，就变成和Officials模块的6等分可以对齐了" — Officials (now the section directly below AR/AGM Cycles) was still the original `<table>` with UNEVEN colgroup percentages (22/16/16/14/16/16, never actually "6 equal" despite 6 columns) — per the table-vs-grid lesson from earlier the same day, that could never genuinely align with AR/AGM Cycles' new real `repeat(6, minmax(0,1fr))` grid no matter how the percentages were tuned. Converted `OfficialsSection` to the identical grid formula (reusing `AR_AGM_GRID_COLS`) so the two adjacent sections' columns actually line up, not just look close. `npx tsc --noEmit` and `npm run build` both clean.)

Previous update: 2026-09-04 (SEC PIC restored to AR/AGM Cycles, with real data behind the decision. After dropping AR/AGM Cycles' PIC column earlier the same day (Vincent: header already shows it), Vincent then asked why the header's own Secretary PIC went blank on a real company right after — investigated and found `companies.sec_pic` (header) and `ar_reminder.pic` (the dropped column, this specific cycle's actual handler) are genuinely different fields, not a display quirk. He asked to check how often they actually diverge before deciding anything: ran a real comparison script (`scripts/diag-pic-comparison.ts`, tsx, deleted after use, reusing the real `formatStaffName()` normalization rather than a naive string-equality heuristic) across all 808 companies with both a `companies` row and an `ar_reminder` row — 78% consistent, but a genuine 20% (159 companies) show a real, different person on each side, plus a further 1% where the header's own field is empty while the cycle's own PIC has a real value. Confirmed via samples this isn't a formatting mismatch (e.g. "Jenny" vs "Jenny Lai") — real different names ("Chin Kah Ye" vs "Tey Shemin", etc.) — most likely PIC handoffs that never made it back into `companies.sec_pic`. Vincent's call: "(AR / AGM Cycles) 中的SEC PIC 还是要保留" — restored as its own labeled column. This makes 6 real columns (Due Date/Filed/SEC PIC/ACC PIC/TAX PIC/Remarks) instead of 5, breaking the just-finished alignment with the header card's 5-column grid — asked Vincent directly how to handle that tradeoff (merge two columns to stay at 5, hide SEC PIC inline like the match badge, or just go to 6 equal columns and drop the header-alignment constraint for this section only) rather than guessing; he picked 6 equal columns, no longer aligned with the header. `app/companies/[id]/_components.tsx`: new `AR_AGM_GRID_COLS = 'repeat(6, minmax(0,1fr))'`, scoped to `ArAgmSection` only — `CommsSection` keeps the original `CARD_GRID_COLS` (5 columns, still aligned with the header, since this request never touched Communications). `npx tsc --noEmit` and `npm run build` both clean.)

Previous update: 2026-09-04 (Communications section: wrap instead of truncate. Vincent, from a real screenshot right after the grid-alignment fix went live: "隐藏的内容往下行展示" — the Subject cell's `nowrap`+ellipsis was cutting off long text behind a title-only tooltip. Removed the `overflow:hidden/textOverflow:ellipsis/whiteSpace:nowrap` truncation from all three text-heavy cells (Campaign/Subject/To) in `CommsSection` so long values wrap onto additional lines instead of being hidden; row `alignItems` switched from `center` to `start` since row height now varies with wrapped content. Scoped to Communications only — AR/AGM Cycles' Remarks cell still truncates with a tooltip, since Vincent's screenshot/request was specifically about this section. `npx tsc --noEmit` and `npm run build` both clean.)

Previous update: 2026-09-04 (Company 360 AR/AGM Cycles + Communications — table replaced with a real CSS grid to actually align with the header card. Vincent, after the previous colgroup-percentage attempt: "上下没有对齐，第2和第3模块要和 第1模块对齐" (screenshot showed real misalignment — module 2/3's column starts didn't line up with module 1's UEN/Status/etc.). Root cause: the header card's own field grid uses `display:grid, gridTemplateColumns: repeat(5, minmax(0,1fr)), gap: 16` — a `gap` genuinely reserves space between columns, so a `<table>`'s colgroup percentages (no gap concept, only per-cell padding) can never reproduce the exact same column boundaries no matter how carefully the percentages are tuned; this is the same lesson already learned earlier in this session for the header's own internal row alignment (six iterations concluding "one shared grid, not reverse-engineered percentages"). Rebuilt `ArAgmSection` and `CommsSection` (`app/companies/[id]/_components.tsx`) as div/grid rows using the literal same `repeat(5, minmax(0,1fr))` + `gap: 16` formula (extracted as a shared `CARD_GRID_COLS` constant) with `padding: '10px 16px'` per row so column 0 starts at the same 16px inset as the header card's own field grid — not a new technique, the same div-grid-row pattern `MasterListTable.tsx` already uses elsewhere, just applied to this page for the first time. AR/AGM Cycles' match badge (no header label) now sits inline at the end of the Remarks cell instead of being a 6th grid column, since a 6th column would shrink the other 5 relative to the header's 5-column grid and reintroduce the same misalignment. Confirmed `.list-column-header-gray`/`.system-list-row`'s own CSS work correctly on plain `<div>`s (not table-specific) before relying on them — `.list-column-header-gray > *` targets any child, and `.system-list-row`'s border-bottom, per a pre-existing code comment, only ever rendered correctly on div-based rows in the first place, table `<tr>`s needed a per-cell workaround. `npx tsc --noEmit` and `npm run build` both clean.

Also this session: Vincent asked why Secretary PIC on the header card went blank on a real company (BAO FORTUNE SHIPPING) right after AR/AGM Cycles' own PIC column was dropped, assuming the edit had broken it. Verified directly against production data before answering — not guessed: the header's Secretary PIC (`companies.sec_pic`) and AR/AGM Cycles' now-removed PIC column (`ar_reminder.pic`, a per-cycle field) are genuinely two different columns from two different tables that can independently disagree; for this company `companies.sec_pic` is null while `ar_reminder.pic` = "Jenny" — a real, pre-existing sync gap that the removed column had been incidentally masking, not something this change caused (the header's own rendering code was never touched). Flagged as possibly worth a broader audit of how often these two disagree, not fixed unilaterally.)

Previous update: 2026-09-04 (Company 360 AR/AGM Cycles + Communications column layout. Vincent, after the section-reorder change: "FYE 和 PIC 这两个不需要在（AR / AGM Cycles）显示，然后分成5等分列宽和 第一模块的5等分列宽一致" + "Communications...也是分成5等分列宽和 第一模块的5等分列宽一致" (module 1 = the header card's own `repeat(5, minmax(0,1fr))` grid — both already show FYE elsewhere on the header, and AR/AGM Cycles' plain PIC column duplicated Secretary PIC already shown there too). `app/companies/[id]/_components.tsx`: `ArAgmSection` — dropped the FYE and PIC `<th>`/`<td>` pair, leaving exactly 5 real columns (Due Date/Filed/ACC PIC/TAX PIC/Remarks), each set to an equal 18.8% (`colgroup`), with the trailing unlabeled match-badge column kept at its previous small 6% (not counted among the "5" — it was never one of the 8 original columns, purely a status icon). `CommsSection` already had exactly 5 columns (Campaign/Subject/To/Status/Sent); just replaced its uneven percentages (18/32/24/12/14) with 5 even 20% columns. `npx tsc --noEmit` and `npm run build` both clean.)

Previous update: 2026-09-04 (A second, separate MANUALLY INVOICED marker bug — this one on the AR Reminder tab's own Invoice column, not Billing Drafts. Vincent tested again after the billingRemarks/arRemarks field-mismatch fix and re-typed the marker into Wolvez Capital's Remarks (confirmed live: `ar_reminder.remarks` = "MANUALLY INVOICED: TAB #02611029", parses correctly), but the AR Reminder tab's own Invoice column still only showed #02611028 — screenshot showed the Remarks and Invoice columns side by side, which is the AR Reminder tab table, not the Billing Drafts tab this session had been fixing so far. Investigated and found: `ArInvoiceCell` (`app/billing/page.tsx`) reads `r.tab_invoice_no`/`r.tac_invoice_no`, which come from an entirely SEPARATE server-side computation in `app/api/ar-reminder/route.ts`'s `latestGenInvoice()` — real `generated_invoices` only, no marker awareness at all, never touched by either of today's earlier fixes (which only ever touched the Billing Drafts tab's own `invoiceRefsFor()`). Two genuinely different Invoice-column implementations for two different tabs, only one of which had ever been marker-aware. Extracted the marker-parsing logic (previously duplicated once already, now shared) into a new `lib/manual-invoice-marker.ts` (`manualInvoiceOverrides()`), imported by `app/billing/page.tsx` in place of its local copy. Added `arInvoiceRefs(r, company)` — computed client-side from `r.remarks` (already present on every `ARRecord`, no server route change needed) combined with the real `tab_invoice_no`/`tac_invoice_no`, deduped by invoice number, same shape as Billing Drafts' `invoiceRefsFor()`. `ArInvoiceCell` now stacks every ref instead of the single real one. Verified the live marker parses correctly against real production data before shipping. `npx tsc --noEmit` and `npm run build` both clean.)

Previous update: 2026-09-04 (Company 360 section order. Vincent, from a real screenshot of the page: "顺序调整一下，AR/AGM CYCLES 放在第2模块，Communications放在第3模块" — counting the header card itself as module 1 (it renders with the same title-bar styling as every section below it, so that's the natural way to count "modules" on this page). `app/companies/[id]/page.tsx`: moved `ArAgmSection` and `CommsSection` to render immediately after the header card; `OfficialsSection`/`ShareholdersSection`/`InvoicesSection`/`NdSection`/`TrademarkSection` keep their previous relative order, just shifted down. `npx tsc --noEmit` and `npm run build` both clean.)

Previous update: 2026-09-04 (Real bug found and fixed: the MANUALLY INVOICED marker read the WRONG Remarks field, so it could never have worked for any company. Vincent tested the just-shipped multi-invoice-display fix live and reported Wolvez Capital's row still only showed #02611028, not #02611029 — investigated rather than assuming the deploy just hadn't propagated. Root cause: `manualInvoiceOverrides()` in `app/billing/page.tsx` was called with `c.billingRemarks`, which (confirmed by reading `arToBillingRow` and the shared `CompanyBilling` interface's own comment in `app/api/billing/renewals/route.ts`) is sourced from `ar_reminder.billing_remarks` — Billing Drafts' own separate free-typed note column (visible in the screenshot as "check with SX"), deliberately split off from `ar_reminder.remarks` back on 2026-08-17 specifically so a billing note could never collide with the AR Reminder tab's TERMINATED/STRIKE OFF/AR COMPLETED compliance-workflow field. But the MANUALLY INVOICED marker was designed (and, per Vincent's own choice via AskUserQuestion earlier this session) to live in THAT other field — `remarks`, not `billing_remarks` — which nothing in `app/billing/page.tsx` ever read at all; the `ARCompany` type sourcing `arList` didn't even declare a `remarks` property, only `billing_remarks`. This means the marker feature could never have worked for ANY company since it shipped, not just Wolvez — `notInvoicedYet()`'s marker-fallback check had the exact same wrong-field bug, so a company genuinely invoiced only via a manual marker (the feature's original purpose) would have kept showing "not invoiced" forever. Fixed by adding a new `arRemarks` field end-to-end (`CompanyBilling.arRemarks` in the shared route interface, `ARCompany.remarks` sourced from `/api/ar-reminder`'s already-existing `.select('*')` response, `arToBillingRow` mapping `ar.remarks` through) and repointing both `invoiceRefsFor()` and `notInvoicedYet()` at `c.arRemarks` instead of `c.billingRemarks` — `billingRemarks` itself is untouched and still correctly drives the Remarks column's own EditField. Separately, live-verified `ar_reminder.remarks` for Wolvez's FYE June 2026 row (id 690) is currently `null` — the marker text Vincent typed earlier this session is no longer there (cleared at some point, cause unconfirmed) — so even after this fix, that row needs the marker re-typed before #02611029 will actually display; told Vincent this directly rather than silently re-writing it back in for him. `npx tsc --noEmit` and `npm run build` both clean.)

Previous update: 2026-09-04 (Billing Drafts TAB/TAC Invoice columns now show ALL matching invoice numbers, not just one. Direct follow-up to the same-day "MANUALLY INVOICED" marker feature — Vincent, mid-conversation: "同时也要再FYE June 2026...的单号那边除了显示02611028，02611029也要显示出来", confirming his original ask (back when this whole feature started) was actually to show BOTH real invoice numbers on the SAME row for a company billed twice in one cycle (Wolvez Capital: 02611028 generated through this system, 02611029 typed straight into QuickBooks for the NEXT cycle at the client's request) — not to fall back to the manual marker only when no real invoice exists, which is what the original design did. `app/billing/page.tsx`: replaced the singular `latestInvoiceNo()`/`manualInvoiceOverride()` helpers with `invoiceRefsFor(c, company)`, which combines every real `generated_invoices` match for this cycle AND any `MANUALLY INVOICED` marker for that same company code, deduped by invoice number — always both, never either/or. Updated all three render call sites (mobile card, desktop TAB column, desktop TAC column) to stack every matching `BillingInvoiceReference` badge instead of rendering one; the TAC column's existing ND-invoice-history fallback (for when there's no real or manually-noted invoice at all this cycle) is preserved unchanged, now just gated on `refs.length === 0` instead of a single null check. One real type error caught by `npm run build` (not `tsc --noEmit`, which passed clean first — `generatedInvoices[].invoiceNo` is typed `string | null`, not always present) and fixed before shipping. `npx tsc --noEmit` and `npm run build` both clean.)

Previous update: 2026-09-04 (Fixed a real Trademark "In Progress" edit bug — Vincent: "为什么这个trademark的还不能填写内容", and confirmed on follow-up that it happened on the FIRST save attempt right after a fresh reload, every time, ruling out the usual stale-tab/multi-device explanation for the "Someone else already changed this record" conflict alert. Reproduced directly against real production data before touching code (not guessed): `trademark_records.company_name` is the one NOT NULL column in this table — Vincent deliberately allows adding a blank Add-Record row and filling it in later, so a fresh row's company_name genuinely stores `''`, never `null`. `components/TrademarkTable.tsx`'s `saveEdit` used the same `value || null` coercion every other (nullable) field uses, which collapsed that real `''` into `null` when building the compare-and-swap request — but the server's `.is(field, null)` filter can never match a row whose actual value is `''`, so the very first edit of any blank-named row always 409'd, no race involved at all. Verified the exact failure by replaying the frontend+backend logic against the real blank row (id 67) before fixing, then verified the fix resolves it the same way, reverting the test write both times. Fix: `company_name` now gets its own branch on both sides that keeps `''` as `''` instead of coercing to `null` — `saveEdit`'s request body (`components/TrademarkTable.tsx`) and the PATCH route's `coerce()`/CAS filter (`app/api/trademark/route.ts`) — which also incidentally fixes a second latent bug in the same root cause (clearing company_name to blank would have tried to write `null` into a NOT NULL column and 500'd). Added `docs/INVARIANTS.md` INV-DATA-013 (the text-field sibling of INV-DATA-001's boolean null/false CAS case) so this class of bug gets checked before it recurs on another NOT NULL column. `npx tsc --noEmit` and `npm run build` both clean.)

Previous update: 2026-09-04 (Master List "Active" column auto-synced from TeamWork's own Status. Vincent, from a real screenshot of the Strike Off page: SHOGUN SPA PTE. LTD. showed "Active" = "YES" even though TeamWork's own company grid already shows its Status as "Striking Off" — asked whether this column was automated at all. Investigated first (not assumed): `master_list.status` was a plain manually-typed field on every list page that shows it (Active Client/Ad-Hoc/Strike Off/Terminated/Change Co Name) — never in `AUTO_SYNCED_FIELDS`, nightly TeamWork sync never wrote to it. Meanwhile `companies.tw_status` already exists and is already reliably synced nightly (`app/api/teamwork/sync/route.ts`, matches TeamWork's own "Status" column exactly — confirmed real values in production: Active/Terminated/Striking Off/Struck-Off/Liquidation in Progress) and already drives roster-inclusion logic elsewhere (AR Reminder generation, Late Filing, Companies page) — so this needed no new scrape, just wiring an existing field through. Vincent confirmed: "MASTER LIST 这边的 ACTIVE 就是TW里面的 STATUS." Added `status` to `AUTO_SYNCED_FIELDS` (`app/api/master-list/route.ts`) so a manual edit protects a row from future overwrites, same as every other auto-synced field. New sync block in `app/api/teamwork/sync/route.ts` (`statusByRegNo` map built in the main TeamWork loop, written to `master_list.status` by UEN match across every list_type, gated on `!manual_fields?.status`, batched + audit-logged like the existing invoice_address/code/email blocks). Updated `MasterListTable.tsx`'s `statusColor()` to recognize "STRIKING OFF" (red, alongside "STRUCK OFF") and "ACTIVE" (green, alongside "YES") so the badge coloring still makes sense once real TeamWork wording starts appearing. Before shipping, ran a real dry-run against production data (temporary diagnostic script, deleted after use) to check the actual blast radius rather than assuming: 852 of 1595 master_list rows would change on the first sync, 743 have no UEN match in `companies` at all and are correctly left untouched — this matters because those unmatched rows are exactly the ones carrying Vincent's own richer hand-typed strike-off descriptions ("Gazetted To Be Struck Off", "In Liquidation - Members voluntary winding up", etc.), which turn out to be safe by construction: a company that's been fully struck off no longer exists in TeamWork's live roster, so it was never going to be a sync candidate in the first place. Every row that WOULD change is either a stale placeholder ("YES"/"TERMINATED") getting corrected to the real live status, or a minor wording normalization (e.g. "In Liquidation - Members voluntary winding up" -> "Liquidation in Progress"). One genuinely interesting finding surfaced by the dry-run and flagged to Vincent directly: 7 companies filed under Terminated in Master List (XSPY, SINGAPORE CHINESE ARTS CENTER, SATORISYS, HALOFUN, ARK PARTNERS MANAGEMENT, ANABLE MANAGEMENT SERVICES, SINO MINING HEAVY INDUSTRIES) still show as "Active" in TeamWork's own Status — a real discrepancy worth him checking, not a sync bug. `npx tsc --noEmit` and `npm run build` both clean.)

Previous update: 2026-09-03 (Added a "MANUALLY INVOICED" Remarks marker to Billing — Vincent walked through a real case, WOLVEZ CAPITAL PTE. LTD.: Chelsea invoiced two FYE cycles ahead of schedule at the client's request, one through this system (invoice 02611028, in `generated_invoices`) and one typed straight into QuickBooks (02611029) specifically to avoid confusing the two. Investigated the "No system invoice" / "not invoiced yet" logic before proposing anything: `app/api/billing/renewals/route.ts`'s `billedCyclesMap` parses a "dd.mm.yyyy" marker out of QuickBooks invoice-line descriptions (or reads `fye_date`) to recognize a cycle as billed even without a `generated_invoices` row — real, but a heuristic, and Chelsea's manually-typed QB line apparently doesn't match that exact pattern, so this one cycle kept showing as unbilled every time the page loaded. Explicitly flagged to Vincent that a plain Remarks note by itself would NOT stop this — nothing in the code reads remarks for this check — and asked which fix he wanted rather than picking one unilaterally, given this touches real billing-status logic (`CLAUDE.md`'s own standing rule on not inventing status-transition rules). He picked reusing the existing Remarks field with a parseable marker (matching Late Filing's own "Resolved:"-prefix precedent) over a new dedicated field or backfilling `generated_invoices` with an invoice that was deliberately kept out of the system. `app/billing/page.tsx`: new `manualInvoiceOverrides()` parses `MANUALLY INVOICED: TAB #<no>` (or TAC) out of `billingRemarks` (the same free-text AR Reminder Remarks field staff already edit — no new UI); `latestInvoiceNo()` falls back to it when no real `generated_invoices` row exists for the cycle, and `notInvoicedYet()` now treats a matching marker as billed, same as the existing `billedCycles` heuristic already does — so this stops re-flagging every year, not just once. The TAB/TAC invoice-reference badge shows a manually-noted number at reduced opacity with a tooltip ("Manually invoiced directly in QuickBooks — see Remarks") so staff can tell it apart from a real system-generated one at a glance. Verified the regex against real and edge-case inputs (marker embedded mid-sentence, lowercase, no marker present) before shipping. `npx tsc --noEmit` and `npm run build` both clean. **Still needs**: Vincent to actually type `MANUALLY INVOICED: TAB #02611029` into Wolvez Capital's Remarks field for the affected cycle — this fix makes the marker work, it doesn't retroactively apply itself.)

Previous update: 2026-09-03 (Shortened the header title to "TCS" — Vincent asked directly. `components/AppShell.tsx`'s top header text next to the logo changed from "Tassure Corporate Services System" to "TCS" — scoped to just that one spot (the app shell header), not the `<title>` metadata in `app/layout.tsx` or the login page's own heading, since Vincent pointed at the header specifically. `npx tsc --noEmit` and `npm run build` both clean.)

Previous update: 2026-09-03 (Fixed a real, production-breaking bug in the Explore rebuild — Vincent hit "This page couldn't load" on `/reports` right after the previous deploy. Root cause: `app/reports/page.tsx`'s own `usePagination(flowDrillRows, flowDrilldown)` call (for the "New This Year"/"Churned This Year" KPI-card drill-down) was placed AFTER three conditional early returns (`if (authorized === null) return...`, `if (error) return...`, `if (!data) return...`) — a genuine React Rules-of-Hooks violation: the hook is skipped entirely on the "still loading" renders and only called once `data` finally arrives, so React sees a different hook count between renders of the same component and hard-crashes with no error boundary on this route to catch it. This is exactly the kind of failure that shows as a raw browser-level "page couldn't load" rather than a normal error message, since it happens deep in React's own render machinery, not in any of this page's own try/catch blocks. `npx tsc --noEmit`/`npm run build` both stayed clean through this the whole time — this project's ESLint pass during `next build` did not catch it, worth a closer look separately (not fixed here) since `eslint-plugin-react-hooks`'s `rules-of-hooks` check normally does catch exactly this pattern. Fixed by moving the `usePagination` call (and the `flowDrillRows` derivation it depends on, made null-safe) above all three early returns, so it's called unconditionally on every render — confirmed `ExploreSection` (the other new component added this session) has no equivalent issue: none of its own hooks sit after any conditional return. `npx tsc --noEmit` and `npm run build` both clean.)

Previous update: 2026-09-03 (Reports rebuilt as an interactive "Explore" tool — Vincent reviewed the fixed-chart version shipped earlier today and said directly: "这个部分主要是给老板自己做分析的，所以是可以手动操作的...目前你提供的内容作用单一，看起来更像是一个摆设" (this needs to be manually operable — what's there now looks decorative). Asked what "手动操作" meant concretely, he picked all four: filters/dimension-switching, click-to-drill-down, a freeform dimension×metric "simple pivot table," and export. **First backfilled real SSIC data for the whole roster** (`scripts/backfill-ssic-full-roster.js`, a new committed script, not a throwaway diagnostic — mirrors the extraction/write logic already in `lib/teamwork-company-profile.ts`/`app/api/teamwork/sync-secretary/route.ts` but runs it against all ~900 companies immediately instead of waiting ~1 day for the natural nightly rotation): 789 companies updated, 111 genuinely have no SSIC on TeamWork (offshore L.P.s etc.), zero errors — verified live afterward, 776/909 active companies (85%) now carry real SSIC. Used Plan mode given the real scope: an Explore agent mapped existing filter/export/drill-down/pagination patterns across the codebase (`app/companies/page.tsx`'s click-to-filter-over-full-fetch idiom, `MasterListTable.tsx`'s private `ColumnFilterMenu`, `lib/export-columns.ts`'s `.xlsx`-only export convention — confirmed zero CSV precedent anywhere in this repo, `components/Pagination.tsx`'s `usePagination`/`PaginationBar`), then a Plan agent designed the concrete architecture — validated directly before finalizing by re-reading `app/api/export/company-data/route.ts` and `lib/export-columns.ts` myself, which confirmed the agent's most safety-relevant finding: **that reference export route has no auth check at all**, a gap the new Reports export route deliberately does not inherit. Shipped: new `lib/reports-data.ts` (the one companies+master_list join — exact-UEN matched, not fuzzy, since `app/api/master-list/move/route.ts`'s delete-then-reinsert pattern keeps at most one `master_list` row per UEN at any moment — deliberately NOT reaching for `lib/company-name.ts`'s fuzzy matching, which `docs/FEATURE_MAP.md` already flags as high-risk shared logic that's never run as a full-roster batch); `app/api/reports/route.ts` extended (purely additively — every existing field byte-for-byte unchanged) with a flat `companyRows` array (id/name/UEN/company type/SSIC/customer source/roster status/PIC/join date/service flags) for the new client-side Explore section to group/filter/pivot with zero network round-trip per interaction, plus per-year `flow.newByYearRows`/`churnedByYearRows` for the "New This Year"/"Churned This Year" KPI-card drill-down — sourced from `master_list`, never joined back to `companies`, because a struck-off company can be entirely absent from `companies` (confirmed fact already on record from `app/api/late-filing/route.ts`'s own comment: ADVANCE BRIGHT GLOBAL / FULLRICH INTERNATIONAL, both real examples) — joining back would have silently under-populated exactly the churned rows most likely to need it. New `app/api/reports/export/route.ts` (`.xlsx` via the existing `buildWorkbook()`/new `REPORTS_EXPORT_COLUMNS`, full active roster, WITH the `canViewReports` auth check the reference route lacks). New `components/dashboard/DimensionFilterMenu.tsx` — reimplements `ColumnFilterMenu`'s exact interaction (funnel icon → search → checkbox list with per-value counts → Select All/Clear → Cancel/OK) as a lighter, Reports-scoped component rather than exporting/genericizing the private original. `app/reports/page.tsx`'s new "Explore" section: 5 faceted filters (each menu's own counts computed by applying every OTHER active filter, standard faceted-filter behavior) for Company Type/SSIC/Customer Source/Roster Status/Secretary PIC, a dimension picker and a metric picker (Company Count, or a COUNTIF against each of the 6 service flags), a chart (Donut ≤8 groups, HBars + "Other" bucket beyond that — components themselves untouched), a plain "Value / Count / %" table that IS the "简易透视表" (deliberately one axis, not a full 2-D crosstab, to stay proportionate), and a click-through drill-down list (`usePagination`/`PaginationBar`, linking each company straight to Company 360) — plus the "New This Year"/"Churned This Year" KPI cards gained `onClick`/`active` for their own drill-down. Per-company revenue is explicitly NOT an Explore metric — stated on-page why (fuzzy-matching `quickbooks_invoices` against the full roster is a real accuracy risk this route has never taken before, worse in a leadership aggregate than in Company 360's single-company context) rather than silently omitted. **Verified against real production data before shipping, not just build-clean**: a direct query confirmed the company_type and SSIC pivots' counts sum EXACTLY to the active-company total (909, no double-counting/dropped rows), and that the join_date join correctly attaches real dates to real companies (77% coverage, sample-checked). `npx tsc --noEmit` and `npm run build` both clean. `docs/FEATURE_MAP.md`'s Reports row updated.)

Previous update: 2026-09-03 (Removed the Documents Generated section from Company 360 — Vincent, looking at it showing 0/empty: "这个不需要". Post Incorporate document generation is still early (only 1 of 13 planned templates live per `docs/CURRENT_STATE.md`), so this section showed an empty state for nearly every company — not useful clutter for now. Removed the render call + its now-unused import from `app/companies/[id]/page.tsx`; left `DocsGeneratedSection` itself and `getCompany360`'s underlying `documentsGenerated` data untouched (not deleted) in case it's worth bringing back once more document types exist. `npx tsc --noEmit` and `npm run build` both clean.)

Previous update: 2026-09-03 (Company 360 Communications status pill — Vincent pointed at the lowercase "sent" text and asked for it uppercase and slightly larger. `CommsSection` (`app/companies/[id]/_components.tsx`) was rendering `d.status` raw with `fontSize: 10.5`; added `textTransform: 'uppercase'` (not a manual `.toUpperCase()` call, matching how every column header label on this page already gets its uppercase styling) and bumped to `fontSize: 12`. `npx tsc --noEmit` and `npm run build` both clean.)

Previous update: 2026-09-03 (Company 360 header layout, take six — Vincent's final regroup: row 1 UEN/Status/Client Type/Company Type/FYE, row 2 Secretary PIC/Contact/Customer Source/SSIC Primary/SSIC Secondary, row 3 Invoice Address alone, with row 1 and row 2 each explicitly "等分5分" (divided into 5 equal columns) and row 3 needing no division, plus more breathing room between rows ("每一行的间距稍微拉长一点"). Both listed rows land at exactly 5 primary fields each — because that count matches on both sides, two separate `repeat(5, minmax(0,1fr))` grids share identical column boundaries by construction (same column count, same equal-width formula), so this doesn't need the previous fix's "one merged grid + explicit gridColumn line ranges" workaround for the 8-vs-3 mismatch — that trick existed specifically because the two rows had different column counts; with matching counts the simpler two-separate-grids structure aligns on its own. Widened the row-to-row gap from 16 to 28. Parent Company (conditional) placed in row 1's grid, Tel (conditional) in row 2's — Vincent's list only named the fields that always exist. `npx tsc --noEmit` and `npm run build` both clean.)

Previous update: 2026-09-03 (Officials no longer height-capped on Company 360 — Vincent, looking at a real 8-official company squeezed into a little internal scrollbox: "这一块不需要限制长度有多少显示多少". Every `DataCard`-based section on this page (`app/companies/[id]/_components.tsx`) shares a `system-list-scroll` wrapper hardcoded to `maxHeight: 360` — fine for occasional overflow, but Officials can genuinely run to 8+ real rows (every appointed role gets its own row) and forcing that into a scrollbox made it harder to read than just letting the card grow with the page. Added an opt-in `scrollable?: boolean` prop to `DataCard` (default `true`, unchanged for every other section — AR/AGM, Invoices, ND, Comms, Docs, Trademark, Shareholders all keep the 360px cap), set to `false` only on `OfficialsSection` per what Vincent actually asked about, not applied blanket to every list. `npx tsc --noEmit` and `npm run build` both clean.)

Previous update: 2026-09-03 (Company 360 header layout, take five — the fixed-column-count fix solved row 1's wrapping, but Vincent's next screenshot showed row 2's fields not lining up under row 1's columns at all, and he named the real priority directly: "我觉得最重要的不是100%填满，而是上下对齐" (vertical alignment matters more than filling the row). Root cause: row 1 and row 2 were still two SEPARATE grids — `repeat(8,...)` for row 1, `repeat(3,...)` for row 2 — so their column boundaries had nothing to do with each other; row 2's 3-way split just happened to land wherever a third of the container's width fell, not under any particular row-1 field. Merged into ONE grid for the whole header card (`repeat(8, minmax(0,1fr))`), with row 2's fields given explicit `gridColumn` line ranges (`'1 / 4'`, `'4 / 7'`, `'7 / 9'`) instead of relying on auto-placement — explicit lines, not spans, so their position is anchored regardless of whether Invoice Address/Parent Company/Tel (all conditional) happen to render, rather than shifting based on what rendered before them. `npx tsc --noEmit` and `npm run build` both clean.)

Previous update: 2026-09-03 (Company 360 header layout, take four — real root cause, not another regroup. Vincent's own screenshot showed row 1 splitting into two visual lines with a big empty gap ("有点不整齐") — Customer Source (the 8th item) had wrapped alone onto its own line. Root cause: the per-row grids from the last three fixes still used `repeat(auto-fit,minmax(180px,1fr))` — auto-fit decides its own column COUNT from how many 180px-minimum items actually fit per line, and at Vincent's real screen width only 7 of row 1's 8 items fit, silently dropping the 8th to a lonely second line. Explicit row grouping (which take two/three both delivered) was never the actual bug — the column count within each row was still non-deterministic. Fixed by switching both rows to a FIXED column count instead — `repeat(8, minmax(0,1fr))` for row 1, `repeat(3, minmax(0,1fr))` for row 2 (removing the now-redundant `gridColumn:'span 2'` on Invoice Address/SSIC, since 3 explicit columns already gives each its own full column) — a fixed count always renders as exactly one line at any width; cells get narrower instead of an item dropping to its own row. `minmax(0,1fr)`, not plain `1fr`, so a long value (an email, a long SSIC description) can't blow out its column via grid's default `min-width:auto` on children. `npx tsc --noEmit` and `npm run build` both clean.)

Previous update: 2026-09-03 (Company 360 header layout, take three — Vincent revised the spec again, collapsing back to 2 rows: row 1 UEN/Status/Client Type/Company Type/FYE/Secretary PIC/Contact/Customer Source, row 2 Invoice Address/SSIC Primary/Secondary (SSIC now shares a row with Invoice Address instead of getting its own). Same explicit-per-row-grid structure from the last fix, just regrouped — Parent Company stays with row 1's identity fields, Tel stays with row 1 too this time (previously grouped with Invoice Address in row 2; with Invoice Address now sharing a row with SSIC instead of Contact, Tel reads more naturally as an identity/contact-adjacent field like Contact itself, which Vincent explicitly kept in row 1). `npx tsc --noEmit` and `npm run build` both clean.)
