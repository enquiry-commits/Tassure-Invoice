...
Last updated: 2026-09-08 (Company 360's Outstanding section — Aging badge recolored yellow. Vincent, from a screenshot of the 31-60/61-90 badges: "放成黄色显示". Changed from the flat gray/navy shared with the Source badge to a yellow pairing (`#fef9c3`/`#854d0e`) — Source stays gray, since Aging is the one column that's actually a severity signal and should read as one at a glance. `npx tsc --noEmit` and `npm run build` both clean.
