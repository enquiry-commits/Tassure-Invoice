...
Last updated: 2026-09-08 (Company 360's Outstanding section — "Invoice" renamed to "Invoice No." and de-styled to match the Invoices section's own Invoice No. cell exactly. Vincent: "Invoice 换成 Invoice No. , 格式要参考 Invoice No. 列的字体格式". Same page's own Invoices section again — its Invoice No. cell is bare inherited text with no "#" prefix, so this drops both the custom `fontSize`/`lineHeight` override and the "#" this column used to add on its own initiative. `npx tsc --noEmit` and `npm run build` both clean.
