...
Last updated: 2026-09-03 (Company 360 Communications status pill — Vincent pointed at the lowercase "sent" text and asked for it uppercase and slightly larger. `CommsSection` (`app/companies/[id]/_components.tsx`) was rendering `d.status` raw with `fontSize: 10.5`; added `textTransform: 'uppercase'` (not a manual `.toUpperCase()` call, matching how every column header label on this page already gets its uppercase styling) and bumped to `fontSize: 12`. `npx tsc --noEmit` and `npm run build` both clean.)
