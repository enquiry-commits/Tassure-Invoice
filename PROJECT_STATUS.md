...
Last updated: 2026-09-08 (Company 360 — spacing between every section (module) increased 35%. Vincent: "每个板块之间的间距拉大35%". `app/companies/[id]/_components.tsx`'s `DataCard` (every module after the header card — Communications/Outstanding/AR-AGM/Officials/Shareholders/Invoices/ND/Trademark) `marginBottom` 16px → 21.6px; `app/companies/[id]/page.tsx`'s header card `marginBottom` 20px → 27px (both exactly ×1.35, kept as the precise value rather than rounded, since he gave an exact percentage). The unrelated `MatchQualityNote` warning banner's own `marginBottom: 16` left untouched — that's spacing around a conditional warning, not between two "板块"/section modules. `npx tsc --noEmit` and `npm run build` both clean.
