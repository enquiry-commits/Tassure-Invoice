...
Last updated: 2026-09-03 (Shortened the header title to "TCS" — Vincent asked directly. `components/AppShell.tsx`'s top header text next to the logo changed from "Tassure Corporate Services System" to "TCS" — scoped to just that one spot (the app shell header), not the `<title>` metadata in `app/layout.tsx` or the login page's own heading, since Vincent pointed at the header specifically. `npx tsc --noEmit` and `npm run build` both clean.)
