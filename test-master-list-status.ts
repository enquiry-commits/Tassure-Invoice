...
import {
  TERMINATED_STATUS, STRIKING_OFF_STATUS, placeholderStatusForMove, isTerminatedStatus,
...
console.log('--- rule 1: TeamWork wins (unchanged behaviour) ---');
{
...
console.log('\n--- rule 2: Terminated Services rows TeamWork cannot inform ---');
{
  const legacy = ['YES', 'NO', 'terminate', 'to be terminate', 'RENAMED', 'Mary', null, '', '  '];
  for (const status of legacy) {
    const p = plan([row({ status })]);
    check(`status ${JSON.stringify(status)} -> "Terminated"`, p.length === 1 && p[0].newValue === TERMINATED_STATUS && p[0].reason === 'terminated_list_default' && p[0].oldValue === status);
  }
  check('"Terminated" is left alone', plan([row({ status: 'Terminated' })]).length === 0);
  ...
  check('a row with no UEN at all still gets the default', plan([row({ roc_no: null, status: 'YES' })]).length === 1);
  check('a manual lock beats the default too', plan([row({ manual_fields: { status: true }, status: 'YES' })]).length === 0);
  check('TeamWork "Active" for a Terminated Services row is NOT forced to Terminated (follow TeamWork)', (() => {
    const p = plan([row({ status: 'TERMINATED' })], [['200000001A', 'Active']]);
    return p.length === 1 && p[0].newValue === 'Active' && p[0].reason === 'teamwork';
  })());
  check('TeamWork knows the company (status present) but the sync skipped its record -> no default', plan([row({ status: 'YES' })], [], ['200000001A']).length === 0);
  check('only the Terminated Services list gets the default', ['strike_off', 'active_client', 'ad_hoc', 'name_change', 'mas', 'inactive_old', null].every(list_type => plan([row({ list_type, status: 'YES' })]).length === 0));
}
...
console.log('\n--- the real 2026-09-24 shape of the Terminated Services list ---');
{
  ...
  check('269 rows in, exactly 186 rewritten to "Terminated" (180 YES + 6 odd values)', rows.length === 269 && p.filter(x => x.reason === 'terminated_list_default').length === 186);
  check('the 9 rows TeamWork still reports as Active / Striking Off are untouched', p.filter(x => x.reason === 'teamwork').length === 0);
  check('nothing else is touched', p.length === 186);
}

console.log('\n--- rule 3: the Move placeholder ---');
{
  check('Terminated Services -> "Terminated" (TeamWork\'s own wording)', placeholderStatusForMove('terminated') === 'Terminated' && TERMINATED_STATUS === 'Terminated');
  ...
  check('isTerminatedStatus is case-insensitive, not fuzzy', isTerminatedStatus('TERMINATED') && isTerminatedStatus(' terminated ') && !isTerminatedStatus('terminate') && !isTerminatedStatus('to be terminate') && !isTerminatedStatus(null));
}
...
console.log('\n--- source guards: nothing bypasses the shared rules ---');
{
  ...
  check('the Active Client page uses the shared constants, not string literals, for its two Move targets',
    /type:\s*'strike_off'[^}]*statusValue:\s*STRIKING_OFF_STATUS/.test(page) && /type:\s*'terminated'[^}]*statusValue:\s*TERMINATED_STATUS/.test(page)
    && !/statusValue:\s*'(TERMINATED|Terminated|STRUCK OFF|Striking Off)'/.test(page));
